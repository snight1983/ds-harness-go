...
// LogBaseSeq 给出一整段日志的起点，也就是它第一条事件的 seq；空段给 0。
//
// 新增: 上游没有这个函数——它的日志从 0 起、一条不删，起点是个常数。本仓库的
// 日志会从最老的一头弹出事件（见 docs/session-log-limit.md 的原则第 1 条），
// 于是「这一段从哪儿起」得当场问出来。
//
// 只给**整段**日志用：一段脱离了会话的完整日志，它的起点就是自己第一条的 seq，
// 没有别的权威可问。要拿一段中间的碎片去折表面本来就是错的，那种情况下这个数
// 也帮不上忙。空段给 0 不违反原则第 3 条：一段没有事件的日志上，任何按 seq
// 的换算都不会发生。
func LogBaseSeq(events []Event) int {
	if len(events) == 0 {
		return 0
	}
	return events[0].Seq
