// Package sessionlog 是会话事件日志这套词汇：一条日志条目长什么样、有哪几种事件、
...
// # 不做什么
//
//   - **没有活的会话。**事件总线、发布时机、seed 生命周期都在 harness/session，
//     这里只有值和纯函数。
//   - **不落盘。**事件存哪儿、怎么读回来是 feature/persistence 那条接缝的事，
//     本包连一个存储接口都不定义。
//   - **不解释事件数据。**[Event.Data] 是原始字节，读不懂的类型原样保管；
//     要具名字段自己调 [DecodeData]，认不出来的落进 [RawData]。
//   - **不算读模型。**把日志折成待办清单、用量、统计那些值的框架在 sessionlog/projection，
//     折出来的东西也永远不是权威——权威只有日志。
//   - **不猜着修。**[InterruptedTurnClosers] 只补那些能确定怎么补的断口，
//     判不安全的时候直接失败，不做「尽量恢复」。
package sessionlog
