...
func Register[S any](registry *Registry, definition Definition[S]) (func(), error) {
	...
	if ok {
		...
		registry.registrations[erased.key] = &registration{
			def:   erased,
			cells: map[sessionlog.SessionID]*unitCell{},
			refs:  1,
		}
	}
	registry.mu.Unlock()
...
// 单元格会照常在那时的日志上重折一遍。
func (r *Registry) Forget(id sessionlog.SessionID) {
	r.mu.Lock()
	defer r.mu.Unlock()
...
func (r *Registry) Drive(view SessionView, event sessionlog.Event) {
...
func buildCell(def erasedDefinition, events []sessionlog.Event) *unitCell {
	state := def.init()
	observed := -1
...
// 下标」这条更强的前提。本仓库的 [sessionlog.Trace] 只保证 seq 严格递增，
// 不保证从 0 起密排，所以这里按 seq 比，不按下标切。
func eventsBefore(events []sessionlog.Event, boundary int) []sessionlog.Event {
