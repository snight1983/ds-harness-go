...
// 所以下面用互斥锁保护登记簿。这不是照录来的，是 Go 侧的必需品。
//
// # 不做什么
//
//   - **不修复。**发现一次违例只报出来，不回滚、不补齐、不改任何人的状态。
//   - **不替代普通输入校验。**参数不合法该在原地返回错误；这里只放跨事件、
//     跨生命周期、正常代码路径永远不会违反的那类持续约束。
//   - **不自己写检查。**每条检查由拥有那条约定的包注册（见 fs、llm、credentials
//     各自的 RegisterInvariants），注册表只负责挑选、隔离和归属。
//   - **不决定生产环境开不开。**装不装、失败是不是阻断服务，由宿主的装配代码定。
//   - **不做离线审计。**要拿一份历史日志重新验一遍，用各模块自己那份纯校验函数
//     （例如 sessionlog 的 ValidateLog），那条路不经过这个注册表。
package invariants
...
type Registry struct {
	...
	// 不同的 goroutine，所以登记簿必须自己上锁。这一层不是照录来的。
	mutex   sync.Mutex
	closed  bool
