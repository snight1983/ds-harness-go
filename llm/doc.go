...
// # 不做什么
//
//   - **不带任何一个具体提供方。**一个空的 [Runtime] 一条路由都没有；
//     OpenAI 兼容那份实现在 adapter/openaicompat。
//   - **不跑 agent 回合，也不跑工具循环。**一次生成之外的编排在 harness/agentloop，
//     系统提示词的内容在 harness/systemprompt。
//   - **不执行重试。**[ResolvedRetryPolicy] 只是一条路由登记那一刻定下来的声明，
//     照着它重发请求的是 feature/llmretry。
//   - **不做鉴权和计费。**[AssertUsableAPIKey] 只判一份密钥本身能不能用，
//     不判调用方有没有资格用它；用量累计和上下文占用率在 feature/tokenmeter。
//   - **不抹平提供方之间的行为差异。**词汇统一不等于结果一致，
//     模型答成什么样由提供方决定，本包只保证形状是同一种。
package llm
