...
func rawEvent(seq int, kind session.EventType, raw string) session.Event {
...
func turnEnd(seq int) session.Event {
	return rawEvent(seq, session.EventTurnEnd, `{"turn":1,"reason":{"kind":"completed"}}`)
}

// stepStart 排一条 step/start。
func stepStart(t *testing.T, seq, turn, step int) session.Event {
	t.Helper()
...
func stepEnd(t *testing.T, seq, turn, step int) session.Event {
	t.Helper()
...
func header(seq int, provider string) session.Event {
	return rawEvent(seq, session.EventRequestHeader, fmt.Sprintf(
		`{"header":{"config":{"provider":%s,"model":"m-1"}},"reason":"initial"}`,
		strconv.Quote(provider)))
}

// retryEvent 排一条 llm/retry。
func retryEvent(t *testing.T, seq int, data RetryData) session.Event {
	t.Helper()
...
func startedEvent(t *testing.T, seq int, data RetryStartedData) session.Event {
	t.Helper()
...
func openStepLog(t *testing.T, provider string) []session.Event {
	t.Helper()
...
func (f *fakeSession) Events() []session.Event {
	f.mutex.Lock()
	defer f.mutex.Unlock()
	return append([]session.Event(nil), f.log...)
}

func (f *fakeSession) Append(candidate session.Event) (session.Event, error) {
	f.mutex.Lock()
	if candidate.Type == f.failOn {
		f.mutex.Unlock()
		return session.Event{}, fmt.Errorf("假会话拒收 %s", candidate.Type)
	}
...
func (f *fakeSession) types() []session.EventType {
	kinds := make([]session.EventType, 0, len(f.log))
