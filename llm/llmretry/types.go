...
const (
	...
	EventRetry session.EventType = "llm/retry"
	// EventRetryStarted 记下那段等待熬过去了，下一次请求尝试就要发出去。
	EventRetryStarted session.EventType = "llm/retry-started"
)
...
func DecodeRetry(event session.Event) (RetryData, error) {
	return decodePayload[RetryData](event, EventRetry)
}

// DecodeRetryStarted 读回一条 llm/retry-started 的负载。
func DecodeRetryStarted(event session.Event) (RetryStartedData, error) {
