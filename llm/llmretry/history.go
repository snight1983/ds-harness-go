...
func routeTransition(event session.Event) (routeDelta, error) {
	switch event.Type {
	case session.EventTurnStart:
		var data session.TurnStartData
		if err := json.Unmarshal(event.Data, &data); err != nil {
			return routeDelta{}, fmt.Errorf("%s 的负载读不回来：%w", session.EventTurnStart, err)
		}
	...
	case session.EventStepStart:
		...
		if err := json.Unmarshal(event.Data, &data); err != nil {
			return routeDelta{}, fmt.Errorf("%s 的负载读不回来：%w", session.EventStepStart, err)
		}
	...
	case session.EventRequestHeader:
		...
		if err := json.Unmarshal(event.Data, &data); err != nil {
			return routeDelta{}, fmt.Errorf("%s 的负载读不回来：%w", session.EventRequestHeader, err)
		}
...
func ProviderForOpenStep(events []session.Event, turn, step int) (string, bool, error) {
	trace := NewTrace()
