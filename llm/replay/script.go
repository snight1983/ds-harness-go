...
var packedRowTypes = map[session.RowType]bool{
	session.RowTextChunks:      true,
	session.RowReasoningChunks: true,
	session.RowToolCallChunks:  true,
}
...
// 仍旧是完整的事件值。
func ParseSessionLog(text string) ([]session.Event, error) {
	...
	for index, raw := range strings.Split(text, "\n") {
		...
		decoded, err := session.DecodeStorageRecord(record)
		if err != nil {
...
func ParseSessionHeader(text string) (session.SessionHeader, error) {
	for _, raw := range strings.Split(text, "\n") {
		...
		if err := json.Unmarshal([]byte(line), &header); err != nil {
			return session.SessionHeader{}, fmt.Errorf("%w：会话头读不回来：%w", ErrMalformedFixture, err)
		}
		return header, nil
	}
...
// turn 和 step。
func DeriveScript(events []session.Event) ([]Entry, error) {
	var script []Entry
