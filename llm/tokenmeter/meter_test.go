...
func assistantEventFrom(t *testing.T, turn, step int, text string, usage *llm.TokenUsage, sources []int) session.Event {
	t.Helper()
...
func measure(t *testing.T, meter *TokenMeter, view *fakeSession, header *session.EpochHeader) Measurement {
	t.Helper()
...
func TestReplayRejectsMalformedLogs(t *testing.T) {
	...
	cases := map[string]*fakeSession{
		...
		"请求头读不回来": {id: "s", events: []session.Event{
			{Seq: 0, Type: session.EventRequestHeader, Data: []byte(`{"header":42}`)},
		}},
	}
