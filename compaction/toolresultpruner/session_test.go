...
func appendEvent(t *testing.T, live *coresession.Session, kind session.EventType,
	payload any, op session.SurfaceOp, sources []int,
) session.Event {
	t.Helper()
	...
	event, err := live.Append(session.Event{
...
func toolResultAt(t *testing.T, live *coresession.Session, callID llm.CallID, text string,
	...
	return appendEvent(t, live, session.EventToolResult, session.ToolResultData{
...
func decodeToolResult(t *testing.T, event session.Event) session.ToolResultData {
	t.Helper()

	decoded, err := session.DecodeData(event)
	if err != nil {
		t.Fatalf("tool/result 解不回来：%v", err)
	}
	data, ok := decoded.(session.ToolResultData)
	if !ok {
...
func TestPruneSession计价事件紧贴在替换件前面(t *testing.T) {
	...
	if replacement.Type != session.EventToolResult {
...
func TestPruneSession替换件只换正文(t *testing.T) {
	...
	if _, ok := replacement.SurfaceOp.(session.ReplaceOp); !ok {
...
func TestPruneSession不是工具结果的节点一律跳过(t *testing.T) {
	...
	live := newLive(t)
	appendEvent(t, live, session.EventUserMessage, session.UserMessageData{
...
func TestPruneSession形状不对的结果消息跳过(t *testing.T) {
	...
	for name, content := range map[string]llm.Content{
		...
		t.Run(name, func(t *testing.T) {
			...
			live := newLive(t)
			appendEvent(t, live, session.EventToolResult, session.ToolResultData{
...
func TestPruneSession负载读不回来就停在动手之前(t *testing.T) {
	...
	if _, err := live.Append(session.Event{
		Type: session.EventToolResult, Data: json.RawMessage("[1,2]"), SurfaceOp: session.AppendOp{},
	}); err != nil {
