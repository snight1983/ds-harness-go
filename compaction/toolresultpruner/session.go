...
func (p *Pruner) PruneSession(
	...
	for _, seq := range live.SurfaceNodes() {
		...
		if !ok {
			// 不可达：[session.DecodeData] 按 Type 分发，tool/result 只会得到这一种
	...
	for _, item := range candidates {
		...
		replacement, err := appendPayloadWith(live, session.EventToolResult, replaced,
			session.ReplaceOp{Start: item.seq, End: item.seq}, []int{item.seq})
		if err != nil {
			return result, err
		}
...
// 所以下标要先减掉这个基准。
func baseSeqOf(events []session.Event) int {
	if len(events) == 0 {
		return 0
	}
	return events[0].Seq
...
func appendPayload(live *coresession.Session, kind session.EventType, payload any) (session.Event, error) {
...
func appendPayloadWith(
	...
	if err != nil {
		return session.Event{}, fmt.Errorf("compaction/toolresultpruner：%s 的负载排不出去：%w", kind, err)
	}
	...
	if err != nil {
		return session.Event{}, fmt.Errorf("compaction/toolresultpruner：%s 写不进日志：%w", kind, err)
	}
	return event, nil
}
