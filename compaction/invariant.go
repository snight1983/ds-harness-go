...
// [session.Trace] 那条逐字相同；本包的 Trace 就是一个普通的值，谁拥有它
...
//
// 分成「验」和「落」两步，理由和 [session.Transition] 逐字相同：一条事件在
...
// 转移**之后**的完整状态，[Trace.Apply] 只管赋值——这也是 [session.Transition]
// 的做法，两个包里同一件事写成同一个样子。
...
//
// 只返回**第一条**违反，理由和 [session.Trace.Validate] 逐字相同。
...
// 一段继承来的日志时为真，见那里的注释。
func (t Trace) validate(event session.Event, staleBracket bool) (Transition, error) {
	next := Transition{
		openTurn:     t.OpenTurn,
	...
	// 一个归属。
	if event.Type == session.EventTurnStart || event.Type == session.EventTurnEnd {
	...
	case session.EventUserMessage:
		if !session.IsReplacementSurfaceEvent(event) {
			return next, nil
		}
...
func (t Trace) validateStart(next Transition, event session.Event) (Transition, error) {
...
func (t Trace) validateSummary(next Transition, event session.Event) (Transition, error) {
...
func (t Trace) validateEnd(next Transition, event session.Event) (Transition, error) {
...
// 认出来之后合不合规才是不变量的事。
func (t Trace) validateCheckpoint(event session.Event) error {
...
func (t Trace) validateOwner(seq int, kind session.EventType, turn int, standalone bool) error {
	if standalone {
		if t.TurnIsOpen {
...
func orphanStartSeqs(events []session.Event) map[int]struct{} {
...
func decodeUserMessage(event session.Event) (session.UserMessageData, error) {
	data, err := session.DecodeData(event)
	if err != nil {
		return session.UserMessageData{}, fmt.Errorf("%w：seq %d 的 user/message：%w",
			ErrMalformedEvent, event.Seq, err)
	}
	...
	if !ok {
		...
		// 不属于当前压缩的检查点。
		return session.UserMessageData{}, fmt.Errorf("%w：seq %d 声称是 user/message，负载却是 %T",
			ErrMalformedEvent, event.Seq, data)
	}
	return message, nil
}
