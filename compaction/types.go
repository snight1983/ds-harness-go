...
const (
	...
	EventCompactionStart session.EventType = "compaction/start"
	// EventCompactionSummary 记下做完的摘要、它的输入、以及那次模型调用的事实。
	EventCompactionSummary session.EventType = "compaction/summary"
	// EventCompactionEnd 标记一次压缩结束并放锁。
	EventCompactionEnd session.EventType = "compaction/end"
	// EventCompactionPrune 记下一次不过模型的裁剪替换的影子价格。
	EventCompactionPrune session.EventType = "compaction/prune"
)

// EventTypes 是本包往会话日志里写的那几种事件类型。
//
// 新增: DSH 靠 `declare module` 把这四个类型合并进 `SessionEventMap`，全局登记表
// 因此自动认得它们。Go 没有声明合并，[session.Vocabulary] 也是个闭合的值，
// 所以改成由本包交出这张单子，装配方自己拼：
//
//	vocabulary := session.CoreVocabulary().With(compaction.EventTypes()...)
//
// 不这么做的话，一段带压缩的日志会被 [session.CheckVocabulary] 判成
// 「有不认识的事件类型」而整个拒掉。
...
func decodeOwner(kind session.EventType, raw json.RawMessage) (int, bool, error) {
	if len(raw) == 0 {
...
// 把负载类型收窄了。[session.EventData] 是个**封闭**接口（带一个不可导出的方法），
// 本包的四种负载进不去那个联合，所以 [session.DecodeData] 只会把它们交成
...
func wrapDecode(event session.Event, kind session.EventType, err error) error {
