...
func InspectEntryState(events []session.Event) (EntryState, error) {
	var state EntryState
	turnKnown, compactionKnown := false, false
	for index := len(events) - 1; index >= 0; index-- {
		event := events[index]
		if !state.HasEndSeed && event.Type == session.EventSessionEndSeed {
...
func CheckNoActiveCompaction(events []session.Event, stage string) error {
...
func decodeTurnStart(event session.Event) (int, error) {
