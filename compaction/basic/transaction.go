...
// 本仓库真的那个计量器（llm/tokenmeter）的 Measure 收的是一份投影视图加一份
...
func commitCompactionBody(
	...
	payload, err := json.Marshal(session.UserMessageData{Message: summarized.Checkpoint})
	if err != nil {
	...
	if _, err := live.Append(session.Event{
		...
		Data:            payload,
		SurfaceOp:       session.ReplaceOp{Start: summarized.Region.Start, End: summarized.Region.End},
...
//
// 新增: [session.EventData] 是个封闭接口，本包的三种负载进不去那个联合，
...
func appendPayload(
	...
	if err != nil {
		return session.Event{}, fmt.Errorf("compaction-basic：%s 的负载排不出去：%w", kind, err)
	}
	event, err := live.Append(session.Event{Type: kind, Data: data})
	if err != nil {
		return session.Event{}, fmt.Errorf("compaction-basic：%s 写不进日志：%w", kind, err)
	}
	return event, nil
}
