...
// # 不做什么
//
//   - **不内置任何 Backend。**文件、数据库、远程配置都不在这里，
//     [Backend] 由装配方从构造函数递进来。
//   - **不加密。**[Redact] 只保护展示路径，秘密值本身怎么存归后端；
//     真正的凭据走 credentials 那条引用接缝，不该整份写进设置文档。
//   - **不解释 schema。**[Options.Schema] 原样带给配置界面，
//     本包不校验它、也不替登记方从 Go 类型生成一份。
//   - **不提供跨进程共识。**队列只串本进程的写，修订号是乐观并发，
//     跨副本能不能挡住冲突取决于 [Backend] 的实现。
//   - **不替登记方定义业务校验。**类型对不对由 encoding/json 判，
//     值合不合理由登记方自己的 Validate 判。
package settings
