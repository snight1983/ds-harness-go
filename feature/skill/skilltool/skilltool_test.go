...
import (
	...
	"github.com/snight1983/ds-harness-go/feature/skill/skilltool"
...
// 见 [sessionlog.SessionHeader.WorkspaceID]。本包只把它原样递给注册表。
var callerWorkspaceID = sessionlog.WorkspaceID("ws-skilltool")
...
type stubCatalog struct {
	...
	seenWorkspace sessionlog.WorkspaceID
}
...
func newWorld(t *testing.T, catalog *stubCatalog, config skilltool.Config) *world {
	...
	sess, err := coresession.NewSession(id, coresession.Options{
		Header: &sessionlog.SessionHeader{ID: id, WorkspaceID: callerWorkspaceID},
	})
	if err != nil {
...
func (w *world) fireStep() agent.PreStepDecision {
	...
	for _, message := range decision.Messages {
		w.appendUserMessage(message, sessionlog.AppendOp{})
	}
	return decision
}

// appendUserMessage 往日志里落一条上表面的用户消息。
func (w *world) appendUserMessage(message llm.Message, op sessionlog.SurfaceOp) sessionlog.Event {
	w.t.Helper()
	data, err := json.Marshal(sessionlog.UserMessageData{Message: message})
	if err != nil {
		w.t.Fatalf("排用户消息失败：%v", err)
	}
	appended, err := w.agent.sess.Append(sessionlog.Event{
...
func (w *world) openTurn() {
	...
	if _, err := w.agent.sess.Append(sessionlog.Event{Type: sessionlog.EventTurnStart, Data: data}); err != nil {
		w.t.Fatalf("追加 turn/start 失败：%v", err)
	}
	w.appendUserMessage(userSay("turn 1"), sessionlog.AppendOp{})
}
...
func TestAMalformedDurableCatalogNeverFailsTheStep(t *testing.T) {
	...
	for _, extra := range []string{
		...
		w.appendUserMessage(seededCatalog(t, "unreadable catalog", extra), sessionlog.AppendOp{})
	}
...
func TestTheCatalogIsReestablishedAfterCompactionHidesIt(t *testing.T) {
	...
	data, err := json.Marshal(sessionlog.UserMessageData{Message: pluginSay("compact", "compacted history")})
	if err != nil {
		t.Fatalf("排压缩消息失败：%v", err)
	}
	if _, err := w.agent.sess.Append(sessionlog.Event{
		Type:            sessionlog.EventUserMessage,
		Data:            data,
		SurfaceOp:       sessionlog.ReplaceOp{Start: initialSeq, End: initialSeq},
