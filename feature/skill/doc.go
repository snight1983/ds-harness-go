...
// # 不做什么
//
//   - **不执行技能。**技能是模型读完之后照着做的话，不是它能调的函数；能调的那些在
//     [github.com/snight1983/ds-harness-go/tools]，驱动它们的循环在
//     [github.com/snight1983/ds-harness-go/harness/agentloop]。
//   - **不产出技能。**从哪来由提供方决定——本地目录、打包进来的一批、远端注册中心都行，
//     本包只把清单合起来、给同名的定一个赢家、把赢家的正文交出去。
//   - **不下载、不编译、不动态加载。**提供方是编译期就在那儿的实现，宿主自己登记进来。
//   - **不信任技能正文。**一份技能能让模型干成什么，仍旧由宿主给的工具和数据权限
//     决定，不由这段文字决定。
//   - **不管上下文预算。**正文渲染成一个块交出去，塞不塞得下、塞几份归调用方。
//   - **不缓存不完整的发现结果。**某个提供方报错或者自称 Incomplete 时那一份永远不进
//     缓存；消费方拿到 [CatalogSnapshot].Complete 为假时应当留着上一份好的。
package skill
