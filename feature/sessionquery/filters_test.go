...
func filterRecords() []Record {
	return []Record{
		{Header: sessionlog.SessionHeader{ID: "a", CreatedAt: 100, WorkspaceID: "ws-x"}, Live: true},
		{Header: sessionlog.SessionHeader{ID: "b", CreatedAt: 200, WorkspaceID: "ws-y", ParentSession: "a"}, Persisted: true},
		{Header: sessionlog.SessionHeader{ID: "c", CreatedAt: 300}, Live: true, Persisted: true},
	}
}
...
func TestFilterSessionsAndsEveryClauseAndKeepsInputOrder(t *testing.T) {
	...
	cases := map[string]struct {
		...
		want    []sessionlog.SessionID
	}{
		"没有过滤器就全留下": {want: []sessionlog.SessionID{"a", "b", "c"}},
		"按 id 取":    {filters: []SessionFilter{IDFilter{Values: []sessionlog.SessionID{"c", "a"}}}, want: []sessionlog.SessionID{"a", "c"}},
		"按工作目录取": {
			filters: []SessionFilter{WorkspaceFilter{Values: []sessionlog.WorkspaceID{"ws-y"}}},
			want:    []sessionlog.SessionID{"b"},
		},
		"空串就是没有工作目录": {
			filters: []SessionFilter{WorkspaceFilter{Values: []sessionlog.WorkspaceID{""}}},
			want:    []sessionlog.SessionID{"c"},
		},
		"按建会话时间取": {
		...
		"下界不封顶": {
			...
			want:    []sessionlog.SessionID{"b", "c"},
		},
		"上界不封底": {
			filters: []SessionFilter{CreatedAtFilter{Range: Range{To: bound(100)}}},
			want:    []sessionlog.SessionID{"a"},
		},
		"按父会话取": {
			filters: []SessionFilter{ParentFilter{Values: []sessionlog.SessionID{"a"}}},
			want:    []sessionlog.SessionID{"b"},
		},
		"空串就是没有父会话": {
			filters: []SessionFilter{ParentFilter{Values: []sessionlog.SessionID{""}}},
			want:    []sessionlog.SessionID{"a", "c"},
		},
		"只要活着的": {
		...
		"两个来源之间是或": {
			...
			want:    []sessionlog.SessionID{"a", "b", "c"},
		},
		"两条过滤器之间是与": {
			filters: []SessionFilter{
		...
		"一个都留不下": {
			filters: []SessionFilter{IDFilter{Values: []sessionlog.SessionID{"没有这个"}}},
		},
	}
	...
	for name, testCase := range cases {
		t.Run(name, func(t *testing.T) {
			...
			ids := make([]sessionlog.SessionID, 0, len(kept))
...
func TestFilterSessionsAlsoTakesTypesThatEmbedARecord(t *testing.T) {
	...
	hits := []SearchHit{
		{Record: Record{Header: sessionlog.SessionHeader{ID: "a"}, Live: true}},
		{Record: Record{Header: sessionlog.SessionHeader{ID: "b"}}},
	}
...
func filterDocuments() []EventSearchDocument {
	return []EventSearchDocument{
		{EventRecord: EventRecord{Seq: 0, Type: sessionlog.EventUserMessage, Time: 100, Surface: SurfaceShadowed}, Text: "改一下  首页 标题"},
		{EventRecord: EventRecord{Seq: 1, Type: sessionlog.EventAssistantMessage, Time: 200, Surface: SurfaceCurrent}, Text: "改好了"},
		{EventRecord: EventRecord{Seq: 2, Type: sessionlog.EventTurnStart, Time: 300, Surface: SurfaceLogOnly}, Text: ""},
	}
}
...
func TestFilterEventDocumentsAndsEveryClause(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"按类型取": {
			filters: []EventFilter{TypeFilter{Values: []sessionlog.EventType{sessionlog.EventUserMessage}}},
			want:    []int{0},
		},
		"按表面位置取": {
...
func TestMaterializeSessionFiltersCopiesEveryClause(t *testing.T) {
	...
	ids := []sessionlog.SessionID{"a"}
	workspaces := []sessionlog.WorkspaceID{"ws-x"}
	parents := []sessionlog.SessionID{"p"}
...
func TestMaterializeEventFiltersCopiesAndValidates(t *testing.T) {
	...
	types := []sessionlog.EventType{sessionlog.EventUserMessage}
	...
	if owned[2].(TypeFilter).Values[0] != sessionlog.EventUserMessage {
		t.Fatal("类型那一列没有复制")
	}
