...
// 但重放校验的两半都已经有了：[sessionlog.ValidateLog] 验关系约束，
// [sessionlog.FoldSurface] 验表面层折不折得出来。这里两半都做，
// 少掉的只是「建出一个活会话」——本方法本来也不需要它。
func (e *Engine) ReadSession(ctx context.Context, id sessionlog.SessionID) (LogSnapshot, error) {
	loaded, err := e.corpus.Load(ctx, id)
	if err != nil {
		return LogSnapshot{}, err
	}
	if _, err := sessionlog.ValidateLog(loaded.Events); err != nil {
		return LogSnapshot{}, wrap(CodeCorruptSession, err, "会话 %q 的日志重放不过", id)
	}
	if _, err := sessionlog.FoldSurface(loaded.Events, sessionlog.LogBaseSeq(loaded.Events)); err != nil {
...
func (e *Engine) ListEvents(ctx context.Context, id sessionlog.SessionID) ([]EventRecord, error) {
	loaded, err := e.corpus.Load(ctx, id)
	if err != nil {
		return nil, err
	}
...
func (e *Engine) ReadSurface(ctx context.Context, id sessionlog.SessionID) (SurfaceSnapshot, error) {
	loaded, err := e.corpus.Load(ctx, id)
	if err != nil {
...
func (e *Engine) TraceSession(ctx context.Context, id sessionlog.SessionID) (LineageTrace, error) {
	records, err := e.corpus.ListSessions(ctx)
	if err != nil {
...
func (e *Engine) ReadEvent(ctx context.Context, request EventReadRequest) (EventWindow, error) {
	...
	// 切片的时候当场减起点。
	baseSeq := sessionlog.LogBaseSeq(loaded.Events)
