...
//     [github.com/snight1983/ds-harness-go/feature/sessiontitle.FoldSnapshot] 的第二个返回值一致。
...
// # 不做什么
//
//   - **不实现全文检索。**没有索引、没有排序算法、没有游标世代；这一层只有 [Searcher]
//     这个挂点和一套请求/结果形状，没挂时两个检索方法一律返回 [CodeSearchDisabled]。
//   - **不写、不改、不修会话。**接口层面就写不了；日志坏了只报，不猜也不补。
//   - **不管来源的生命周期。**活会话表、持久化后端、检索后端的挂载与卸载全归装配方，
//     本包只在 [Options] 里收它们。
//   - **不提供跨节点一致性。**「活的优先」是本进程此刻的可见性规则，不是分布式协议。
//   - **不做鉴权和租户隔离。**摆给模型的那条工作目录边界在
//     [github.com/snight1983/ds-harness-go/feature/sessionquery/querytool]，它防的是模型越界，不是防人。
//   - **不自己计时。**取消点全落在调用方传进来的那条 ctx 上。
