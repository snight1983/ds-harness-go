...
func EventRecords(id sessionlog.SessionID, events []sessionlog.Event) ([]EventRecord, error) {
...
func CurrentSurfaceEvents(id sessionlog.SessionID, events []sessionlog.Event) ([]sessionlog.Event, error) {
	analysis, err := analyzeEventLog(id, events)
	if err != nil {
		return nil, err
	}
	surface := make([]sessionlog.Event, 0, len(analysis.currentSeqs))
	for _, seq := range analysis.currentSeqs {
		event, ok := eventAtSeq(events, seq)
		if !ok || !sessionlog.IsSurfaceEvent(event) {
...
func TraceEvent(id sessionlog.SessionID, events []sessionlog.Event, seq int) (EventTrace, error) {
	...
	return EventTrace{
		...
		Target:            analysis.records[seq-sessionlog.LogBaseSeq(events)],
...
func TraceSession(records []Record, id sessionlog.SessionID) (LineageTrace, error) {
	byID := make(map[sessionlog.SessionID]Record, len(records))
	...
	trace := LineageTrace{
		Target:      target,
		Ancestors:   ancestors,
		Descendants: buildDescendants(childrenByParent, id, map[sessionlog.SessionID]bool{id: true}),
	}
	if unresolved != "" {
...
func traceAncestors(byID map[sessionlog.SessionID]Record, target Record) ([]Record, sessionlog.SessionID, error) {
	var ancestors []Record
	seen := map[sessionlog.SessionID]bool{target.Header.ID: true}
...
// 会让同一份数据的两次查询给出不同的字节。
func groupChildren(records []Record) map[sessionlog.SessionID][]Record {
	childrenByParent := make(map[sessionlog.SessionID][]Record)
	for _, record := range records {
...
func buildDescendants(
	childrenByParent map[sessionlog.SessionID][]Record,
...
func analyzeEventLog(id sessionlog.SessionID, events []sessionlog.Event) (eventLogAnalysis, error) {
...
func eventAtSeq(events []sessionlog.Event, seq int) (sessionlog.Event, bool) {
	index := seq - sessionlog.LogBaseSeq(events)
