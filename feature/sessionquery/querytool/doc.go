...
// # 不做什么
//
//   - **不做读、过滤、检索和追溯。**那些一行都不在这里，全在
//     [github.com/snight1983/ds-harness-go/feature/sessionquery]。
//   - **不做人的鉴权。**它只画一条**工作目录**边界，防的是模型越界；租户和用户权限
//     归宿主。
//   - **不透露越界目标存不存在。**所有越界一律报同一句话，不区分「不在你的工作区」
//     和「压根没这个会话」。
//   - **不把引擎的错误码交给模型。**那些码是内情，一律翻成本包自己的 [Code] 加一句
//     安全说法，原始的那条错误链只进日志。
//   - **不自己计时。**超时是 [github.com/snight1983/ds-harness-go/tools.Definition.Timeout]，由工具
//     运行时统一执行，也统一不发给模型。
//   - **不保证交给模型的结果是全量。**结果上限和工作区边界都会把它裁小，裁到了会明说。
package querytool
