...
func formatSessionSearch(
	...
	titles map[sessionlog.SessionID]titleView,
	authorizedParents map[sessionlog.SessionID]struct{},
) string {
...
func renderDescendants(nodes []*authorizedDescendant, titles map[sessionlog.SessionID]titleView) []string {
	var lines []string
...
func formatEventTrace(sessionID sessionlog.SessionID, title titleView, trace sessionquery.EventTraceObservation) string {
	replacedBy := "none"
	if trace.Shadowed {
...
func formatEventRead(sessionID sessionlog.SessionID, title titleView, window sessionquery.EventWindow) (string, error) {
	...
	var before, after []sessionlog.Event
...
// 这个取舍和 [github.com/snight1983/ds-harness-go/feature/sessionquery] 的 doc.go 第 6 条是同一个。
func formatNeighbor(event sessionlog.Event) (string, error) {
