...
func buildSessionFilters(args sessionSearchArgs) ([]sessionquery.SessionFilter, error) {
	...
	if args.SessionIDs != nil {
		...
		ids := make([]sessionlog.SessionID, 0, len(args.SessionIDs))
		for _, id := range args.SessionIDs {
			ids = append(ids, sessionlog.SessionID(id))
		}
...
// 重复的 id 会让同一个父会话被数两遍。
func materializeParentSessionIDs(values []string) ([]sessionlog.SessionID, bool, error) {
	if values == nil {
	...
	seen := make(map[sessionlog.SessionID]struct{}, len(values))
	unique := make([]sessionlog.SessionID, 0, len(values))
	for _, value := range values {
		id := sessionlog.SessionID(value)
		if _, ok := seen[id]; ok {
			continue
		}
		seen[id] = struct{}{}
...
func buildEventFilters(input eventFilterInput) ([]sessionquery.EventMetadataFilter, error) {
	...
	if input.eventTypes != nil {
		...
		types := make([]sessionlog.EventType, 0, len(input.eventTypes))
		for _, kind := range input.eventTypes {
			types = append(types, sessionlog.EventType(kind))
		}
...
// 事件时间戳本身也是整毫秒（[sessionlog.Event.Time] 是 int64），所以筛出来的集合
