...
//
// 源: packages/session-query/tool-session-query/src/invariant.ts:12-13（name）
const PackageName = "@deepseek-ai/dsh-tool-session-query"

// DefaultMaxSearchResults 是一次检索最多交回多少条已授权的命中。
//
// 源: packages/session-query/tool-session-query/src/index.ts:21-22（DEFAULT_MAX_SEARCH_RESULTS）
const DefaultMaxSearchResults = 100

// DefaultSearchTimeout 是两件检索工具的默认协作式截止时间。
//
// 源: packages/session-query/tool-session-query/src/index.ts:24-25（DEFAULT_SEARCH_TIMEOUT_MS）
const DefaultSearchTimeout = 30 * time.Second

// Service 是本包用得到的那一小块读引擎。
//
// 新增: DSH 直接注入整个 SessionQueryEngine。这里只写出真正被调到的那七个方法，
// 装配方交进来的 [github.com/snight1983/ds-harness-go/feature/sessionquery.Engine] 自然满足它。窄口子的好处是
...
type Service interface {
	...
	TraceSession(ctx context.Context, id sessionlog.SessionID) (sessionquery.LineageTrace, error)
	...
	ReadTitleSnapshots(ctx context.Context, ids []sessionlog.SessionID) ([]sessionquery.ProjectionResult[sessionquery.TitleObservation], error)
}
...
type Config struct {
	...
	// 所以由装配方交一条查回去的路，做法和 [github.com/snight1983/ds-harness-go/feature/plan/planmode.Config.AgentOf]
	// 逐字相同。
	...
	// [github.com/snight1983/ds-harness-go/tools.Definition.Timeout]，由工具运行时统一执行——那件事
...
// New 造一个控制器。
//
// 源: packages/session-query/tool-session-query/src/index.ts:124-136
//
