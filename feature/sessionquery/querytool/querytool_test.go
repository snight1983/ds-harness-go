...
// 见 [sessionlog.SessionHeader.WorkspaceID]。本包只拿它们做相等判断。
var callerWorkspace = sessionlog.WorkspaceID("ws-caller")

// otherWorkspace 是边界另一侧那个工作区登记。
var otherWorkspace = sessionlog.WorkspaceID("ws-elsewhere")
...
type stubService struct {
	...
	sessions map[sessionlog.SessionID]sessionquery.Record
	titles   map[sessionlog.SessionID]string
	...
	titleErr map[sessionlog.SessionID]error
...
func (s *stubService) FilterSessions(
	...
	var workspaces []sessionlog.WorkspaceID
...
func (s *stubService) TraceSession(_ context.Context, id sessionlog.SessionID) (sessionquery.LineageTrace, error) {
	if s.lineage == nil {
...
func newWorld(t *testing.T, service *stubService) *world {
	...
	const id sessionlog.SessionID = "caller"
	sess, err := coresession.NewSession(id, coresession.Options{
		Header: &sessionlog.SessionHeader{ID: id, WorkspaceID: callerWorkspace},
	})
	if err != nil {
	...
	if service.sessions == nil {
		service.sessions = map[sessionlog.SessionID]sessionquery.Record{}
	}
...
func (w *world) appendStepStart(turn, step int) {
	w.t.Helper()
	w.appendEvent(sessionlog.EventTurnStart, map[string]int{"turn": turn})
	w.appendEvent(sessionlog.EventStepStart, map[string]int{"turn": turn, "step": step})
}

// appendEvent 往调用方日志里追加一条事件。
func (w *world) appendEvent(kind sessionlog.EventType, payload any) {
	w.t.Helper()
	data, err := json.Marshal(payload)
	if err != nil {
		w.t.Fatalf("排 %s 负载失败：%v", kind, err)
	}
	if _, err := w.agent.sess.Append(sessionlog.Event{Type: kind, Data: data}); err != nil {
		w.t.Fatalf("追加 %s 失败：%v", kind, err)
	}
}

// record 造一条会话记录。
func record(id sessionlog.SessionID, workspace sessionlog.WorkspaceID, parent sessionlog.SessionID) sessionquery.Record {
	return sessionquery.Record{
		Header: sessionlog.SessionHeader{ID: id, WorkspaceID: workspace, ParentSession: parent},
		Live:   true,
	}
}
...
func TestCrossSessionSearchKeepsTheWorkspaceBoundary(t *testing.T) {
	...
	service := &stubService{
		sessions: map[sessionlog.SessionID]sessionquery.Record{"inside": inside, "outside": outside},
		titles:   map[sessionlog.SessionID]string{"inside": "Inside work"},
		pages: []sessionquery.SearchPage[sessionquery.SearchHit]{{
...
func TestCrossSessionSearchNeedsACallerWorkspace(t *testing.T) {
	...
	sess, err := coresession.NewSession("caller", coresession.Options{
		Header: &sessionlog.SessionHeader{ID: "caller"},
	})
	if err != nil {
...
func TestCrossSessionSearchHidesOutOfWorkspaceParents(t *testing.T) {
	...
	service := &stubService{
		sessions: map[sessionlog.SessionID]sessionquery.Record{"child": child, "hidden": hidden},
		titles:   map[sessionlog.SessionID]string{"child": "Child"},
		pages: []sessionquery.SearchPage[sessionquery.SearchHit]{{
...
func TestCrossSessionSearchStopsAtTheResultCap(t *testing.T) {
	...
	service := &stubService{
		sessions: map[sessionlog.SessionID]sessionquery.Record{"one": first, "two": second},
		pages: []sessionquery.SearchPage[sessionquery.SearchHit]{{
...
func TestEventSearchExcludesTheCurrentStep(t *testing.T) {
	service := &stubService{titles: map[sessionlog.SessionID]string{"caller": "Caller"}}
	w := newWorld(t, service)
...
func TestSessionTraceMarksTheAncestorBoundary(t *testing.T) {
	...
	service := &stubService{
		...
		titles:  map[sessionlog.SessionID]string{"target": "Target", "parent": "Parent"},
		lineage: &trace,
	}
	w := newWorld(t, service)
...
func TestSessionTraceMarksAnIncompleteLineage(t *testing.T) {
	...
	service := &stubService{
		sessions: map[sessionlog.SessionID]sessionquery.Record{"target": target},
		titles:   map[sessionlog.SessionID]string{"target": "Target"},
		lineage:  &trace,
	}
	w := newWorld(t, service)
	text, err := w.call("session_trace", map[string]any{"session_id": "target"})
...
func TestSessionTracePrunesOutOfWorkspaceSubtrees(t *testing.T) {
	...
	service := &stubService{
		...
		titles:  map[sessionlog.SessionID]string{"target": "Target", "child": "Child"},
		lineage: &trace,
	}
	w := newWorld(t, service)
	text, err := w.call("session_trace", map[string]any{"session_id": "target"})
...
func TestEventReadEmitsTheTargetVerbatim(t *testing.T) {
	...
	window := sessionquery.EventWindow{
		...
		Target: sessionlog.Event{
			Seq: 7, Type: sessionlog.EventStepStart, Time: 1700000000000,
			Data: json.RawMessage(`{"turn":2,"step":1}`),
		},
		StartSeq: 7, EndSeq: 7,
	}
	service := &stubService{
		titles: map[sessionlog.SessionID]string{"caller": "Caller"},
		window: &window,
	}
	w := newWorld(t, service)
