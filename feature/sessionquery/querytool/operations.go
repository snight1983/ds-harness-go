...
func (c *Controller) executeSessionSearch(
	...
	if requestedParentsGiven || args.IncludeRootSessions {
		...
		var parentValues []sessionlog.SessionID
	...
	sessionFilters = append(sessionFilters, sessionquery.WorkspaceFilter{Values: []sessionlog.WorkspaceID{from.header.WorkspaceID}})
	...
	var parentIDs []sessionlog.SessionID
	ids := make([]sessionlog.SessionID, 0, len(collected.items))
...
func (c *Controller) executeSessionTrace(
	...
	visibleIDs := make([]sessionlog.SessionID, 0, 1+len(ancestors))
...
func lastStepStart(events []sessionlog.Event) (sessionlog.Event, bool) {
	for index := len(events) - 1; index >= 0; index-- {
		if events[index].Type == sessionlog.EventStepStart {
