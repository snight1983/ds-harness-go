...
func newEngine(t *testing.T, events []sessionlog.Event) *Engine {
	t.Helper()
...
func TestReadSessionRefusesALogThatDoesNotReplay(t *testing.T) {
	...
	broken := []sessionlog.Event{
		userEvent(t, 0, "开个头"),
		plainEvent(t, sessionlog.EventStepEnd, 1, sessionlog.StepEndData{Turn: 1, Step: 1}),
	}
...
func TestReadSessionRefusesALogWhoseSurfaceCannotFold(t *testing.T) {
	...
	_, err := newEngine(t, []sessionlog.Event{replacingUserEvent(t, 0, "盖谁", 7, 7, 7)}).
