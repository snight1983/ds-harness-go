...
type LiveSessions interface {
	...
	Get(id sessionlog.SessionID) (LogicalSource, bool)
	// List 列举此刻所有活会话的当前观察。
	List() []LogicalSource
}
...
func (c *Corpus) ListSessions(ctx context.Context) ([]Record, error) {
	...
	var persisted []sessionlog.SessionHeader
	...
	records := make(map[sessionlog.SessionID]Record, len(persisted))
...
func (c *Corpus) Load(ctx context.Context, id sessionlog.SessionID) (LogicalSession, error) {
	if err := checkAborted(ctx.Err()); err != nil {
...
func ProjectMany[V any](
	...
	byID := make(map[sessionlog.SessionID]sessionlog.SessionHeader, len(headers))
...
func projectPending[V any](
	ctx context.Context,
	c *Corpus,
	unique []sessionlog.SessionID,
	pending []int,
	byID map[sessionlog.SessionID]sessionlog.SessionHeader,
...
func (c *Corpus) listPersisted(ctx context.Context) ([]sessionlog.SessionHeader, error) {
...
func (c *Corpus) inspectPersisted(ctx context.Context, id sessionlog.SessionID) (persistence.Inspection, error) {
...
// 得走 [sessionlog.Event.Clone]。
func detach(source LogicalSource) LogicalSession {
	events := make([]sessionlog.Event, 0, len(source.Events))
...
func headerWithID(headers []sessionlog.SessionHeader, id sessionlog.SessionID) (sessionlog.SessionHeader, bool) {
	for _, header := range headers {
		if header.ID == id {
			return header, true
		}
...
func uniqueIDs(ids []sessionlog.SessionID) []sessionlog.SessionID {
	seen := make(map[sessionlog.SessionID]bool, len(ids))
	unique := make([]sessionlog.SessionID, 0, len(ids))
	for _, id := range ids {
		if seen[id] {
			continue
		}
