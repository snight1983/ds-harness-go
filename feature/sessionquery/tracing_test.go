...
func lineageRecord(id, parent sessionlog.SessionID, createdAt int64) Record {
...
func chainedReplacementLog(t *testing.T) []sessionlog.Event {
	t.Helper()
...
func TestEventRecordsRefusesALogWhoseSurfaceCannotFold(t *testing.T) {
	...
	_, err := EventRecords("s1", []sessionlog.Event{replacingUserEvent(t, 0, "盖谁", 7, 7, 7)})
...
func TestCurrentSurfaceEventsRefusesALogWhoseSurfaceCannotFold(t *testing.T) {
	...
	_, err := CurrentSurfaceEvents("s1", []sessionlog.Event{replacingUserEvent(t, 0, "盖谁", 7, 7, 7)})
	requireCode(t, err, CodeInvalidSurface)
}
...
func TestTraceEventRefusesASeqThatIsNotThere(t *testing.T) {
	...
	broken := []sessionlog.Event{userEvent(t, 0, "只有一条")}
	broken[0].Seq = 5
...
func TestTraceEventRefusesALogWhoseSurfaceCannotFold(t *testing.T) {
	...
	_, err := TraceEvent("s1", []sessionlog.Event{replacingUserEvent(t, 0, "盖谁", 7, 7, 7)}, 0)
	requireCode(t, err, CodeInvalidSurface)
}
...
func TestTraceSessionWalksBothDirections(t *testing.T) {
	...
	wantChildren := []sessionlog.SessionID{"兄弟甲", "leaf", "兄弟乙"}
