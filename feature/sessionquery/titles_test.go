...
func titleEvent(t *testing.T, seq int, title string) sessionlog.Event {
	t.Helper()
...
func titleEngine(t *testing.T, logs map[sessionlog.SessionID][]sessionlog.Event) *Engine {
	t.Helper()
...
func TestReadingATitleFoldsTheLatestOneOffTheLog(t *testing.T) {
	...
	engine := titleEngine(t, map[sessionlog.SessionID][]sessionlog.Event{
...
func TestABatchOfTitlesKeepsFirstOccurrenceOrderAndIsolatesFailures(t *testing.T) {
	...
	engine := titleEngine(t, map[sessionlog.SessionID][]sessionlog.Event{
		...
		"s2": {sessionlog.Event{Type: sessiontitle.EventSessionTitle, Seq: 0, Data: []byte(`not json`)}},
		"s3": {titleEvent(t, 0, "三号")},
	})

	results, err := engine.ReadTitleSnapshots(t.Context(), []sessionlog.SessionID{"s1", "s2", "s3", "s1"})
	if err != nil {
...
func TestReadingOneTitleSurfacesTheIsolatedFailure(t *testing.T) {
	...
	engine := titleEngine(t, map[sessionlog.SessionID][]sessionlog.Event{
		"s1": {sessionlog.Event{Type: sessiontitle.EventSessionTitle, Seq: 0, Data: []byte(`{"title":42}`)}},
	})
...
func TestCancellingATitleReadFailsTheWholeCall(t *testing.T) {
	...
	_, err := engine.ReadTitleSnapshots(canceledContext(), []sessionlog.SessionID{"s1"})
