...
func TestOpenBootstrap把已落地会话并进它们点名的工作区(t *testing.T) {
	...
	// 组内按创建时刻从新到旧。
	if got := mustSessionIDs(t, ctx, mustGet(t, ctx, registry, alpha)); !slices.Equal(got, []sessionlog.SessionID{"s2", "s1"}) {
		t.Fatalf("组内该按创建时刻从新到旧，拿到 %v", got)
	}
	if got := mustSessionIDs(t, ctx, mustGet(t, ctx, registry, beta)); !slices.Equal(got, []sessionlog.SessionID{"s3"}) {
		t.Fatalf("拿到 %v", got)
	}
}
...
func TestOpenBootstrap同刻的会话按会话id定序(t *testing.T) {
	...
	if got := mustSessionIDs(t, ctx, mustGet(t, ctx, registry, ws)); !slices.Equal(got, []sessionlog.SessionID{"s-a", "s-b"}) {
...
func TestOpenBootstrap跳过归不进任何工作区的会话头(t *testing.T) {
	...
	if got := mustSessionIDs(t, ctx, mustGet(t, ctx, registry, ws)); !slices.Equal(got, []sessionlog.SessionID{"s5"}) {
...
func TestOpenBootstrap被打断之后重跑会并进已有的工作区(t *testing.T) {
	...
	if got := mustSessionIDs(t, ctx, mustGet(t, ctx, registry, ws)); !slices.Equal(got, []sessionlog.SessionID{"s3", "s2", "s1"}) {
...
func TestOpenBootstrap候选已经记在别的工作区名下时不重复收编(t *testing.T) {
	...
	record.SessionIDs = []sessionlog.SessionID{"s1"}
	...
	// 也不该去动认领它的那一家。
	if got := mustRecord(t, ctx, registry, elsewhere).SessionIDs; !slices.Equal(got, []sessionlog.SessionID{"s1"}) {
...
func TestArchiveSession收下活着的和已落地的会话(t *testing.T) {
	...
	want := []sessionlog.SessionID{"已落地", "活着的"}
...
func TestArchiveSession不动工作区账目(t *testing.T) {
	...
	if got := mustSessionIDs(t, ctx, mustGet(t, ctx, registry, ws)); !slices.Equal(got, []sessionlog.SessionID{"s2", "s1"}) {
...
func TestOpen候选没通过归属判据时留下诊断且不抹数据(t *testing.T) {
	...
	for _, one := range cases {
		t.Run(one.name, func(t *testing.T) {
			...
			if got := mustSessionIDs(t, ctx, mustGet(t, ctx, registry, here)); !slices.Equal(got, []sessionlog.SessionID{"s1"}) {
			...
			if got := mustRecord(t, ctx, registry, here).SessionIDs; !slices.Equal(got, []sessionlog.SessionID{"s1"}) {
