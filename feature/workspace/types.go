...
// （[sessionlog.SessionHeader.WorkspaceID] 就是归属判据的那一侧），而 workspace
// 认识 session、session 不认识 workspace。别名而不是另开一个具名类型，
// 是因为两处说的必须是同一个东西——两个类型之间来回转换的地方，
// 迟早会有一处转反了而编译器不吭声。
type WorkspaceID = sessionlog.WorkspaceID
...
type Workspace interface {
	...
	SessionIDs(ctx context.Context) ([]sessionlog.SessionID, error)
	...
	// 一律拒绝且不写。
	AttachSession(ctx context.Context, sessionID sessionlog.SessionID) error
	...
	// 和 [sessionlog.SessionHeader.ParentSession] 用空串表示「不是分叉来的」是同一条约定。
	InsertSessionBefore(ctx context.Context, sessionID, beforeSessionID sessionlog.SessionID) error
...
type LiveSessions interface {
	...
	Header(id sessionlog.SessionID) (sessionlog.SessionHeader, bool)
