...
type Registry struct {
	...
	invalidSessions map[sessionlog.SessionID]string
...
// [sessionlog.SessionHeader.WorkspaceID]，而会话头本身已经在 headers 里了，
...
func (r *Registry) ArchivedSessionIDs(ctx context.Context) ([]sessionlog.SessionID, error) {
	state, err := r.readState(ctx)
	if err != nil {
		return nil, err
	}
...
func (r *Registry) ArchiveSession(ctx context.Context, sessionID sessionlog.SessionID) error {
...
// [sessionlog.SessionHeader.WorkspaceID]，一个不透明标识，填不出任何记录字段，
...
func (r *Registry) bootstrap(ctx context.Context, headers []sessionlog.SessionHeader) error {
	table, err := r.table()
	if err != nil {
		return err
	}
	...
	accounted := map[sessionlog.SessionID]WorkspaceID{}
	...
	for _, group := range groups {
		...
		historical := make([]sessionlog.SessionID, 0, len(group.headers))
		for _, header := range group.headers {
...
func (r *Registry) groupHeaders(headers []sessionlog.SessionHeader) []bootstrapGroup {
	...
	for _, header := range headers {
		...
		if !exists {
			grouped[header.WorkspaceID] = &bootstrapGroup{
				...
				headers:     []sessionlog.SessionHeader{header},
			}
			continue
		}
...
func (r *Registry) replaceHeaderIndex(ctx context.Context, headers []sessionlog.SessionHeader) {
	r.mutex.Lock()
	r.headers = make(map[sessionlog.SessionID]sessionlog.SessionHeader, len(headers))
	r.invalidSessions = map[sessionlog.SessionID]string{}
	r.mutex.Unlock()
...
func (r *Registry) indexHeaders(ctx context.Context, headers []sessionlog.SessionHeader) {
	for _, header := range headers {
...
// 那个标识自己（见 [sessionlog.SessionHeader.WorkspaceID]），没有解析这一步，
// 那三条理由随之消失——剩下的唯一一种「用不了」就是它压根没点名工作区。
//
// ctx 留着不用，是因为 [Registry.indexHeaders] 的两个调用点都在 I/O 路径上，
// 摘掉它会让上游一路跟着改签名，而这里将来仍可能要发请求。
func (r *Registry) indexHeader(_ context.Context, header sessionlog.SessionHeader) {
	r.mutex.Lock()
...
func (r *Registry) markInvalidSession(id sessionlog.SessionID, reason string) {
	r.mutex.Lock()
...
func (r *Registry) sessionKnown(ctx context.Context, id sessionlog.SessionID) (bool, error) {
	if r.live != nil {
...
func (r *Registry) sessionWorkspaceID(id sessionlog.SessionID) (sessionlog.WorkspaceID, bool) {
	r.mutex.RLock()
	defer r.mutex.RUnlock()
...
func (r *Registry) readSessionHeader(ctx context.Context, id sessionlog.SessionID) (sessionlog.SessionHeader, error) {
	if r.live != nil {
	...
	headers, err := r.persistence.List(ctx)
	if err != nil {
		return sessionlog.SessionHeader{}, wrap(CodeStorageFailed, err,
	...
	if !found {
		return sessionlog.SessionHeader{}, fail(CodeUnknownSession,
