...
import (
	...
	"github.com/snight1983/ds-harness-go/feature/spillpolicy"
...
const ownerSession = sessionlog.SessionID("s1")
...
func newHarness(t *testing.T, maxInlineBytes int, owned bool) *harness {
	...
	logs := &strings.Builder{}
	store := &stubStore{}
	installed, err := spillpolicy.New(spillpolicy.Config{
...
func TestUndoRemovesTheRule(t *testing.T) {
	...
	installed, err := spillpolicy.New(spillpolicy.Config{
		MaxInlineBytes: 200,
		Store:          &stubStore{},
		OwnerOf:        func(*scope.Key) (sessionlog.SessionID, bool) { return ownerSession, true },
...
func TestConfigValidationFailsLoud(t *testing.T) {
	t.Parallel()
	ok := func(*scope.Key) (sessionlog.SessionID, bool) { return ownerSession, true }
	cases := map[string]spillpolicy.Config{
	...
	for label, config := range cases {
		t.Run(label, func(t *testing.T) {
			t.Parallel()
			if _, err := spillpolicy.New(config); !errors.Is(err, spillpolicy.ErrInvalidConfig) {
