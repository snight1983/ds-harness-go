...
// # 不做什么
//
//   - **不表达上游本身的残缺。**Truncated 专指「**因为预算**丢掉了本来拿得到的内容」；
//     权限失败、跳过的二进制文件、上游的部分失败一律留在工具自己的字段里。
//   - **不拥有恢复的措辞。**footer 的后半截由调用方给（见 [FormatRetentionNotice]），
//     因为只有工具知道该怎么恢复。
//   - **不做分组、排序、行号、退出码、每行预览和溢出文件。**那些是工具输出的形状，
//     这里只回答留了多少、丢了多少。
//   - **不落盘、不注册、不发事件。**它是一个纯库，没有服务那一面，也就没有任何存储
//     或者作用域上的牵连。
//   - **不跨调用累积。**一个留存器的状态只属于它自己那一次使用，用完就完。
