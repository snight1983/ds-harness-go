...
// # 面向模型的那件工具不在这儿
//
// ask_user_question 那件工具在 [github.com/snight1983/ds-harness-go/feature/interaction/askuser]。本包只有接缝，
...
// 作用域键（[github.com/snight1983/ds-harness-go/scope.Key]），活 agent 注册表是循环那一块的东西，
...
// # 不做什么
//
//   - **不问任何人。**一个界面都不画：终端、桌面端、网页端各自实现一个 [Provider]，
//     一个上下文里只准有一个活着的。本包只是它前面那道门。
//   - **不提供面向模型的那件工具。**ask_user_question 在
//     [github.com/snight1983/ds-harness-go/feature/interaction/askuser]；
//     一个无界面的装配根本不装它，模型也就问不出问题来。
//   - **不自己认活 agent。**「这是不是那个活着的实例」「它是不是被另一个 agent 拥有」
//     靠 [Config.CallerStatus] 这条接缝，不接就等于没有活注册表——活 agent 注册表
//     是循环那一块的东西。
//   - **不让包外加意图变体。**[Intent] 的封印方法未导出；一个不认识的变体在界面侧
//     走通用问答那条路，而不是被拒掉。
//   - **不区分「没给」和「给了空串」。**一份空的计划正文和没有计划正文一样看不见，
//     所以这两件事在本包是同一回事。
