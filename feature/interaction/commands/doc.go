...
// # 不做什么
//
//   - **不是 shell 解释器。**[Parse] 只认 `/名字` 后面紧跟行尾或者一个空白的那种行，
//     尾巴上的输入原样交给处理器，不分词、不展开、不做任何归一化。
//   - **不定义任何命令。**表是插件自己填的，本包只管登记、按 agent 作用域遮蔽、
//     准入、图片限额和那一对生命周期事件。
//   - **不进模型。**斜杠命令是人对着这套装置本身说的话，既不占模型的上下文，
//     也不由模型决定跑不跑。
//   - **不画界面，也不摆上线。**列表和执行都是普通的 Go 方法，把它们变成能远程调的
//     东西归 [github.com/snight1983/ds-harness-go/protocol/sdk/sdkserver] 那一层。
//   - **停不下不合作的处理器。**取消赢了那一局，这次执行就结算成 cancelled、
//     落一条 command/done，可那个 goroutine 还在跑——Go 没有掐死别人协程的办法。
//   - **不自己找活会话和附件仓库。**它们分别是 [Options.LogOf] 和 [Options.Attachments]
//     这两条显式的接缝，由装配方填。
package commands
