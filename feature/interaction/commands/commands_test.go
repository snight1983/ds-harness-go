...
func eventAt(t *testing.T, seq int, kind sessionlog.EventType, data any) sessionlog.Event {
	t.Helper()
	raw, err := json.Marshal(data)
	if err != nil {
		t.Fatalf("造事件失败：%v", err)
	}
	return sessionlog.Event{Type: kind, Seq: seq, Data: raw}
}

// runAt 造一条 command/run。
func runAt(t *testing.T, seq int, id commands.ID) sessionlog.Event {
	t.Helper()
...
func doneAt(t *testing.T, seq int, id commands.ID, kind commands.ResultKind, source *int) sessionlog.Event {
	t.Helper()
...
func plainAt(t *testing.T, seq int) sessionlog.Event {
	t.Helper()
	return eventAt(t, seq, sessionlog.EventTurnStart, map[string]int{"turn": 1})
}

func TestTheTraceAcceptsAPairSettledAgainstAnEarlierDomainEvent(t *testing.T) {
	t.Parallel()
	source := 0
	trace, err := commands.ValidateLog([]sessionlog.Event{
		plainAt(t, 0),
		runAt(t, 1, "cmd-valid"),
...
func TestTheTraceIgnoresEventsThatAreNotItsOwn(t *testing.T) {
	t.Parallel()
	trace, err := commands.ValidateLog([]sessionlog.Event{plainAt(t, 0), plainAt(t, 1)})
	if err != nil {
...
func TestTheTraceCatchesAnUnpairedOrRepeatedLifecycle(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"结算配不上任何一次执行": {
			events:   []sessionlog.Event{doneAt(t, 0, "cmd-orphan", commands.ResultSuccess, nil)},
...
func TestTheTraceCatchesAnInvalidAuthoritativeReference(t *testing.T) {
	...
	for label, item := range cases {
		t.Run(label, func(t *testing.T) {
			...
			_, err := commands.ValidateLog([]sessionlog.Event{
...
func TestTheTraceCatchesAnUnreadablePayload(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"command/run 的负载读不回来": {
			event:    sessionlog.Event{Type: commands.EventRun, Data: json.RawMessage(`"not an object"`)},
			fragment: "command/run carries an unreadable payload",
		},
		"command/done 的负载读不回来": {
			event:    sessionlog.Event{Type: commands.EventDone, Data: json.RawMessage(`42`)},
	...
	for label, item := range cases {
		t.Run(label, func(t *testing.T) {
			t.Parallel()
			_, err := commands.ValidateLog([]sessionlog.Event{item.event})
			if err == nil || !strings.Contains(err.Error(), item.fragment) {
				t.Fatalf("该报这一条：%v", err)
			}
		})
