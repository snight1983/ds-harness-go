...
const (
	...
	EventRun sessionlog.EventType = "command/run"
	// EventDone 记下配对的那条命令结算了。
	EventDone sessionlog.EventType = "command/done"
)
...
//
//	vocabulary := sessionlog.CoreVocabulary().With(commands.EventTypes()...)
func EventTypes() []sessionlog.EventType {
	return []sessionlog.EventType{EventRun, EventDone}
}
