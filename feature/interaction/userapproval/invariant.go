...
//
// 验和改分成两步，和 [sessionlog.Trace] 的做法一致：验的那一步是纯的，可以在事件真的
...
// **第一条**违例，和 [sessionlog.Trace.Validate] 一致：它因此可以脱离不变量注册表单独用，
...
func (t *Trace) Validate(event sessionlog.Event) (Transition, error) {
	switch event.Type {
	case sessionlog.EventTurnStart, sessionlog.EventTurnEnd:
...
func (t *Trace) Apply(event sessionlog.Event, transition Transition) {
	switch event.Type {
...
// [github.com/snight1983/ds-harness-go/feature/todo.RegisterInvariants] 逐字相同。
//
