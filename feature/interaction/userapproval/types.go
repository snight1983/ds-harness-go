...
const (
	...
	// [github.com/snight1983/ds-harness-go/tools.ApprovalUnavailable]。
	PolicyAsk Policy = "ask"
	// PolicyNever 是「谁都不问」：每一次询问都确定地结算成
	// [github.com/snight1983/ds-harness-go/tools.ApprovalRejected]。
	//
...
//
// 词汇表本身在 [github.com/snight1983/ds-harness-go/tools]（见包文档），这里只是把那四个值列成
// 一份能遍历的单子。
...
const (
	...
	EventAsked sessionlog.EventType = "approval/asked"
	// EventDecided 记下前面那条 [EventAsked]（同一个 id）的结局。
	//
	// 一问一答，一条不多一条不少：一次决定、一次取消、或者那个失败关闭的
	// unavailable，都在这里落地。
	EventDecided sessionlog.EventType = "approval/decided"
	// EventPolicy 记下这条会话的审批策略被切换了。
	//
	// **最后一条**就是这条会话的覆盖值（[EffectivePolicy]）。
	EventPolicy sessionlog.EventType = "approval/policy"
)

// EventTypes 是本包往会话日志里写的那几种事件类型。
//
// 新增: 理由和 [github.com/snight1983/ds-harness-go/feature/compaction.EventTypes] 逐字相同——Go 没有声明合并，
// [sessionlog.Vocabulary] 是个闭合的值，所以由本包交出单子、装配方自己拼：
//
//	vocabulary := sessionlog.CoreVocabulary().With(userapproval.EventTypes()...)
//
// 不这么做的话，一段带审批审计的日志会被 [sessionlog.CheckVocabulary] 整个拒掉。
func EventTypes() []sessionlog.EventType {
	return []sessionlog.EventType{EventAsked, EventDecided, EventPolicy}
}
...
func EffectivePolicy(events []sessionlog.Event) (Policy, bool) {
