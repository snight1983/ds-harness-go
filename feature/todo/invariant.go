...
// ValidateEvent 验一条事件里本包拥有的那些字段；不是 [sessionlog.EventTodoWrite]
// 就什么都不做。
//
...
// 违例，和 [sessionlog.Trace.Validate] 的做法一致：它因此可以脱离不变量注册表单独用
// （离线校验一份日志、或者在写之前自己先验一遍），而 [RegisterInvariants] 只是
// 把这个错误接到 [invariants.Fail] 上。
func ValidateEvent(event sessionlog.Event) error {
	if event.Type != sessionlog.EventTodoWrite {
		return nil
	}
	var payload struct {
...
//
// 它按**原始 JSON**验而不是先解成 [sessionlog.TodoItem]，因为要分开报的正是那几种
...
// 做法和 [github.com/snight1983/ds-harness-go/feature/workspace.RegisterInvariants] 收 facility 与 live 一致。
//
