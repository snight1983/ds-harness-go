...
import (
	...
	"github.com/snight1983/ds-harness-go/feature/todo"
...
func TestWritesTheWholeListAndReportsTheCounts(t *testing.T) {
	...
	want := []sessionlog.TodoItem{
		{Content: "读代码", Status: sessionlog.TodoCompleted},
		{Content: "写代码", Status: sessionlog.TodoInProgress},
		{Content: "跑测试", Status: sessionlog.TodoPending},
	}
...
func TestUndoRemovesTheTool(t *testing.T) {
	...
	tool, err := todo.New(todo.Config{Append: func(*scope.Key, []sessionlog.TodoItem) error { return nil }})
	if err != nil {
		t.Fatalf("造这件工具失败：%v", err)
	}
...
func todoEvent(seq int, payload string) sessionlog.Event {
	return sessionlog.Event{Type: sessionlog.EventTodoWrite, Seq: seq, Data: json.RawMessage(payload)}
}

// turnEvent 造一条回合边界事件。
func turnEvent(seq int, kind sessionlog.EventType) sessionlog.Event {
	return sessionlog.Event{Type: kind, Seq: seq, Data: json.RawMessage(`{}`)}
}

// projectionOf 把这些事件折一遍，交出 todos 这个键的值。
func projectionOf(t *testing.T, events ...sessionlog.Event) (any, bool) {
	t.Helper()
...
func itemsOf(t *testing.T, value any) []sessionlog.TodoItem {
	t.Helper()
	items, ok := value.([]sessionlog.TodoItem)
	if !ok {
...
func TestTheProjectionKeepsTheLastWrite(t *testing.T) {
	...
	if len(items) != 1 || items[0].Content != "c" || items[0].Status != sessionlog.TodoCompleted {
...
func TestANewTurnClearsThePlanButTheEndOfATurnDoesNot(t *testing.T) {
	...
	cleared, _ := projectionOf(t,
		...
		turnEvent(1, sessionlog.EventTurnStart),
	)
	...
	kept, _ := projectionOf(t,
		todoEvent(0, `{"todos":[{"content":"a","status":"completed"}]}`),
		turnEvent(1, sessionlog.EventTurnEnd),
	)
...
func TestTheProjectionIgnoresUnrelatedEvents(t *testing.T) {
	...
	value, _ := projectionOf(t,
		...
		sessionlog.Event{Type: sessionlog.EventUserMessage, Seq: 1, Data: json.RawMessage(`{}`)},
	)
...
func TestTheProjectionReportsNoChangeForARedundantClear(t *testing.T) {
	...
	registry.Drive(view, turnEvent(0, sessionlog.EventTurnStart))
	if changes != 0 {
		t.Fatalf("空清单上的清空不算变化，拿到 %d 次通知", changes)
	}
	view.events = append(view.events, turnEvent(0, sessionlog.EventTurnStart))
...
func TestARestoredStateThatDoesNotFitIsRejected(t *testing.T) {
	...
	events := []sessionlog.Event{todoEvent(0, `{"todos":[{"content":"a","status":"pending"}]}`)}
...
func TestValidateEventIgnoresEverythingElse(t *testing.T) {
	...
	if err := todo.ValidateEvent(sessionlog.Event{Type: sessionlog.EventUserMessage, Data: json.RawMessage(`{"todos":42}`)}); err != nil {
