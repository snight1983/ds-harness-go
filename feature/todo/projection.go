...
func RegisterProjection(registry *projection.Registry) (func(), error) {
	...
	return projection.Register(registry, projection.Definition[[]sessionlog.TodoItem]{
		Key:          ProjectionKey,
		StateVersion: projectionStateVersion,
		Init:         func() []sessionlog.TodoItem { return nil },
		Apply:        applyProjection,
		DecodeState:  decodeProjectionState,
		View:         func(state []sessionlog.TodoItem) any { return state },
	})
}
...
func applyProjection(state []sessionlog.TodoItem, event sessionlog.Event) ([]sessionlog.TodoItem, bool) {
	switch event.Type {
	case sessionlog.EventTodoWrite:
		var data sessionlog.TodoWriteData
...
var decodeProjectionState = projection.StrictDecoder[[]sessionlog.TodoItem]()
