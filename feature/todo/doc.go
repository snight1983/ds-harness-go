...
// # 为什么是整表替换，不是逐条编辑
//
// 每一次 todo_write 都带着**完整的**清单，追加成一条 [github.com/snight1983/ds-harness-go/sessionlog.EventTodoWrite]
...
//     [github.com/snight1983/ds-harness-go/sessionlog/projection.Registry]。可选：装配里没有投影注册表时
//     就不装，界面读到的就是「这个能力不在」。
...
// agent 对象上摸到一个活会话。Go 这边 [github.com/snight1983/ds-harness-go/tools.Execution.Agent]
...
// # 不做什么
//
//   - **不调度，也不跨会话同步。**没有依赖图、没有截止时间、没有提醒——一份清单
//     只是模型和界面读得到的一份事实。
//   - **不评判条目写得对不对。**文本是模型写的，[ValidateEvent] 只验形状；
//     方案好不好没有判据，本包也不假装有。
//   - **不把当下的策略写成持久规则。**允不允许并行由部署选，那条不变量对
//     「有几条在做」保持沉默——今天收紧策略不该让昨天的日志变得不合法。
//   - **不替装配方决定装哪几条胳膊。**[RegisterProjection] 和 [RegisterInvariants]
//     都是可选的，本包既不代劳，也不因为它们缺席而报错。
//   - **不知道该往哪个会话追加。**从那把不透明的作用域键到一条活会话的映射只有
//     装配方有，本包只认 [Config.Append] 这条接缝。
package todo
