...
func (c *Coordinator) attachPrepared(
	...
	if mismatch {
		state.finish(fmt.Errorf(
			"feature/persistence: 会话 %q 的准备成果和它的持久化状态已经对不上了", string(live.ID())))
		return
	}
...
//     并把它的 seed 写一次。
func (c *Coordinator) onCreated(live *coresession.Session, seed []sessionlog.Event) error {
	id := live.ID()
...
// 接着按情形 2／3／4 走。
func (c *Coordinator) reconcileTracked(live *coresession.Session, seed []sessionlog.Event) (bool, error) {
	id := live.ID()
	...
	if owner == nil {
		...
		if trackedWorkspace != live.Header().WorkspaceID {
			return true, fmt.Errorf(
				"feature/persistence: 会话 %q 已经归在另一个工作区下（存档：%q，活的：%q），撞号了",
		...
		if !matches {
			return true, fmt.Errorf(
				"feature/persistence: 会话 %q 已经落盘到 seq %d，和这个活会话对不上，撞号了",
				string(id), nextSeq)
		}
		c.mutex.Lock()
		tracked.owner = live
	...
	return true, fmt.Errorf(
		"feature/persistence: 会话 %q 在这个后端里已经绑在另一个活会话上了，撞号了", string(id))
}
...
func (c *Coordinator) adoptLivePrefix(
	...
	if !covers {
		return fmt.Errorf(
			"feature/persistence: 会话 %q 在磁盘上那份日志和这个活会话对不上，撞号了", string(id))
	}
...
func (c *Coordinator) newWriteBehind(live *coresession.Session, state *liveState) *WriteBehind {
	...
	return NewWriteBehind(WriteBehindOptions{
		...
		Write: func(batch []sessionlog.Event) error {
...
func (c *Coordinator) appendLiveBatch(id sessionlog.SessionID, batch []sessionlog.Event) error {
	var nextSeq int
	var started bool
...
func splitAt(id sessionlog.SessionID, events []sessionlog.Event, at int) (before, from []sessionlog.Event, err error) {
	if len(events) == 0 {
		return nil, nil, nil
	}
	...
	if events[index].Seq != at {
		return nil, nil, fmt.Errorf(
			"feature/persistence: 会话 %q 的事件不连续：下标 %d 上应该是 seq %d，实际是 %d",
...
func nextSeqOf(events []sessionlog.Event, baseSeq int) int {
	if len(events) == 0 {
		return baseSeq
	}
...
func cloneEvents(events []sessionlog.Event) []sessionlog.Event {
