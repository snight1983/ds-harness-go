...
type Store interface {
	...
	// 没有逐会话的存档。
	Locate(meta sessionlog.SessionHeader) (Location, bool)
	...
	ReadRaw(ctx context.Context, id sessionlog.SessionID) (RawArtifact, error)
	...
	// 什么都不留下。
	Create(ctx context.Context, meta sessionlog.SessionHeader) error
	...
	Append(ctx context.Context, id sessionlog.SessionID, events []sessionlog.Event) error
	...
	Load(ctx context.Context, id sessionlog.SessionID) (Inspection, error)
	...
	//
	// 调用方只借到不可变的头和日志。
	Inspect(ctx context.Context, id sessionlog.SessionID) (Inspection, error)
	...
	ReadFrom(ctx context.Context, id sessionlog.SessionID, fromSeq int) (StoredSuffix, error)
