...
// 一个真的 [github.com/snight1983/ds-harness-go/harness/session.Store] 结构上就满足它。
// 成例是 compaction/basic.Agents。
type Sessions interface {
	// Get 按身份找一个活会话。
	Get(id sessionlog.SessionID) (*coresession.Session, bool)

	// List 列出此刻所有活会话，装载时补种要用。
	List() []*coresession.Session

	// PrepareRestored 从一份存档前缀造一个还没发布的会话。
	PrepareRestored(id sessionlog.SessionID, options coresession.RestoreOptions) (*coresession.Session, error)
...
type CoordinatorDeps struct {
	...
	// 核心词汇。Go 里 [github.com/snight1983/ds-harness-go/sessionlog.Vocabulary] 是值类型、零值是
...
// [github.com/snight1983/ds-harness-go/harness/agentloop.SessionPersistence]
// 而在这里转了一手，理由写在那个方法上。
//
...
type Coordinator struct {
	...
	idle        *sync.Cond
	states      map[sessionlog.SessionID]*sessionState
	live        map[*coresession.Session]*liveState
	retirements map[sessionlog.SessionID]*retirement
	chains      map[sessionlog.SessionID]*chainEntry
}
...
func NewCoordinator(deps CoordinatorDeps, options CoordinatorOptions) (*Coordinator, error) {
	...
	case deps.Backend == nil:
		return nil, errors.New("feature/persistence: 编排器要一个后端")
	case deps.Sessions == nil:
		return nil, errors.New("feature/persistence: 编排器要一张活会话表")
	case options.PreparedSessionCacheSize < 0:
		return nil, fmt.Errorf(
			"feature/persistence: 准备池容量不能是负数（给的是 %d）",
			options.PreparedSessionCacheSize)
	case options.WriteBatchMaxDelay < 0:
		return nil, fmt.Errorf(
			"feature/persistence: 攒批时长不能是负数（给的是 %s）",
			options.WriteBatchMaxDelay)
	case options.MaxStoredEvents < 0:
		return nil, fmt.Errorf(
			"feature/persistence: 存档条数上限不能是负数（给的是 %d）",
...
func (c *Coordinator) Install(ctx context.Context, owner *scope.Scope) (func(context.Context) error, error) {
	if owner == nil {
		return nil, errors.New("feature/persistence: 需要一个持有这次登记的作用域")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("feature/persistence: 登记排干失败：%w", err)
	}
	...
	for _, entry := range registrations {
		...
		if err != nil {
			...
			return nil, fmt.Errorf("feature/persistence: 装%s观察者失败：%w", entry.label, err)
		}
...
func (c *Coordinator) onSessionEvent(live *coresession.Session, event sessionlog.Event) {
