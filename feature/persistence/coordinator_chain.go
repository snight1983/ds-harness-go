...
// 依据，一个还在排队的等待方也算在途活儿。
func (c *Coordinator) acquire(ctx context.Context, id sessionlog.SessionID) (*chainEntry, error) {
	c.mutex.Lock()
...
func (c *Coordinator) releaseChain(id sessionlog.SessionID, entry *chainEntry) {
	<-entry.lock
	c.releaseHolder(id, entry)
}
...
// 判据：chains 空了才叫静止。
func (c *Coordinator) releaseHolder(id sessionlog.SessionID, entry *chainEntry) {
	c.mutex.Lock()
	entry.holders--
...
func (c *Coordinator) serialize(ctx context.Context, id sessionlog.SessionID, op func() error) error {
...
func (c *Coordinator) waitForRetirement(ctx context.Context, id sessionlog.SessionID) error {
	c.mutex.Lock()
...
func (c *Coordinator) stateOf(id sessionlog.SessionID) *sessionState {
	c.mutex.Lock()
...
// putState 把一份状态记进册子。
func (c *Coordinator) putState(id sessionlog.SessionID, state *sessionState) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	c.states[id] = state
}
...
// [github.com/snight1983/ds-harness-go/sessionlog.SessionHeader] 是一个全是标量的结构体，按值传就是拷贝,
// 「排不成 JSON」在类型上不成立，所以只剩 createdAt 那条取值检查。
func (c *Coordinator) Create(ctx context.Context, meta sessionlog.SessionHeader) error {
	if meta.CreatedAt < 0 {
		return fmt.Errorf("feature/persistence: 会话元数据的 CreatedAt 不能是负数（给的是 %d）", meta.CreatedAt)
	}
...
// 之后续跑谁就说不准了。
func (c *Coordinator) createCore(ctx context.Context, meta sessionlog.SessionHeader) error {
	if c.stateOf(meta.ID) != nil || c.preparations.has(meta.ID) {
		return fmt.Errorf("feature/persistence: 会话 %q 在这个后端里已经存在了", string(meta.ID))
	}
	_, err := c.backend.LoadStored(ctx, meta.ID)
	switch {
	case err == nil:
		return fmt.Errorf(
			"feature/persistence: 会话 %q 在磁盘上已经有一份日志了，去加载或者续跑它，别再造一次",
			string(meta.ID))
...
// 改不动这一批。
func (c *Coordinator) Append(ctx context.Context, id sessionlog.SessionID, events []sessionlog.Event) error {
	batch := make([]sessionlog.Event, len(events))
...
func (c *Coordinator) appendCore(ctx context.Context, id sessionlog.SessionID, events []sessionlog.Event) error {
	if len(events) == 0 {
		return nil
	}
	...
	for index, event := range events {
		if event.Seq != nextSeq+index {
			return fmt.Errorf(
				"feature/persistence: 会话 %q 的这一批 seq 对不上：第 %d 条应该是 %d，给的是 %d",
...
func (c *Coordinator) trim(ctx context.Context, id sessionlog.SessionID, state *sessionState) {
	if c.trimmer == nil {
		return
	}
	c.mutex.Lock()
...
// 开头先看一眼。
func (c *Coordinator) adopt(ctx context.Context, id sessionlog.SessionID) (*sessionState, error) {
	for {
		if err := ctx.Err(); err != nil {
			return nil, err
		}
...
func (c *Coordinator) ReadFrom(ctx context.Context, id sessionlog.SessionID, fromSeq int) (StoredSuffix, error) {
	if fromSeq < 0 {
...
func (c *Coordinator) readFromCore(ctx context.Context, id sessionlog.SessionID, fromSeq int) (StoredSuffix, error) {
	if err := ctx.Err(); err != nil {
		return StoredSuffix{}, err
	}
...
func (c *Coordinator) readStoredPrefix(ctx context.Context, id sessionlog.SessionID) (StoredPrefix, error) {
	if err := ctx.Err(); err != nil {
...
// 「这份存档坏了」。
func wrapCorruption(id sessionlog.SessionID, err error) error {
