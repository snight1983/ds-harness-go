...
// 一个能续跑的会话。
func (c *Coordinator) Prepare(ctx context.Context, id sessionlog.SessionID) (*coresession.Preparation, error) {
	for {
		...
		if _, live := c.sessions.Get(id); live {
			return nil, fmt.Errorf("feature/persistence: 会话 %q 还活着，准备不了它", string(id))
		}
...
func (c *Coordinator) Load(ctx context.Context, id sessionlog.SessionID) (Inspection, error) {
	for {
		if err := ctx.Err(); err != nil {
...
// 不可变视图。
func (c *Coordinator) Inspect(ctx context.Context, id sessionlog.SessionID) (Inspection, error) {
	for {
		if err := ctx.Err(); err != nil {
			return Inspection{}, err
		}
...
//
// 摆在编排器上的理由只有一个：[github.com/snight1983/ds-harness-go/harness/agentloop.SessionPersistence]
...
func (c *Coordinator) List(ctx context.Context) ([]sessionlog.SessionHeader, error) {
...
func (c *Coordinator) inspectOnce(ctx context.Context, id sessionlog.SessionID) (Inspection, bool, error) {
...
func (c *Coordinator) preparationLoaderFor(id sessionlog.SessionID) preparationLoader {
...
func (c *Coordinator) prepareCore(ctx context.Context, id sessionlog.SessionID) (*preparedSource, error) {
...
func (c *Coordinator) buildPreparedSource(id sessionlog.SessionID, stored StoredPrefix) (*preparedSource, error) {
...
func (c *Coordinator) commitPrepared(ctx context.Context, source *preparedSource) (*commitResult, error) {
	...
	c.mutex.Unlock()
	if owned {
		return nil, fmt.Errorf("feature/persistence: 会话 %q 已经有一个活着的持久化主人了", string(id))
	}
...
func (c *Coordinator) loadLiveSnapshot(live *coresession.Session) (Inspection, error) {
	...
	if state == nil {
		...
		return Inspection{}, fmt.Errorf(
			"feature/persistence: 会话 %q 在加载途中丢了持久化状态", string(live.ID()))
	}
	if len(events) == 0 {
	...
	if len(closers) > 0 {
		return Inspection{}, fmt.Errorf(
			"feature/persistence: 会话 %q 的回合还开着，这时候加载不了它；"+
