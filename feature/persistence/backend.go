...
type Backend interface {
	...
	LoadStored(ctx context.Context, id sessionlog.SessionID) (StoredPrefix, error)

	// ReadStoredRevision 只读出一个会话当前的变更令牌，不读它的事件日志。
	//
	// 身份不存在时返回 [ErrSessionNotFound]。
	ReadStoredRevision(ctx context.Context, id sessionlog.SessionID) (Revision, error)
...
type SeekableBackend interface {
	...
	LoadStoredFrom(ctx context.Context, id sessionlog.SessionID, fromSeq int) (StoredSuffix, error)
}
...
func LocateWith(backend Backend, meta sessionlog.SessionHeader) (Location, bool) {
