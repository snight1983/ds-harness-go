...
func (r *recorder) write(events []sessionlog.Event) error {
...
// event 造一条只带 seq 的事件。
func event(seq int) sessionlog.Event {
...
func TestAStaleDeadlineStandsDown(t *testing.T) {
	...
	behind.mu.Lock()
	behind.pending = []sessionlog.Event{event(1)}
	behind.generation = 7
	behind.mu.Unlock()
...
func TestEnqueueClonesTheMutableSlices(t *testing.T) {
	...
	// 影响不到将要落盘的内容。
	captured := make(chan sessionlog.Event, 1)
	behind := NewWriteBehind(WriteBehindOptions{
...
func TestCloneEventKeepsAnEmptyButPresentSourceList(t *testing.T) {
	...
	cloned := cloneEvent(sessionlog.Event{SourceEventSeqs: []int{}})
	...
	if cloneEvent(sessionlog.Event{}).SourceEventSeqs != nil {
		t.Fatalf("没给就该还是没给")
	}
}
