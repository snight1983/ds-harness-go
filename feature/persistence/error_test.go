...
func TestCorruptionErrorAnswersBothQuestions(t *testing.T) {
	...
	err := &CorruptionError{ID: "s1", Cause: sessionlog.ErrTraceViolation}

	if !errors.Is(err, ErrCorrupted) {
		t.Fatalf("该认出这是一次损坏")
	}
	if !errors.Is(err, sessionlog.ErrTraceViolation) {
...
func TestFormatVersionRefusalPointsInTheRightDirection(t *testing.T) {
	...
	newer := FormatVersionRefusal("s1", sessionlog.FormatVersion+1)
	older := FormatVersionRefusal("s1", sessionlog.FormatVersion-1)
