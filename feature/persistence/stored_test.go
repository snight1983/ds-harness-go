...
func TestCheckStoredVersion(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"本构建的版本":   {version: sessionlog.FormatVersion},
		"比本构建新":    {version: sessionlog.FormatVersion + 1, refuses: true},
		"比本构建旧":    {version: sessionlog.FormatVersion - 1, refuses: true},
		"没写版本（零值）": {version: 0, refuses: sessionlog.FormatVersion != 0},
	}
...
func TestCheckStoredVocabulary(t *testing.T) {
	...
	t.Run("全认识就放行", func(t *testing.T) {
		...
		events := []sessionlog.Event{userEvent(t, 1, "hi")}
	...
	t.Run("不认识的必需事件拒整份日志", func(t *testing.T) {
		...
		events := []sessionlog.Event{{Type: "future/thing", Seq: 1}}
	...
	t.Run("标了可跳过就跳过", func(t *testing.T) {
		...
		events := []sessionlog.Event{{Type: "future/thing", Seq: 1, Ignorable: true}}
		if err := CheckStoredVocabulary(meta, events, vocabulary); err != nil {
...
func TestCheckStoredRunsTheChecksInOrder(t *testing.T) {
	...
	err := CheckStored(nil, "s1", meta, events, sessionlog.CoreVocabulary())
	if !errors.Is(err, ErrIdentityMismatch) {
	...
	meta.Version = sessionlog.FormatVersion
	err = CheckStored(nil, "s1", meta, events, sessionlog.CoreVocabulary())
	...
	// 三道全过就放行。
	good := []sessionlog.Event{userEvent(t, 1, "hi")}
	if err := CheckStored(nil, "s1", meta, good, sessionlog.CoreVocabulary()); err != nil {
...
func TestCheckStoredLocatesTheRefusalWhenTheBackendKnowsTheWay(t *testing.T) {
	...
	t.Run("认路的后端把路径补上去", func(t *testing.T) {
		...
		err := CheckStored(locatingStub{path: "/logs/s1.jsonl"}, "s1", meta, nil, sessionlog.CoreVocabulary())
		var refusal *FormatUnsupportedError
	...
	t.Run("认路但这个会话没有独立存档", func(t *testing.T) {
		...
		err := CheckStored(locatingStub{}, "s1", meta, nil, sessionlog.CoreVocabulary())
		var refusal *FormatUnsupportedError
		if !errors.As(err, &refusal) {
			t.Fatalf("该给出一条格式拒绝：%v", err)
		}
	...
	t.Run("不认路的后端原样上抛", func(t *testing.T) {
		...
		err := CheckStored(plainStub{}, "s1", meta, nil, sessionlog.CoreVocabulary())
		var refusal *FormatUnsupportedError
		if !errors.As(err, &refusal) || refusal.Location != nil {
...
func TestBalanceStored(t *testing.T) {
	...
	t.Run("平衡的日志原样返回", func(t *testing.T) {
		...
		events := []sessionlog.Event{turnStart(t, 1, 0), turnEnd(t, 2, 0)}
	...
	t.Run("断掉的尾巴补上收尾", func(t *testing.T) {
		...
		events := []sessionlog.Event{turnStart(t, 1, 0)}
		balanced, closers, err := BalanceStored(events)
		if err != nil {
			t.Fatalf("不该报错：%v", err)
		}
		...
		if balanced[len(balanced)-1].Type != sessionlog.EventTurnEnd {
	...
	t.Run("坏负载上抛", func(t *testing.T) {
		...
		events := []sessionlog.Event{{Type: sessionlog.EventTurnStart, Seq: 1, Data: []byte(`"不是对象"`)}}
...
func TestSeedCoversPrefix(t *testing.T) {
	...
	cases := map[string]struct {
		seed, prefix []sessionlog.Event
		covers       bool
	}{
		"逐条复现":       {seed: []sessionlog.Event{a, b}, prefix: []sessionlog.Event{a, b}, covers: true},
		"seed 更长":    {seed: []sessionlog.Event{a, b}, prefix: []sessionlog.Event{a}, covers: true},
		"空前缀":        {seed: []sessionlog.Event{a}, covers: true},
		"前缀比 seed 长": {seed: []sessionlog.Event{a}, prefix: []sessionlog.Event{a, b}},
		"某一条不一样":     {seed: []sessionlog.Event{a, a}, prefix: []sessionlog.Event{a, b}},
	}
	...
	t.Run("排不出去的事件报错", func(t *testing.T) {
		...
		broken := sessionlog.Event{Type: sessionlog.EventUserMessage, Seq: 1, Data: []byte("{")}
		if _, err := SeedCoversPrefix([]sessionlog.Event{broken}, []sessionlog.Event{a}); err == nil {
			t.Fatalf("坏事件该报错")
		}
		if _, err := SeedCoversPrefix([]sessionlog.Event{a}, []sessionlog.Event{broken}); err == nil {
			t.Fatalf("坏事件该报错")
		}
	})
}
...
func (plainStub) LoadStored(context.Context, sessionlog.SessionID) (StoredPrefix, error) {
	return StoredPrefix{}, ErrSessionNotFound
}

func (plainStub) ReadStoredRevision(context.Context, sessionlog.SessionID) (Revision, error) {
	return "", ErrSessionNotFound
}

func (plainStub) AppendBatch(context.Context, sessionlog.SessionHeader, []sessionlog.Event, bool) error {
	return nil
}

func (plainStub) CommitRepair(context.Context, sessionlog.SessionHeader, any, []sessionlog.Event) error {
	return nil
}

func (plainStub) List(context.Context) ([]sessionlog.SessionHeader, error) { return nil, nil }
...
func (s locatingStub) Locate(sessionlog.SessionHeader) (Location, bool) {
	if s.path == "" {
