...
type memoryBackend struct {
	mutex sync.Mutex
	logs  map[sessionlog.SessionID]*memoryLog
...
func newMemoryBackend() *memoryBackend {
	return &memoryBackend{logs: map[sessionlog.SessionID]*memoryLog{}}
}

func (b *memoryBackend) Name() string { return "内存后端" }

// seed 直接往存档里塞一份日志，绕过编排器——用来搭「磁盘上已经有东西」的局面。
func (b *memoryBackend) seed(meta sessionlog.SessionHeader, events []sessionlog.Event, torn any) {
	b.mutex.Lock()
...
func (b *memoryBackend) storedEvents(id sessionlog.SessionID) []sessionlog.Event {
	b.mutex.Lock()
	defer b.mutex.Unlock()
	log, ok := b.logs[id]
	if !ok {
...
func (b *memoryBackend) LoadStored(ctx context.Context, id sessionlog.SessionID) (StoredPrefix, error) {
	b.mutex.Lock()
	defer b.mutex.Unlock()
	if b.loadErr != nil {
...
// 那份存档就没了」的局面。
func (b *memoryBackend) drop(id sessionlog.SessionID) {
	b.mutex.Lock()
	defer b.mutex.Unlock()
	delete(b.logs, id)
}

func (b *memoryBackend) ReadStoredRevision(ctx context.Context, id sessionlog.SessionID) (Revision, error) {
	b.mutex.Lock()
	defer b.mutex.Unlock()
...
func (b *memoryBackend) List(ctx context.Context) ([]sessionlog.SessionHeader, error) {
	b.mutex.Lock()
	defer b.mutex.Unlock()
	headers := make([]sessionlog.SessionHeader, 0, len(b.logs))
...
func (h *harness) createLive(t *testing.T, id sessionlog.SessionID, options coresession.CreateOptions) *coresession.Session {
	t.Helper()
...
func TestCoordinator登记之后第一次追加才落盘(t *testing.T) {
	...
	if err := h.Append(t.Context(), id, []sessionlog.Event{userEvent(t, 0, "甲")}); err != nil {
		t.Fatalf("追加失败：%v", err)
	}
...
func TestCoordinator重复登记与撞上存档都被拦下(t *testing.T) {
	...
	h.backend.seed(existing, []sessionlog.Event{userEvent(t, 0, "甲")}, nil)
	if err := h.Create(t.Context(), existing); err == nil {
		t.Fatal("磁盘上已经有一份日志的身份不该造得出来")
	}

	if err := h.Create(t.Context(), sessionlog.SessionHeader{
		Version: sessionlog.FormatVersion, ID: "负时间", CreatedAt: -1,
	}); err == nil {
...
func TestCoordinator追加要接得上存档的尾巴(t *testing.T) {
	...
	h := newHarness(t)
	id := sessionlog.SessionID("连续性")
	if err := h.Create(t.Context(), testHeader(t, id)); err != nil {
		t.Fatalf("登记失败：%v", err)
	}
	if err := h.Append(t.Context(), id, []sessionlog.Event{userEvent(t, -1, "负号")}); !errors.Is(err, ErrMalformedSeq) {
	...
	// 定下来之后，接不上尾巴的那些照旧拒掉。
	if err := h.Append(t.Context(), id, []sessionlog.Event{userEvent(t, 505, "跳号")}); err == nil {
...
func TestCoordinator准备之后发布那个会话(t *testing.T) {
	...
	// 独占期间不许有别的东西写进这个身份。
	if err := h.Append(t.Context(), id, []sessionlog.Event{userEvent(t, 9, "插队")}); err == nil {
		t.Fatal("独占期间不该写得进去")
	}
...
func TestCoordinator查看不刷盘也不拦开着的回合(t *testing.T) {
	...
	h := newHarness(t)
	id := sessionlog.SessionID("查看")
	h.backend.seed(testHeader(t, id), []sessionlog.Event{turnStart(t, 0, 1), userEvent(t, 1, "甲")}, nil)
...
func TestCoordinator加载活会话时开着的回合被拦下(t *testing.T) {
	...
	h := newHarness(t)
	id := sessionlog.SessionID("活的回合开着")
	live := h.createLive(t, id, coresession.CreateOptions{})
	if _, err := live.Append(sessionlog.Event{
		Type: sessionlog.EventTurnStart, Data: turnStart(t, 0, 1).Data,
	}); err != nil {
		t.Fatalf("追加失败：%v", err)
	}
	if _, err := h.Load(t.Context(), id); err == nil {
...
func TestCoordinator收编一份磁盘上的前缀(t *testing.T) {
	...
	h := newHarness(t)
	id := sessionlog.SessionID("收编")
	seed := []sessionlog.Event{userEvent(t, 0, "甲"), userEvent(t, 1, "乙")}
...
func TestCoordinator工作目录不同也是撞号(t *testing.T) {
	...
	h := newHarness(t)
	h.drainMayFail = true
	id := sessionlog.SessionID("目录不同")
	meta := testHeader(t, id)
	meta.WorkspaceID = "ws-这一处"
	h.backend.seed(meta, []sessionlog.Event{userEvent(t, 0, "甲")}, nil)
...
func TestCoordinator收编时只截尾巴不补收尾(t *testing.T) {
	...
	h := newHarness(t)
	id := sessionlog.SessionID("只截")
	seed := []sessionlog.Event{turnStart(t, 0, 1), userEvent(t, 1, "甲")}
...
func TestCoordinator认领一份没主的状态(t *testing.T) {
	...
	if stored[0].Type != sessionlog.EventUserMessage {
		t.Fatalf("头一条该还是那条用户消息，拿到 %q", stored[0].Type)
	}
	if stored[1].Type != sessionlog.EventSessionEndSeed {
...
func TestCoordinator认领没主状态时seed对不上被拒(t *testing.T) {
	...
	h := newHarness(t)
	h.drainMayFail = true
	id := sessionlog.SessionID("没主但对不上")
	if err := h.Create(t.Context(), testHeader(t, id)); err != nil {
		t.Fatalf("登记失败：%v", err)
	}
	if err := h.Append(t.Context(), id, []sessionlog.Event{userEvent(t, 0, "落盘的")}); err != nil {
		t.Fatalf("追加失败：%v", err)
	}

	live := h.createLive(t, id, coresession.CreateOptions{
		Seed: []sessionlog.Event{userEvent(t, 0, "活的")},
	})
	if err := h.flush(live); err == nil {
...
func TestCoordinator后端写不下去时错传得回来(t *testing.T) {
	...
	id := sessionlog.SessionID("写不下去")
	if err := h.Create(t.Context(), testHeader(t, id)); err != nil {
		t.Fatalf("登记失败：%v", err)
	}
	err := h.Append(t.Context(), id, []sessionlog.Event{userEvent(t, 0, "甲")})
	if err == nil {
		t.Fatal("后端写不下去时该报错")
	}
