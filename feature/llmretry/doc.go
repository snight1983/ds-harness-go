...
//
// 本包只认 [github.com/snight1983/ds-harness-go/harness/agent.Registry] 一个宿主。
//
// # 为什么排期和熬过去是两条事件
//
...
//     [github.com/snight1983/ds-harness-go/sessionlog.Vocabulary] 也是个闭合的值，所以本包交出 [EventTypes]：
//
...
// # 不做什么
//
//   - **不定重试几次、退避多久。**那份策略是适配器登记路由那一刻定下来的
//     （[github.com/snight1983/ds-harness-go/llm.ResolveRetryPolicy]），本包只执行它。
//   - **不判断什么算失败。**失败分类和可不可重试是
//     [github.com/snight1983/ds-harness-go/llm.Error] 上的事实，不是这里的判断。
//   - **不发请求，也不重发请求。**重发那一下由请求失败瀑布上的宿主接着做，本包只回答
//     「要不要、等多久」并把这个决定写进日志。
//   - **不动模型看得见的历史。**两条重试事件只进日志，不上表面。
//   - **不自己拼会话词汇。**[EventTypes] 交出去，并进
//     [github.com/snight1983/ds-harness-go/sessionlog.Vocabulary] 是装配方的事。
package llmretry
