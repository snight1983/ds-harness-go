...
func rejects(t *testing.T, events []sessionlog.Event) string {
	t.Helper()
...
func rejectsWith(t *testing.T, events []sessionlog.Event, want string) {
	t.Helper()
...
func TestARetryOutsideAnyOpenTurnIsRejected(t *testing.T) {
	...
	rejectsWith(t, []sessionlog.Event{retryEvent(t, 1, normalRetry("r-1", 1, 3))}, "开着的回合之外")
}
...
func TestAnUnreadableRetryPayloadIsAViolation(t *testing.T) {
	...
	message := rejects(t, []sessionlog.Event{rawEvent(1, EventRetry, `{"retry":"一"}`)})
...
func newRetryInvariantHarness(t *testing.T, loaded ...sessionlog.Event) *retryInvariantHarness {
	t.Helper()
...
func (h *retryInvariantHarness) emit(event sessionlog.Event) {
