...
//
// 这个包只认 [github.com/snight1983/ds-harness-go/harness/agent.Registry] 一个宿主：策略是适配器在登记路由
...
type Options struct {
	...
	// [github.com/snight1983/ds-harness-go/feature/interaction/userapproval] 上那条一样——不变量要验「链身份一路
	// 不变」，测试得排得出一串认得出来的身份。
	NewID func() string
...
func (i *installation) backoff(
	...
	if _, err := live.Append(sessionlog.Event{Type: EventRetry, Data: payload}); err != nil {
		return agent.RequestErrorAction{}, err
	}
	...
	if _, err := live.Append(sessionlog.Event{Type: EventRetryStarted, Data: started}); err != nil {
		return agent.RequestErrorAction{}, err
	}
...
type sessionAppender interface {
	...
	Append(candidate sessionlog.Event) (sessionlog.Event, error)
}
...
func lastChainRetry(
	events []sessionlog.Event, turn, step int, provider, policyKey string,
