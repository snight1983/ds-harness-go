...
// [sessionlog.CheckVocabulary] 判成「有不认识的事件类型」而整个拒掉。
...
func TestEventTypesListsBothSoTheVocabularyCanBeOpened(t *testing.T) {
	...
	vocabulary := sessionlog.CoreVocabulary().With(got...)
	events := []sessionlog.Event{
		eventAt(1, EventRetry, json.RawMessage(`{}`)),
		eventAt(2, EventRetryStarted, json.RawMessage(`{}`)),
	}
	if err := sessionlog.CheckVocabulary(events, vocabulary); err != nil {
...
func TestDecodeTreatsAnEmptyPayloadAsAnEmptyObject(t *testing.T) {
	...
	data, err := DecodeRetryStarted(sessionlog.Event{Type: EventRetryStarted, Seq: 1})
	if err != nil {
