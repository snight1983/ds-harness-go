...
func FoldMode(events []sessionlog.Event) bool {
...
func FoldModeUntil(events []sessionlog.Event, end int) bool {
	if end > len(events) {
		end = len(events)
	}
	active := false
...
// 更接近日志真正说的话。理由和 [github.com/snight1983/ds-harness-go/feature/todo] 那个投影折叠逐字相同。
func decodeMode(event sessionlog.Event) (bool, bool) {
	var payload struct {
...
func modeAtLastHeader(events []sessionlog.Event) (bool, bool) {
	lastHeader := -1
