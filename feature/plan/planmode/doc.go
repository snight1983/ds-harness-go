...
// 先后两次请求会带着互相矛盾的指引，而模型看到的誊本里没有任何东西解释这件事。
//
...
// Go 没有弱引用，所以换成一张按 [github.com/snight1983/ds-harness-go/sessionlog.SessionID] 索引的表，
// 由装配方在会话散掉时调 [Controller.OnSessionDisposed] 清理（成例见
// [github.com/snight1983/ds-harness-go/feature/sessiontitle.Service.OnSessionDisposed]）。不清理的后果
// 只是一条永远不会被读到的挂起记录，不会影响任何别的会话。
//
// 新增: DSH 的 exit 工具、`/plan` 命令、提示词段落都从一个结构类型的 agent 对象上
// 直接摸到 `agent.session`。Go 这几处拿到的是一把不透明的
// [github.com/snight1983/ds-harness-go/scope.Key]，所以由装配方经 [Config.AgentOf] 交进来一条
// 「从钥匙找 agent」的路（成例见
// [github.com/snight1983/ds-harness-go/feature/interaction/commands.Options.LogOf]、
// [github.com/snight1983/ds-harness-go/feature/todo.Config.Append]）。
//
...
// # 不做什么
//
//   - **不是沙箱，也不是审批策略。**那两样各自独立地施加限制，它们不读也不写计划
//     状态；计划模式是一条软引导，不是一道闸。
//   - **不改请求里的工具表。**[ExitToolName] 一直挂着，进出计划模式只换一段提示词——
//     工具表一变，提示词缓存就整段作废。
//   - **不写那段指引的正文。**指引归部署方，本包只负责在计划模式开着的时候把它装进去。
//   - **不评价计划写得好不好。**计划正文由模型产出，本包只管状态怎么翻、评审这条路
//     怎么走。
//   - **不自己找 agent。**从一把 [github.com/snight1983/ds-harness-go/scope.Key] 到一个 agent 的
//     映射由装配方经 [Config.AgentOf] 交进来。
//   - **不留第二份状态。**当下生效的那个状态只从日志折（见 [FoldMode]）；挂起的用户
//     选择是进程局部的一张表，丢了只是那一次选择没落地。
package planmode
