...
type Service struct {
	...
	work map[sessionlog.SessionID]*workState
}
...
func (s *Service) OnEvent(sess Session, event sessionlog.Event) {
	switch event.Type {
...
func (s *Service) OnMainRequest(sess Session, options llm.GenerateOptions) {
	...
	if !hasBoundary || boundary.Type != sessionlog.EventStepStart || boundary.Seq <= state.pending.throughSeq {
		return
	}
	route, err := routeOf(events)
...
func (s *Service) onUserMessage(sess Session, event sessionlog.Event) {
...
func (s *Service) onRequestHeader(sess Session, event sessionlog.Event) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	header, ok := data.(sessionlog.RequestHeaderData)
	if !ok {
		return
	}
...
func routeOf(events []sessionlog.Event) (*ModelProvenance, error) {
	header, ok, err := sessionlog.FoldRequestHeader(events, sessionlog.EpochHeader{}, false)
	if err != nil {
		return nil, err
	}
	if !ok {
...
func lastStepBoundary(events []sessionlog.Event) (sessionlog.Event, bool) {
	for index := len(events) - 1; index >= 0; index-- {
		switch events[index].Type {
		case sessionlog.EventStepStart, sessionlog.EventStepEnd:
