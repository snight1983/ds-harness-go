...
const EventTitleLLMRequest sessionlog.EventType = "session/title-llm-request"

// EventTypes 是本包往会话日志里写的那几种事件类型。
//
// 新增: 理由与 [sessiontitle.EventTypes] 逐字相同——Go 没有声明合并，
// [sessionlog.Vocabulary] 是个闭合的值，所以装配方要自己把它拼进去：
//
...
func EventTypes() []sessionlog.EventType {
	return []sessionlog.EventType{EventTitleLLMRequest}
}
