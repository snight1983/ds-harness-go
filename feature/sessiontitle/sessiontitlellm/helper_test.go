...
func (s *fakeSession) Header() sessionlog.SessionHeader { return sessionlog.SessionHeader{} }
...
func (s *fakeSession) Append(kind sessionlog.EventType, data any) error {
	...
	event := sessionlog.Event{Type: kind, Seq: len(s.events), Data: raw}
...
// 用来铺测试的初始日志。
func (s *fakeSession) seed(events ...sessionlog.Event) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *fakeSession) watchAppends() chan sessionlog.Event {
	...
	s.notify = make(chan sessionlog.Event, 32)
	return s.notify
}
...
func userMessageEvent(t *testing.T, text string) sessionlog.Event {
	t.Helper()

	raw, err := json.Marshal(sessionlog.UserMessageData{Message: llm.Message{
		ID:      "m",
		Role:    llm.RoleUser,
	...
	return sessionlog.Event{Type: sessionlog.EventUserMessage, Data: raw, SurfaceOp: sessionlog.AppendOp{}}
}

// requestHeaderEvent 造一条钉住路由的 request/header。
func requestHeaderEvent(t *testing.T, provider, model string) sessionlog.Event {
	t.Helper()

	raw, err := json.Marshal(sessionlog.RequestHeaderData{
	...
	return sessionlog.Event{Type: sessionlog.EventRequestHeader, Data: raw}
}
...
func waitForTitleEvent(t *testing.T, appended <-chan sessionlog.Event) sessiontitle.EventData {
	t.Helper()
