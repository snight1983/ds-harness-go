...
// [sessionlog.EpochHeader] 自己的 MarshalJSON 会先失败，那样的事件根本写不出来。）
func TestContextBreakdownKeepsThePreviousFiguresOnAnUnreadableHeader(t *testing.T) {
	t.Parallel()

	good := sessionlog.EpochHeader{
	...
	broken := sessionlog.Event{Seq: 1, Type: sessionlog.EventRequestHeader, Data: []byte(`{"header":42}`)}
