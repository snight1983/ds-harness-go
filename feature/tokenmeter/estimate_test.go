...
func TestEstimateHeaderOfAnEmptyHeaderIsZero(t *testing.T) {
	...
	got, err := EstimateHeader(sessionlog.EpochHeader{})
	if err != nil {
		t.Fatalf("估价不该失败：%v", err)
	}
	if got != 0 {
	...
	if got := EstimateSystemTokens(sessionlog.EpochHeader{}); got != 0 {
...
func TestEstimateToolsTokensFailsOnAnUnmarshalableSchema(t *testing.T) {
	...
	header := sessionlog.EpochHeader{Tools: []llm.ToolSchema{{
