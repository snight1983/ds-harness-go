...
//   - **切不出单个回合。**它是整份日志累计下来的账，同一个 (turn, step) 重复采样是
//     last-wins，「这一个回合花了多少、走了哪几条路由」答不了——这一角在能力账本里
//     明写着是缺口。
//   - **不按路由给图片计价。**图片走的是一条固定的启发式，
//     [github.com/snight1983/ds-harness-go/llm] 那边没有计价接缝可以问（账本里同样记着）。
