...
// Go 这边 [sessionlog.EpochHeader] 是值类型、System 是空串表示没有，所以「头不在」
// 和「一份空头」算出来是同一个 0——这两件事在**定价**上本来就没有区别。
// 它们的区别只在 [TokenMeter.Measure] 里那次锚点复用判定上，由那边自己拿着
// 存在位去分。
func EstimateSystemTokens(header sessionlog.EpochHeader) int {
	if header.System == "" {
		return 0
	}
...
func EstimateToolsTokens(header sessionlog.EpochHeader) (int, error) {
...
func EstimateHeader(header sessionlog.EpochHeader) (int, error) {
