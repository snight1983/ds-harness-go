...
func foldSurfaceProjection(claim *ShadowPriceClaim, event sessionlog.Event) (surfaceTokensFold, error) {
	switch event.Type {
	...
	if event.SurfaceOp.SurfaceOpKind() == sessionlog.OpAppend {
		return surfaceTokensFold{deltaTokens: tokens}, nil
	}
	replace, isReplace := event.SurfaceOp.(sessionlog.ReplaceOp)
	if !isReplace {
...
func foldSurfaceProjectionLenient(claim *ShadowPriceClaim, event sessionlog.Event) surfaceTokensFold {
