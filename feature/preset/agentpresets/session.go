...
const EventPresetSelected sessionlog.EventType = "agent-preset/selected"
...
//
//	vocabulary := sessionlog.CoreVocabulary().With(agentpresets.EventTypes()...)
//
// 不拼的话，一段换过预设的日志会被 [sessionlog.CheckVocabulary] 判成
// 「有不认识的事件类型」而整个拒掉。
func EventTypes() []sessionlog.EventType {
	return []sessionlog.EventType{EventPresetSelected}
}
...
// 再包一层只是让调用方多建一个临时值。
func ResolveSessionPreset(header sessionlog.SessionHeader, events []sessionlog.Event) string {
	for index := len(events) - 1; index >= 0; index-- {
		...
		// 字节，所以要多问一句「读得回来吗」（成例见 [github.com/snight1983/ds-harness-go/feature/plan/planmode] 的
