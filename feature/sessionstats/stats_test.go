...
// event 造一条带着这份负载的事件。
func event(t *testing.T, kind sessionlog.EventType, at int64, payload any) sessionlog.Event {
	t.Helper()
	...
	return sessionlog.Event{Type: kind, Time: at, Data: data}
}

// stepStart 造一条步骤开始。
func stepStart(t *testing.T, turn, step int, at int64) sessionlog.Event {
	t.Helper()

	return event(t, sessionlog.EventStepStart, at, sessionlog.StepStartData{Turn: turn, Step: step})
}

// stepEnd 造一条步骤结束。
func stepEnd(t *testing.T, turn, step int, at int64) sessionlog.Event {
	t.Helper()

	return event(t, sessionlog.EventStepEnd, at, sessionlog.StepEndData{Turn: turn, Step: step})
}

// chunk 造一条助手分块。
func chunk(t *testing.T, turn, step int, at int64, piece llm.StreamChunk) sessionlog.Event {
	t.Helper()

	return event(t, sessionlog.EventAssistantChunk, at,
		sessionlog.AssistantChunkData{Turn: turn, Step: step, Chunk: piece})
}
...
func assistantMessage(t *testing.T, turn, step int, at int64, usage *llm.TokenUsage) sessionlog.Event {
	t.Helper()

	return event(t, sessionlog.EventAssistantMessage, at, sessionlog.AssistantMessageData{
		Turn:    turn,
		Step:    step,
...
// toolCall 造一条工具调用派发。
func toolCall(t *testing.T, turn, step int, at int64, callID llm.CallID) sessionlog.Event {
	t.Helper()

	return event(t, sessionlog.EventToolCall, at, sessionlog.ToolCallData{
		Turn: turn, Step: step, CallID: callID, Name: "read", Arguments: "{}",
	})
}

// toolResult 造一条工具结果。
func toolResult(t *testing.T, turn, step int, at int64, callID llm.CallID) sessionlog.Event {
	t.Helper()

	return event(t, sessionlog.EventToolResult, at, sessionlog.ToolResultData{
		Turn:    turn,
		Step:    step,
...
// turnEnd 造一条回合结束。
func turnEnd(t *testing.T, turn int, at int64) sessionlog.Event {
	t.Helper()

	return event(t, sessionlog.EventTurnEnd, at,
		sessionlog.TurnEndData{Turn: turn, Reason: sessionlog.CompletedTurnEnd{}})
}

// fold 把这些事件依次折进一份全新的状态，顺带记下每一步说自己改没改。
func fold(t *testing.T, events ...sessionlog.Event) (State, []bool) {
	t.Helper()
...
func figuresOf(t *testing.T, events ...sessionlog.Event) Figures {
	t.Helper()
...
func TestAResultMessageWithoutAToolSourceChangesNothing(t *testing.T) {
	...
	broken := event(t, sessionlog.EventToolResult, 300, sessionlog.ToolResultData{
...
func TestUnrelatedEventsAreLeftAlone(t *testing.T) {
	...
	cases := map[string]sessionlog.Event{
		"回合开始":   {Type: sessionlog.EventTurnStart, Time: 10, Data: json.RawMessage(`{"turn":0}`)},
		"用户消息":   {Type: sessionlog.EventUserMessage, Time: 10, Data: json.RawMessage(`{}`)},
		"待办":     {Type: sessionlog.EventTodoWrite, Time: 10, Data: json.RawMessage(`{}`)},
...
func TestAPayloadThatCannotBeReadBackIsPassedOver(t *testing.T) {
	...
	for _, kind := range kinds {
		t.Run(string(kind), func(t *testing.T) {
			...
			broken := sessionlog.Event{Type: kind, Time: 10, Data: json.RawMessage(`[]`)}
...
func TestAnEmptyPayloadReadsAsTheZeroValue(t *testing.T) {
	...
	// 一条负载整个缺席的步骤结束读成 (turn 0, step 0)，和 [sessionlog.DecodeData]
	// 的处理一致——这是日志里合法的形状，不是坏掉的形状。
	state, changes := fold(t, sessionlog.Event{Type: sessionlog.EventStepEnd, Time: 10})
...
func TestTheUnitGoesIntoARegistryAndComesOutAsASnapshot(t *testing.T) {
	...
	view := &fakeSession{events: []sessionlog.Event{
		stepStart(t, 0, 0, 100),
		chunk(t, 0, 0, 180, text("你")),
		assistantMessage(t, 0, 0, 400, &llm.TokenUsage{OutputTokens: 42}),
		stepEnd(t, 0, 0, 410),
...
func (s *fakeSession) ID() sessionlog.SessionID { return "s1" }
