...
func (r Record) Validate() error {
	if !r.Channel.Valid() {
		return fmt.Errorf("feature/telemetry: 通道只能是 %q 或 %q，给的是 %q",
			ChannelLedger, ChannelOps, r.Channel)
	}
	if !r.Severity.Valid() {
		return fmt.Errorf("feature/telemetry: 级别只能是 %q、%q 或 %q，给的是 %q",
			SeverityInfo, SeverityWarn, SeverityError, r.Severity)
	}
	for name, value := range r.Attributes {
		if !validAttribute(value) {
			return fmt.Errorf("feature/telemetry: 属性 %q 的值只能是字符串或数字，给的是 %T",
				name, value)
		}
	}
	if !json.Valid(r.Body) {
		return fmt.Errorf("feature/telemetry: body 不是一段合法的 JSON")
	}
	return nil
}
