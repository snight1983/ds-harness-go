...
func TestFeedingTheProjectionIgnoresEverythingItCannotKey(t *testing.T) {
	...
	view := newView("s1",
		userEvent(t, 1, 11),
		broken(sessionlog.EventAssistantChunk, 2),
		chunkEvent(t, 3, 13, 0, 0))
	view.firstLive = 3
...
func TestAChunkWhosePayloadIsUnreadableIsWithheld(t *testing.T) {
	...
	fixture := newFixture(t)
	view := newView("s1", broken(sessionlog.EventAssistantChunk, 1))
...
func TestSeverityComesFromTheEventsOwnOutcomeBit(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"工具结果的负载读不回来": {
			broken(sessionlog.EventToolResult, 1), SeverityInfo,
		},
		"回合以出错收场": {
			turnEndEvent(t, 1, 11, sessionlog.ErrorTurnEnd{
				Error: llm.Failure{Message: "boom"},
			}),
			SeverityError,
		},
		"回合正常收场": {
			turnEndEvent(t, 1, 11, sessionlog.CompletedTurnEnd{}), SeverityInfo,
		},
		"回合结束的负载读不回来": {
			broken(sessionlog.EventTurnEnd, 1), SeverityInfo,
		},
		"没有结果位的事件": {userEvent(t, 1, 11), SeverityInfo},
		"没听说过的类型": {
			sessionlog.Event{Type: "vendor/thing", Seq: 1, Time: 11}, SeverityInfo,
		},
	}
...
func TestTheIdentityAttributesAreTheEnvelopePlusWhatTheHeaderReallyHas(t *testing.T) {
	...
	bare := identityOf("s1", sessionlog.SessionHeader{Version: 1}, userEvent(t, 7, 11))
	...
	full := identityOf("s1", sessionlog.SessionHeader{
...
func TestAnEventWithNoPayloadStillHasABody(t *testing.T) {
	...
	// 空负载是 `{}`，和 [sessionlog.Event.MarshalJSON] 同一条规则。补齐一次，
	// body 就永远是一段合法 JSON，也就不会因为一条本来没有负载的事件被扣下。
	fixture := newFixture(t)
	view := newView("s1", sessionlog.Event{Type: sessionlog.EventTurnStart, Seq: 1, Time: 11})
...
func TestTheCoordinatorTakesEventsFromManyGoroutinesAtOnce(t *testing.T) {
	...
	for worker := range 4 {
		...
		go func() {
			...
			view := newView(sessionlog.SessionID(string(rune('a'+worker))), userEvent(t, 1, 11))
			fixture.coordinator.Adopt(view)
			fixture.coordinator.Observe(view, view.events[0])
			fixture.coordinator.HintFlush(view)
			fixture.coordinator.Retire(view)
