...
func (v *fakeView) ID() sessionlog.SessionID { return v.header.ID }

func (v *fakeView) Events() []sessionlog.Event { return v.events }

func (v *fakeView) Header() sessionlog.SessionHeader { return v.header }

func (v *fakeView) FirstLiveSeq() int { return v.firstLive }

// newView 造一个假活会话：种子长度为零，也就是整份日志都是这次生命周期产出的。
func newView(id sessionlog.SessionID, events ...sessionlog.Event) *fakeView {
	return &fakeView{
...
// event 造一条带着这份负载的事件。
func event(t *testing.T, kind sessionlog.EventType, seq int, at int64, payload any) sessionlog.Event {
	t.Helper()
	...
	return sessionlog.Event{Type: kind, Seq: seq, Time: at, Data: data}
}

// userEvent 造一条本包不特别对待的事件。
func userEvent(t *testing.T, seq int, at int64) sessionlog.Event {
	t.Helper()

	return event(t, sessionlog.EventUserMessage, seq, at,
...
func chunkEvent(t *testing.T, seq int, at int64, turn, step int) sessionlog.Event {
	t.Helper()

	return event(t, sessionlog.EventAssistantChunk, seq, at, sessionlog.AssistantChunkData{
		Turn: turn, Step: step, Chunk: llm.TextDeltaChunk{Index: 0, Text: "x"},
	})
}

// toolResultEvent 造一条工具结果事件，isError 决定它自己的结果位。
func toolResultEvent(t *testing.T, seq int, at int64, isError bool) sessionlog.Event {
	t.Helper()

	return event(t, sessionlog.EventToolResult, seq, at, sessionlog.ToolResultData{
		Turn: 0, Step: 0,
...
func turnEndEvent(t *testing.T, seq int, at int64, reason sessionlog.TurnEndReason) sessionlog.Event {
	t.Helper()

	return event(t, sessionlog.EventTurnEnd, seq, at,
		sessionlog.TurnEndData{Turn: 0, Reason: reason})
}

// broken 造一条负载读不回来的事件——那段字节不是任何一个负载结构体的形状。
func broken(kind sessionlog.EventType, seq int) sessionlog.Event {
	return sessionlog.Event{Type: kind, Seq: seq, Time: 1, Data: json.RawMessage(`[1,2,3]`)}
}
