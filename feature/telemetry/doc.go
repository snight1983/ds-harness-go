...
//     位置调。同样的做法见 feature/projectioncache。
//
...
// # 不做什么
//
//   - **不投递。**攒批、重试、排队、丢弃策略全在 [Sink.Emit] 下游那份上报 SDK 里。
//     采集同步跑在 agent 循环的热路径上，把一个「要等的」搬进来，迟早会拖住一个
//     「不能等的」。
//   - **不实现任何后端。**OTLP 那一侧在本仓库整支不落地，缺的是采集端这个前置，
//     不是代码。
//   - **不保证送达。**那个交接游标标的是「交出去了」，不是「送到了」——再往下发生
//     什么，采集侧看不见。
//   - **不落盘、不建表。**游标是 [Coordinator] 的一个内存字段，[Coordinator.Retire]
//     时删掉；记录交给接收器就没了。
//   - **不做鉴权和计费。**记录里留下什么由脱敏规则那条瀑布定，谁能看这些记录是上报
//     那一侧的事。
package telemetry
