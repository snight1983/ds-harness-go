...
func (l *fakeLive) ID() sessionlog.SessionID { return l.header.ID }

func (l *fakeLive) Events() []sessionlog.Event { return l.events }
...
func (l *fakeLive) Header() sessionlog.SessionHeader { return l.header }

// newLive 造一个带着这些事件的假活会话，建会话时刻固定，好让身份可预期。
func newLive(id sessionlog.SessionID, createdAt int64, events ...sessionlog.Event) *fakeLive {
	return &fakeLive{
		header: sessionlog.SessionHeader{Version: 1, ID: id, CreatedAt: createdAt},
		events: events,
	}
}
...
func turnEndEvent(seq int) sessionlog.Event {
	return sessionlog.Event{Type: sessionlog.EventTurnEnd, Seq: seq, Data: json.RawMessage(`{}`)}
}
...
type fakeStore struct {
	...
	logs map[sessionlog.SessionID][]sessionlog.Event
	// metas 是每个会话在存档里那份头。
	metas map[sessionlog.SessionID]sessionlog.SessionHeader
...
// put 把一份存档摆进假存储。
func (s *fakeStore) put(meta sessionlog.SessionHeader, events ...sessionlog.Event) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *fakeStore) ReadFrom(_ context.Context, id sessionlog.SessionID, fromSeq int) (persistence.StoredSuffix, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *fakeStore) Locate(sessionlog.SessionHeader) (persistence.Location, bool) {
	return persistence.Location{}, false
}

func (s *fakeStore) SupportsRawArtifacts() bool { return false }

func (s *fakeStore) ReadRaw(context.Context, sessionlog.SessionID) (persistence.RawArtifact, error) {
	return persistence.RawArtifact{}, errNotImplemented
}

func (s *fakeStore) Create(context.Context, sessionlog.SessionHeader) error { return errNotImplemented }

func (s *fakeStore) Append(context.Context, sessionlog.SessionID, []sessionlog.Event) error {
	return errNotImplemented
}

func (s *fakeStore) Load(context.Context, sessionlog.SessionID) (persistence.Inspection, error) {
	return persistence.Inspection{}, errNotImplemented
}

func (s *fakeStore) Inspect(context.Context, sessionlog.SessionID) (persistence.Inspection, error) {
	return persistence.Inspection{}, errNotImplemented
}

func (s *fakeStore) List(context.Context) ([]sessionlog.SessionHeader, error) {
...
func (f *fixture) flushes() []sessionlog.SessionID {
	f.mu.Lock()
	defer f.mu.Unlock()

	return append([]sessionlog.SessionID(nil), f.flushed...)
}
...
func (f *fixture) record(t *testing.T, id sessionlog.SessionID) Record {
	t.Helper()
...
func (f *fixture) stored(ctx context.Context, id sessionlog.SessionID) bool {
