...
// # 这一层在解决什么
//
// sessionlog/projection 那一层能从一份日志把状态折出来，但折的代价随日志长度线性
...
// # 三级读法
//
// 和 sessionlog/projection 那三级（[projection.Registry.Snapshot] 走活会话、
...
// # 不做什么
//
//   - **不是权威。**它只是折叠的捷径：一条记录可能旧（它自己的 seq 说明旧到哪儿），
//     但绝不会错，任何时候整个丢掉都只是退回去多折一段。
//   - **不认识任何投影单元的语义。**状态版本对不上就丢掉那一行，不迁移——迁移要求它
//     读得懂每个单元的状态，而它一个都不认识。
//   - **不自己写文件。**检查点落在装配方交进来的那个
//     [github.com/snight1983/ds-harness-go/storage/domain] 句柄上；介质是什么、什么时候关，
//     归打开它的那一方。
//   - **不让自己的失败传染出去。**每条写路径都是 fail-soft，一次写不下去不该让触发它
//     的那次读或者那条事件跟着失败。
//   - **不管日志本身怎么落盘。**「日志先于缓存」这件事由 [Options.Flush] 那道屏障保证，
//     而那个钩子来自持有活会话的那一层，不是这里。
