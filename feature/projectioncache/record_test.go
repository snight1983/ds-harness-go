...
func TestIdentityIsComparableSoMatchingIsOneEquals(t *testing.T) {
	...
	base := sessionlog.SessionHeader{ID: "s1", CreatedAt: 7, WorkspaceID: "ws-1"}

	cases := map[string]struct {
		other sessionlog.SessionHeader
		same  bool
	}{
		"同一段生命":      {other: sessionlog.SessionHeader{ID: "s1", CreatedAt: 7, WorkspaceID: "ws-1"}, same: true},
		"换了个 id 不影响": {other: sessionlog.SessionHeader{ID: "s2", CreatedAt: 7, WorkspaceID: "ws-1"}, same: true},
		"重建过（时刻变了）":  {other: sessionlog.SessionHeader{ID: "s1", CreatedAt: 8, WorkspaceID: "ws-1"}, same: false},
		"换了工作区":      {other: sessionlog.SessionHeader{ID: "s1", CreatedAt: 7, WorkspaceID: "ws-2"}, same: false},
		"没给工作目录":     {other: sessionlog.SessionHeader{ID: "s1", CreatedAt: 7}, same: false},
	}
