...
type Cache struct {
	...
	mu     sync.Mutex
	dirty  map[sessionlog.SessionID]*dirtyState
	closed bool
}
...
func New(opened *domain.Domain, options Options) (*Cache, error) {
	switch {
	case opened == nil:
		return nil, fmt.Errorf("feature/projectioncache: 建缓存需要一个已经打开的域")
	case options.Registry == nil:
		return nil, fmt.Errorf("feature/projectioncache: 建缓存需要一张投影单元表")
	case options.Store == nil:
		return nil, fmt.Errorf("feature/projectioncache: 建缓存需要一个会话存储")
	case options.Flush == nil:
		// 见 [Options.Flush]：没有它，缓存可能跑到日志前面去。
		return nil, fmt.Errorf("feature/projectioncache: 建缓存需要一道落盘屏障（Flush）")
	case options.WriteEveryEvents < 1:
		return nil, fmt.Errorf("feature/projectioncache: WriteEveryEvents 至少是 1，给的是 %d",
			options.WriteEveryEvents)
	case options.WriteInterval <= 0:
		return nil, fmt.Errorf("feature/projectioncache: WriteInterval 必须是正的，给的是 %s",
...
func (c *Cache) CachedSnapshot(
	ctx context.Context, meta sessionlog.SessionHeader,
...
// 这是原则第 5 条认下的代价。
func (c *Cache) ColdSnapshot(ctx context.Context, id sessionlog.SessionID) (projection.Snapshot, error) {
	...
	if reason != nil {
		...
		c.logger.Warn("feature/projectioncache: 缓存行用不了，退回整读",
...
//
// 三个触发点：[sessionlog.EventTurnEnd] 是必写点（多数读要的就是回合结束时那份
// 值），计数和间隔两个阈值节流回合中间那一串。
func (c *Cache) Observe(live LiveSession, event sessionlog.Event) {
	c.mu.Lock()
	if c.closed {
...
func (c *Cache) writeSoft(live LiveSession, trigger string) {
	if err := c.Write(c.background, live); err != nil {
		c.logger.Warn("feature/projectioncache: 写检查点失败，缓存留在旧位置",
...
func (c *Cache) markClean(id sessionlog.SessionID) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Cache) recordFor(
	ctx context.Context, id sessionlog.SessionID, expected Identity,
) (Record, bool, error) {
...
func (c *Cache) put(ctx context.Context, id sessionlog.SessionID, identity Identity, rows projection.Checkpoint) error {
...
func (c *Cache) putSoft(ctx context.Context, id sessionlog.SessionID, identity Identity,
...
func lastSeq(events []sessionlog.Event) int {
	if len(events) == 0 {
		return -1
	}
