...
func TestCachedSnapshotServesNothingWhenThereIsNoUsableRecord(t *testing.T) {
	...
	cases := map[string]struct {
		...
		arrange func(t *testing.T, f *fixture) sessionlog.SessionHeader
	}{
		"介质上压根没有这条记录": {
			arrange: func(_ *testing.T, _ *fixture) sessionlog.SessionHeader {
				return sessionlog.SessionHeader{ID: "s1", CreatedAt: 7}
			},
		},
		"记录绑的是另一段日志": {
			arrange: func(t *testing.T, f *fixture) sessionlog.SessionHeader {
				...
				return sessionlog.SessionHeader{ID: "s1", CreatedAt: 8}
			},
		},
		"有记录但一行都端不出来": {
			arrange: func(t *testing.T, f *fixture) sessionlog.SessionHeader {
...
func TestCachedSnapshotTakesTheLowestWatermarkAmongTheRowsItServes(t *testing.T) {
	...
	meta := sessionlog.SessionHeader{ID: "s1", CreatedAt: 7}
...
func TestCachedSnapshotSurfacesAStorageFailure(t *testing.T) {
	...
	if _, err := f.cache.CachedSnapshot(t.Context(), sessionlog.SessionHeader{ID: "s1", CreatedAt: 7}); err == nil {
		t.Fatalf("该把读失败报上来")
	}
}
...
func TestColdSnapshotSurfacesAFailureOnTheRecordRead(t *testing.T) {
	...
	f := newFixture(t, 1, time.Second)
	mustRegister(t, f.registry, countUnit("count", 0))
	f.store.put(sessionlog.SessionHeader{ID: "s1", CreatedAt: 7}, userEvent(0))
	closeFixtureDomain(t, f)
...
func TestColdSnapshotProbesTheLogEvenWithNoUnitRegistered(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"空日志":   {want: -1},
		"日志有三条": {events: []sessionlog.Event{userEvent(0), otherEvent(1), userEvent(2)}, want: 2},
	}
	...
	for name, item := range cases {
		t.Run(name, func(t *testing.T) {
			...
			f := newFixture(t, 1, time.Second)
			f.store.put(sessionlog.SessionHeader{ID: "s1", CreatedAt: 7}, item.events...)
...
func TestColdSnapshotDiscardsARecordBoundToAnotherLifetime(t *testing.T) {
	...
	meta := sessionlog.SessionHeader{ID: "s1", CreatedAt: 8}
...
func TestColdSnapshotStillServesWhenTheWriteBackFails(t *testing.T) {
	...
	f.store.put(sessionlog.SessionHeader{ID: "s1", CreatedAt: -1}, userEvent(0), userEvent(1))
...
func (c *Cache) pending(id sessionlog.SessionID) int {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Cache) generation(id sessionlog.SessionID) uint64 {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Cache) tracked(id sessionlog.SessionID) bool {
	c.mu.Lock()
	defer c.mu.Unlock()
