...
// 就是空串，两者本来就是同一件事（见 [sessionlog.SessionHeader.WorkspaceID]）。
type Identity struct {
	...
	WorkspaceID sessionlog.WorkspaceID `json:"workspaceId,omitempty"`
}

// IdentityOf 把一份会话头投影成它的日志身份。
//
// 源: packages/session/session-projection-cache/src/index.ts:289-292
func IdentityOf(header sessionlog.SessionHeader) Identity {
