...
//     [github.com/snight1983/ds-harness-go/harness/session.Session]：本包排出去的每一份字节都要真的过一遍
...
func changeEvent(payload string) sessionlog.Event {
	return sessionlog.Event{Type: EventChange, Data: json.RawMessage(payload)}
}

// userEvent 造一条带来源的用户消息事件；source 为 nil 时是一条普通人类回合。
func userEvent(t *testing.T, source *Source) sessionlog.Event {
	t.Helper()
	data := sessionlog.UserMessageData{}
	if source != nil {
	...
	// （少了它 [github.com/snight1983/ds-harness-go/harness/session.Session.Append] 当场拒收）。只走折叠的
...
func (a *stubAgent) changes() []sessionlog.Event {
	var picked []sessionlog.Event
...
func (a *stubAgent) append(t *testing.T, event sessionlog.Event) sessionlog.Event {
	t.Helper()
...
func (a *stubAgents) drop(id sessionlog.SessionID) {
	a.mutex.Lock()
	defer a.mutex.Unlock()
	delete(a.live, id)
}
