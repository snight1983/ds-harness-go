...
//     [github.com/snight1983/ds-harness-go/feature/goal] 里，不在本包。用替身就等于把那份规则重写一遍，
//     然后测的是副本而不是本包。所以台架接的是真的 [goal.Service]。
...
type testAgent struct {
	...
	//
	// [agent.Inbox] 自己不带锁，真实实现（harness/agentloop 那台）是拿 agent 那把锁
...
func (a *testAgent) ID() sessionlog.SessionID                  { return a.id }
...
func (a *testAgent) Cancel(sessionlog.TurnEndCancelCause, agent.CancelOptions) {
	a.mutex.Lock()
	a.cancels++
	a.mutex.Unlock()
}
...
func (a *testAgent) appendEvent(t *testing.T, event sessionlog.Event) sessionlog.Event {
	t.Helper()
...
// 那一段。
func sessionWithID(t *testing.T, id sessionlog.SessionID) *coresession.Session {
	t.Helper()
	header := sessionlog.SessionHeader{Version: sessionlog.FormatVersion, ID: id, CreatedAt: 2}
...
func newTestAgents() *testAgents {
	return &testAgents{live: map[sessionlog.SessionID]agent.Agent{}}
}

func (a *testAgents) Get(id sessionlog.SessionID) (agent.Agent, bool) {
	a.mutex.Lock()
	defer a.mutex.Unlock()
...
func (a *testAgents) drop(id sessionlog.SessionID) {
	a.mutex.Lock()
	defer a.mutex.Unlock()
	delete(a.live, id)
...
func (s *testSessions) emitEvent(live *coresession.Session, event sessionlog.Event) {
	s.mutex.Lock()
...
func userMessageEvent(t *testing.T, message llm.Message) sessionlog.Event {
	t.Helper()
	encoded, err := json.Marshal(sessionlog.UserMessageData{Message: message})
	if err != nil {
		t.Fatalf("排用户消息事件失败：%v", err)
	}
	return sessionlog.Event{Type: sessionlog.EventUserMessage, Data: encoded, SurfaceOp: sessionlog.AppendOp{}}
}

// turnEndEvent 造一条带这个理由的 turn/end 事件。
func turnEndEvent(t *testing.T, reason sessionlog.TurnEndReason) sessionlog.Event {
	t.Helper()
	encoded, err := json.Marshal(sessionlog.TurnEndData{Turn: 1, Reason: reason})
	if err != nil {
		t.Fatalf("排回合结束事件失败：%v", err)
	}
	return sessionlog.Event{Type: sessionlog.EventTurnEnd, Data: encoded}
}
