...
//     [github.com/snight1983/ds-harness-go/feature/goal] 那条不变量报，本包必须闭嘴——同一件事响两遍会让人
//     去查错的那个包。
...
func goalStream(t *testing.T, maxRounds int) ([]sessionlog.Event, *goal.View) {
	t.Helper()
	live := newHarness(t)
...
func withEvent(events []sessionlog.Event, extra sessionlog.Event) []sessionlog.Event {
	return append(append([]sessionlog.Event(nil), events...), extra)
}
...
func TestValidateStreamRejectsRoundsWithNoGoalAtAll(t *testing.T) {
	...
	stream := []sessionlog.Event{userMessageEvent(t, roundMessage(t, orphan, 1))}
	if err := ValidateStream(stream); err == nil {
...
func TestValidateStreamLetsOtherEventsThrough(t *testing.T) {
	...
	cases := map[string]sessionlog.Event{
		"助手分块":      {Type: sessionlog.EventAssistantChunk, Data: json.RawMessage(`{}`)},
		"人自己发的话":    userMessageEvent(t, plainMessage("do the thing")),
		"读不回来的用户消息": {Type: sessionlog.EventUserMessage, Data: json.RawMessage(`not json`)},
...
func TestValidateStreamDefersUnfoldableStreamsToTheGoalPackage(t *testing.T) {
	// 一条折不动的日志归 [github.com/snight1983/ds-harness-go/feature/goal] 那条不变量报。本包在那一刻收手，
	// 后面那条伪造的续推它一个字都不说——同一件事响两遍会让人去查错的那个包。
	events, view := goalStream(t, 3)
	broken := sessionlog.Event{Type: goal.EventChange, Data: json.RawMessage(`{"version":1}`)}
...
func forgedStream(t *testing.T) []sessionlog.Event {
	t.Helper()
	events, view := goalStream(t, 3)
...
func TestRegisterInvariantsSweepsAlreadyLoadedStreams(t *testing.T) {
	...
	violation := violationOf(t, func() {
		_, _ = RegisterInvariants(t.Context(), registry,
			func() [][]sessionlog.Event { return [][]sessionlog.Event{stream} },
