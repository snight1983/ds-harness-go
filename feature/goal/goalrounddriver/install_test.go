...
func TestDisposedStopsAndForgetsTheDriver(t *testing.T) {
	...
	live.sessions.emitEvent(live.owner.log, turnEndEvent(t, sessionlog.AbortedTurnEnd{Reason: sessionlog.ParentCancel{}}))
}
...
func TestSessionEventsOnlyReachTheOwningDriver(t *testing.T) {
	...
	live.sessions.emitEvent(orphan.log, turnEndEvent(t, sessionlog.AbortedTurnEnd{Reason: sessionlog.ParentCancel{}}))

	// 一个在注册表里但本包还没为它建过驱动的 agent：也不该为一条广播凭空建一台。
	stranger := newTestAgent(t, "session-stranger", live.scope, live.agents)
	live.sessions.emitEvent(stranger.log, turnEndEvent(t, sessionlog.AbortedTurnEnd{Reason: sessionlog.ParentCancel{}}))

	// 自己那一段上的广播才落到驱动身上：一次 max-tokens 把这个目标打回未活化。
	live.createGoal("ship the release", 3)
	live.sessions.emitEvent(live.owner.log, turnEndEvent(t, sessionlog.MaxTokensTurnEnd{}))
	if got := live.currentGoal().Activation; got != goal.Disarmed {
...
func TestSessionEventsAreIgnoredForASupersededSession(t *testing.T) {
	...
	live.sessions.emitEvent(superseded, turnEndEvent(t, sessionlog.MaxTokensTurnEnd{}))
