...
// [github.com/snight1983/ds-harness-go/feature/schedule.Runtime]）。
type driver struct {
	agent agent.Agent
	deps  driverDeps
...
func (d *driver) queueRound(view *goal.View) {
	...
	// [agent.Agent.Followup] 不交回错误（见 harness/agent 那份接口），所以那条捕获
...
// （见 feature/goal 里 parseSourceStrict 那道 `parsed.Round < 1`），拿它来问这个问题
...
//
// 清的只是进程本地那点账。目标本身归 [github.com/snight1983/ds-harness-go/feature/goal] 那台服务管，
...
func (d *driver) onSessionEvent(event sessionlog.Event) {
	switch event.Type {
...
func (d *driver) onUserMessageEvent(event sessionlog.Event) {
...
// 有人按了停，那就交出自动权。
func (d *driver) onTurnEndEvent(event sessionlog.Event) {
	var data sessionlog.TurnEndData
	if err := json.Unmarshal(event.Data, &data); err != nil {
		return
	}
	// 新增: 这一条走不到——[sessionlog.TurnEndData] 那份 UnmarshalJSON 已经把「没带收场
...
// 它们已经从收件箱里被取走了，这个步骤一被拒它们就到此为止（见 harness/agent 里
...
func (d *driver) stop(ctx context.Context) {
	d.stopOnce.Do(func() {
		...
		if attempt != nil && d.agent.Status() == agent.StatusRunning {
			d.agent.Cancel(sessionlog.ParentCancel{}, agent.CancelOptions{})
