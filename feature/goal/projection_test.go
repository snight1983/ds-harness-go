...
func TestApplyProjectionSkipsWhatItCannotUse(t *testing.T) {
	...
	cases := map[string]sessionlog.Event{
		"不是本包的事件":  {Type: sessionlog.EventTurnStart, Data: json.RawMessage(`{}`)},
...
func newFakeSession(events ...sessionlog.Event) *fakeSession {
...
func (s *fakeSession) ID() sessionlog.SessionID   { return "session-1" }
