...
func TestFoldRejectsMalformedGoalSources(t *testing.T) {
	...
	for name, raw := range cases {
		t.Run(name, func(t *testing.T) {
			...
			_, err = Fold([]sessionlog.Event{
				createEvent(),
				{Type: sessionlog.EventUserMessage, Data: encoded},
			})
			expectFoldError(t, err, name)
		})
	}
...
func TestValidateStreamAgreesWithFold(t *testing.T) {
	good := []sessionlog.Event{createEvent()}
	if err := ValidateStream(good); err != nil {
		t.Fatalf("一条合规的流本该过：%v", err)
	}
	bad := []sessionlog.Event{changeEvent(changeJSON("create", goalJSON(2, "active"), 0, 10, 10))}
