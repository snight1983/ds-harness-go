...
//
// 源: packages/jobs/tool-jobs/src/invariant.ts
...
//
// 源: packages/jobs/tool-jobs/src/invariant.ts:18-29
//
// 装进去的检查是**空的**，这是刻意的，不是没写完，DSH 那边把理由写全了：本包
// 只是那台作业注册表面向模型的一层适配，自己不开任何独立的生命周期流；执行之间
// 那些关系归它调的那道能力接缝所有，也由它去验。
//
...
func RegisterInvariants(ctx context.Context, registry *invariants.Registry) (func(), error) {
	if registry == nil {
		return nil, fmt.Errorf("jobstool: 注册不变量需要一个不变量注册表")
	}
