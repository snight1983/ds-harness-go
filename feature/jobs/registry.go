...
// Registry 是后台作业注册表这条缝。进程内那台实现在 github.com/snight1983/ds-harness-go/adapter/localjobs。
//
// 源: packages/jobs/jobs/src/index.ts:62-177
//
// 新增: DSH 是一个抽象类，子类当插件装上去就注册成 ctx.jobs。Go 这边它是一个
// 接口，装配方把实现直接交给需要它的那几个包（成例见
// [github.com/snight1983/ds-harness-go/feature/subagent/reporttool.Service]）。DSH 那道
...
// 本仓库要多副本部署，账本因此在库里（见
// [github.com/snight1983/ds-harness-go/adapter/domainjobs]），一次 List 是一趟
// 数据库往返：会超时、会被取消、会失败。接口上没有 ctx 和 error 就没地方说这件事，
// 而把它藏起来（吞掉错误交一份空列表）等于让「后端坏了」冒充「一件作业都没有」。
//
// 进程内那台实现（[github.com/snight1983/ds-harness-go/adapter/localjobs]）照旧
...
type Registry interface {
	...
	// 释放函数（见 [github.com/snight1983/ds-harness-go/harness/agent.Registry.OnCreated]）。owner 必填，
	// 想罩住每一个属主就交一个 [github.com/snight1983/ds-harness-go/scope.NewRoot] 造的无身份作用域
