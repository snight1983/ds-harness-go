...
func TestLifecycleDispatchHonoursScopeLayering(t *testing.T) {
	...
	var global, scoped []sessionlog.SessionID
	...
	for _, carrier := range []agent.Agent{parent, sibling, child, nil} {
		id := sessionlog.SessionID("nobody")
		if carrier != nil {
			id = carrier.ID()
		}
	...
	wantGlobal := []sessionlog.SessionID{"parent", "sibling", "child", "nobody"}
	if !equalIDs(global, wantGlobal) {
		t.Fatalf("全局层该看见每一次派发，实际 %#v", global)
	}
	// 兄弟那次和没有载体那次都落在 parent 那条父链外面。
	if !equalIDs(scoped, []sessionlog.SessionID{"parent", "child"}) {
...
func TestActivationObserverReadsOnlyItsOwnEpoch(t *testing.T) {
	...
	earlier := append(
		steppedTurn(t, 1, sessionlog.MaxTokensTurnEnd{}),
	...
	for _, event := range append(
		steppedTurn(t, 2, sessionlog.CompletedTurnEnd{}),
...
func TestEpochStopReasonMapsEachTurnEndReason(t *testing.T) {
	cases := map[string]struct {
		...
		"完成":   {sessionlog.CompletedTurnEnd{}, StopCompleted},
		"撞天花板": {sessionlog.MaxTokensTurnEnd{}, StopMaxTokens},
		"被停":   {sessionlog.AbortedTurnEnd{Reason: sessionlog.UserCancel{}}, StopAborted},
		"被打断":  {sessionlog.InterruptedTurnEnd{}, StopAborted},
		"出错":   {sessionlog.ErrorTurnEnd{}, StopError},
		"被拒":   {sessionlog.BlockedTurnEnd{}, StopRefusal},
	}
...
func TestEpochStopReasonReportsWorkDroppedBeforeAnyTurn(t *testing.T) {
	events := []sessionlog.Event{canceledSplice(t)}
...
func TestEpochStopReasonReportsWorkDroppedAfterACleanTurn(t *testing.T) {
	events := append(steppedTurn(t, 1, sessionlog.CompletedTurnEnd{}), canceledSplice(t))
	if got := epochStopReason(events); got != StopAborted {
...
func TestEpochStopReasonKeepsARecordedFailureOverACancellation(t *testing.T) {
	events := append(steppedTurn(t, 1, sessionlog.ErrorTurnEnd{}), canceledSplice(t))
...
func TestEpochStopReasonRejectsAnUnknownReason(t *testing.T) {
	events := append(
		steppedTurn(t, 1, sessionlog.CompletedTurnEnd{})[:2],
		event(t, sessionlog.EventTurnEnd, map[string]any{"turn": 1, "reason": map[string]any{"kind": "未来的变体"}}),
	)
	if got := epochStopReason(events); got != StopError {
...
func TestEpochStopReasonRejectsAnUnreadableTurnEnd(t *testing.T) {
	events := append(
		steppedTurn(t, 1, sessionlog.CompletedTurnEnd{})[:2],
		sessionlog.Event{Type: sessionlog.EventTurnEnd, Data: json.RawMessage(`{"turn":1,"reason":`)},
	)
	if got := epochStopReason(events); got != StopCompleted {
...
func canceledSplice(t *testing.T) sessionlog.Event {
	t.Helper()
...
func appendEvent(t *testing.T, child *fakeAgent, source sessionlog.Event) {
	t.Helper()
...
// equalIDs 比两串会话身份。
func equalIDs(got, wanted []sessionlog.SessionID) bool {
