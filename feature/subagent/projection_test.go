...
// 那个不填时间的 event 帮手。
func timedEvent(t *testing.T, kind sessionlog.EventType, at int64, payload any) sessionlog.Event {
	t.Helper()
...
func turnStartAt(t *testing.T, at int64, turn int) sessionlog.Event {
	t.Helper()
	return timedEvent(t, sessionlog.EventTurnStart, at, sessionlog.TurnStartData{Turn: turn})
}

func turnEndAt(t *testing.T, at int64, turn int) sessionlog.Event {
	t.Helper()
	return timedEvent(t, sessionlog.EventTurnEnd, at, sessionlog.TurnEndData{
		Turn: turn, Reason: sessionlog.CompletedTurnEnd{},
	})
}

// descriptorAt 造一条带时间的描述符事件。
func descriptorAt(t *testing.T, at int64) sessionlog.Event {
	t.Helper()
	return timedEvent(t, EventDescriptor, at, continuableInput())
}

// foldTiming 把这些事件顺次折过计时那个单元，交出上线的那个值。
func foldTiming(t *testing.T, events ...sessionlog.Event) TimingProjection {
	t.Helper()
	state := timingState{}
...
func TestTimingAdvancesTheOpenIntervalWithEveryEvent(t *testing.T) {
	projected := foldTiming(t,
		descriptorAt(t, 10),
		turnStartAt(t, 20, 1),
		timedEvent(t, sessionlog.EventStepStart, 35, sessionlog.StepStartData{Turn: 1, Step: 0}),
	)
...
func TestTimingReportsNoChangeWhenNothingIsOpen(t *testing.T) {
	quiet := []sessionlog.Event{
		timedEvent(t, sessionlog.EventStepStart, 10, sessionlog.StepStartData{Turn: 1, Step: 0}),
		turnEndAt(t, 20, 1),
	}
...
func foldIdentity(t *testing.T, events ...sessionlog.Event) any {
	t.Helper()
	state := identityState{}
	for _, each := range events {
...
func TestIdentityKeepsTheLastDescriptor(t *testing.T) {
	...
	folded := foldIdentity(t,
		...
		sessionlog.Event{Seq: 7, Type: EventDescriptor, Data: data(t, mine)},
	)
...
func TestIdentityClearsWhenTheLastDescriptorDoesNotStand(t *testing.T) {
	...
	for name, broken := range map[string]sessionlog.Event{
...
func occupy(t *testing.T, registry *projection.Registry, key string) {
	...
	if _, err := projection.Register(registry, projection.Definition[struct{}]{
		...
		Apply:        func(state struct{}, _ sessionlog.Event) (struct{}, bool) { return state, false },
...
func TestRegisterProjectionsUnwindsWhenEitherKeyIsTaken(t *testing.T) {
	for name, taken := range map[string]string{
		...
		t.Run(name, func(t *testing.T) {
			...
			live := newFreeSession(t, "child", "parent", []sessionlog.Event{descriptorAt(t, 30)})
...
func TestRegisterProjectionsPublishesAndWithdrawsBothUnits(t *testing.T) {
	...
	live := newFreeSession(t, "child", "parent", []sessionlog.Event{
		turnStartAt(t, 10, 1),
		descriptorAt(t, 30),
		turnEndAt(t, 50, 1),
	})
