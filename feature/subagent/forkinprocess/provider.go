...
// 因为活着的序号就等于数组下标，切出来的结果仍然是一段从 seq 0 起连续的合法种子。
func completedTurnPrefix(parent agent.Agent) []sessionlog.Event {
	...
	for i := len(events) - 1; i >= 0; i-- {
		...
		// 数组，Go 的切片却和 [github.com/snight1983/ds-harness-go/harness/session.Session.Events] 那份
		// 快照共用底层数组——而那份快照会被重复交给别的调用方。不掐容量的话，
		// 拿到这段种子的人一次 append 就会写进快照第 n 格，别人手里那份跟着变。
		end := events[i].Seq + 1
		return events[:end:end]
	}
	return nil
}
...
func (p *Provider) Start(ctx context.Context, request subagent.ResolvedStartRequest) (subagent.Run, error) {
	...
	return inprocessdriver.StartInProcessRun(ctx, p.services, request, inprocessdriver.RunOptions{Seed: seed})
}
...
func (p *Provider) PrepareContinuable(
	...
	return subagent.ContinuableCreateSpec{Seed: completedTurnPrefix(request.Parent)}, nil
}
