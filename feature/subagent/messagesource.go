...
// 成例见 [github.com/snight1983/ds-harness-go/feature/compaction.NewCheckpointSource]。
...
type senderExtra struct {
	SenderSessionID sessionlog.SessionID `json:"senderSessionId"`
}

// marshalSenderExtra 把发送方那一项编成 [github.com/snight1983/ds-harness-go/llm.PluginSource] 的 Extra。
func marshalSenderExtra(senderSessionID sessionlog.SessionID) (json.RawMessage, error) {
	if senderSessionID == "" {
...
func NewCoordinatorSource(senderSessionID sessionlog.SessionID) (llm.PluginSource, error) {
...
func NewReportSource(senderSessionID sessionlog.SessionID) (llm.PluginSource, error) {
	extra, err := marshalSenderExtra(senderSessionID)
	if err != nil {
		return llm.PluginSource{}, err
	}
...
func NewSettledSource(senderSessionID sessionlog.SessionID, summary string) (llm.PluginSource, error) {
	extra, err := marshalSenderExtra(senderSessionID)
	if err != nil {
		return llm.PluginSource{}, err
	}
...
func SenderSessionIDOf(source llm.MessageSource) (sessionlog.SessionID, bool, error) {
