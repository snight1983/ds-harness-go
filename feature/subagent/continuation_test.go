...
func (c *childAgent) Cancel(cause sessionlog.TurnEndCancelCause, options agent.CancelOptions) {
	c.mu.Lock()
...
type fakeFactory struct {
	...
	onPublished func(sessionlog.SessionID)
}
...
func (f *fakeFactory) child(id sessionlog.SessionID) *childAgent {
	f.mu.Lock()
	defer f.mu.Unlock()
...
type continuationFixture struct {
	...
	retires map[sessionlog.SessionID]func()
}
...
// 游离的假 agent。
func (f *continuationFixture) spawnParent(t *testing.T, id sessionlog.SessionID, parent sessionlog.SessionID) *childAgent {
	t.Helper()
...
func (f *continuationFixture) retire(t *testing.T, id sessionlog.SessionID) {
	t.Helper()
...
func (f *continuationFixture) livingActivation(t *testing.T, childID sessionlog.SessionID) *activation {
	t.Helper()
...
func (f *continuationFixture) resident(childID sessionlog.SessionID) bool {
	f.manager.mutex.Lock()
	defer f.manager.mutex.Unlock()
