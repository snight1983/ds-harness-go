...
type continuationHost interface {
	...
	observeActivation(provider string, childID sessionlog.SessionID, parent agent.Agent) *activationObserver
}
...
func settlementSummary(childID sessionlog.SessionID, stopReason StopReason) string {
...
type ContinuationManager struct {
	...
	activations map[sessionlog.SessionID]*activation
