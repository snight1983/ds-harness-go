...
//
// 源: packages/subagent/tool-subagent-report/src/invariant.ts
...
//
// 源: packages/subagent/tool-subagent-report/src/invariant.ts:17-29
//
// 装进去的检查是**空的**，这是刻意的，不是没写完：本包只是那台子 agent 运行时
// 面向模型的一层适配，自己不开任何独立的生命周期流；「谁有资格投」和「投出去的
// 那条消息和哪个父结成什么关系」这两件事都归它调的那台服务所有，也由它去验。
//
// 那为什么还要登记？理由和 [github.com/snight1983/ds-harness-go/feature/skill/skilltool.RegisterInvariants]
...
func RegisterInvariants(ctx context.Context, registry *invariants.Registry) (func(), error) {
	if registry == nil {
		return nil, fmt.Errorf("reporttool: 注册不变量需要一个不变量注册表")
	}
