...
// # 不做什么
//
//   - **不创建孩子。**派发归 [github.com/snight1983/ds-harness-go/feature/subagent/subagenttool]；
//     这三件工具只作用在已经存在的那些孩子上。
//   - **不决定谁看得见谁。**作用域过滤和父子归属由
//     [github.com/snight1983/ds-harness-go/feature/subagent] 的查询回答，模型报一个 id 不能凭它越过
//     这条边界。
//   - **不定义打断的语义。**interrupt_agent 递的是一个请求；真正停不停、什么时候停，
//     由那个孩子自己那台 agent 的静止判断说了算。
//   - **不写日志、不建投影。**它只读服务交回来的行，再排成模型看得懂的样子。
package controltool
