...
func (s *stubService) Interrupt(target sessionlog.SessionID, authority subagent.InterruptAuthority) error {
	s.interrupts = append(s.interrupts, interruptCall{target: target, authority: authority})
	return s.interruptErr
}

func (s *stubService) ListChildren(context.Context, sessionlog.SessionID) ([]subagent.ListEntry, error) {
...
// childRow 造一行可续孩子。
func childRow(id sessionlog.SessionID, label string) subagent.ListEntry {
