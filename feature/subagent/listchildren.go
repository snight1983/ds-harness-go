...
//
// 只有持久头上 Origin 是 [github.com/snight1983/ds-harness-go/sessionlog.OriginSubagent] 的候选才会被解读。
...
type listing struct {
	...
	corpus          map[sessionlog.SessionID]corpusRecord
	subagentParents map[sessionlog.SessionID]struct{}
}
...
func ListChildren(
	ctx context.Context,
	services ListingServices,
	parentSessionID sessionlog.SessionID,
) ([]ListEntry, error) {
	...
	for _, record := range prepared.corpus {
		if record.header.ParentSession == parentSessionID &&
			record.header.Origin == sessionlog.OriginSubagent {
...
func ListDescendants(
	ctx context.Context,
	services ListingServices,
	rootSessionID sessionlog.SessionID,
...
func prepareListing(ctx context.Context, services ListingServices) (*listing, error) {
	...
	parents := map[sessionlog.SessionID]struct{}{}
	for _, record := range corpus {
		if record.header.Origin == sessionlog.OriginSubagent && record.header.ParentSession != "" {
...
func descendantCandidates(
	...
	children := map[sessionlog.SessionID][]corpusRecord{}
	for _, record := range corpus {
	...
	pushLevel := func(parentID sessionlog.SessionID, depth int) {
	...
	visited := map[sessionlog.SessionID]struct{}{rootSessionID: {}}
	...
	for len(stack) > 0 {
		...
		visited[id] = struct{}{}
		if position.record.header.Origin == sessionlog.OriginSubagent {
...
func (l *listing) resolveColdIdentity(
	...
	restored, err := l.services.Projections.Restore(
		nil, inspected.Events, 0, sessionlog.LogBaseSeq(inspected.Events))
	if err != nil {
		return diagnosticRow(childID, DiagnosticCorrupt), nil
	}
...
// hasChildren 表示有没有一个直接后代的持久 Origin 是 subagent。
func (l *listing) hasChildren(id sessionlog.SessionID) bool {
...
func diagnosticRow(id sessionlog.SessionID, reason DiagnosticReason) ListEntry {
...
func sameLifecycle(meta, expected sessionlog.SessionHeader) bool {
