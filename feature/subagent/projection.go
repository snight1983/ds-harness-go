...
// 的函数（成例见 [github.com/snight1983/ds-harness-go/feature/plan/planmode.RegisterProjection]）。
//
...
// 那一次清零就是这个孩子权威的计时原点。
func applyTiming(state timingState, event sessionlog.Event) (timingState, bool) {
	switch event.Type {
...
func applyIdentity(state identityState, event sessionlog.Event) (identityState, bool) {
	if event.Type != EventDescriptor {
		return state, false
	}
	// 一次折叠绝不许报错，所以坏掉的负载在这里折成「没有身份」。
	descriptor, found, err := FoldDescriptor([]sessionlog.Event{event})
	if err != nil || !found {
