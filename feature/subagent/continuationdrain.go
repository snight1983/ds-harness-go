...
// 直接当 [github.com/snight1983/ds-harness-go/scope.Scope.Defer] 的清理交出去，见
...
func (m *ContinuationManager) Drain(ctx context.Context) error {
	...
	// 会孩子优先地递归进整片森林。
	m.mutex.Lock()
	owned := map[sessionlog.SessionID]struct{}{}
...
func (m *ContinuationManager) DrainDescendants(ctx context.Context, parents []agent.Agent) error {
	...
	ownedTargets := map[sessionlog.SessionID]struct{}{}
...
func (m *ContinuationManager) liveLineage(from agent.Agent) []agent.Agent {
	...
	seen := map[sessionlog.SessionID]struct{}{from.ID(): {}}
...
//
// 光看 [github.com/snight1983/ds-harness-go/harness/agent.Agent.Status] 不够：在「一次唤醒投递被接受」和
