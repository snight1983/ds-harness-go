...
// testWorkspaceID 是这些用例里会话头上那个归属工作区。
//
// 新增: 它是一个不透明标识，不是路径，也和文件系统里有什么东西没有关系，
// 见 [sessionlog.SessionHeader.WorkspaceID]。
var testWorkspaceID = sessionlog.WorkspaceID("ws-subagent")
...
func turnEnd(t *testing.T, turn int, reason sessionlog.TurnEndReason) sessionlog.Event {
	t.Helper()
	return event(t, sessionlog.EventTurnEnd, sessionlog.TurnEndData{Turn: turn, Reason: reason})
}
...
func newFreeSession(t *testing.T, id sessionlog.SessionID, parent sessionlog.SessionID, seed []sessionlog.Event) *coresession.Session {
	t.Helper()
...
func newFakeAgent(t *testing.T, id string, parentScope *scope.Key, parentSession sessionlog.SessionID) *fakeAgent {
	t.Helper()
...
func (r *fakeRun) ID() sessionlog.SessionID { return r.id }
