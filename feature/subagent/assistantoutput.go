...
// [github.com/snight1983/ds-harness-go/sessionlog.Event.Data] 是原样保管的 JSON，所以先解一次；解不出来
// （日志坏了）在这里当**没贡献**处理而不是报错——这条折叠只负责选出一份输出，
// 判日志成不成立是 [github.com/snight1983/ds-harness-go/sessionlog] 那道边界的事，在这里报错只会让一次
// 本来能收场的运行多一种收不了场的方式。
func (f *AssistantOutputFold) Push(event sessionlog.Event) {
	switch event.Type {
	...
	case sessionlog.EventAssistantChunk:
		...
		chunk, ok := data.(sessionlog.AssistantChunkData)
		if !ok {
...
// 一份都选不出来时交回 nil。
func FinalAssistantOutput(events []sessionlog.Event) llm.Content {
