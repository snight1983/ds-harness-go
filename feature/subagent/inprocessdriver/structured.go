...
type StructuredAttachment struct {
	...
	// 办法。Go 的 github.com/snight1983/ds-harness-go/tools.ExecutionToken 本身就是可比较、可当 map
	...
	// 这件事（理由和 github.com/snight1983/ds-harness-go/harness/agent.ConsumedWork.HasEnd 逐字相同）。
	hasCaptured bool
}
...
// github.com/snight1983/ds-harness-go/tools.AssertObjectSchema）。
//
// 新增: DSH 那四笔登记都是 void，撤销靠 cordis 处置孩子那个上下文。Go 这边它们
// 各自交回一个撤销函数，而这里**有意**把它们丢掉：owner 就是孩子自己那个作用域，
// 作用域一处置这四笔就跟着没了（成例见
// github.com/snight1983/ds-harness-go/feature/subagent.ApplyChildComposition）。
func AttachStructuredRuntime(
	...
	// 校验包装。Go 的 github.com/snight1983/ds-harness-go/tools.Runtime 在进执行体之前统一验过
