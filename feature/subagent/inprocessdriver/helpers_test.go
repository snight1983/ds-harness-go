...
var testWorkspaceID = sessionlog.WorkspaceID("ws-inprocessdriver")
...
// event 造一条会话事件。
func event(t *testing.T, kind sessionlog.EventType, payload any) sessionlog.Event {
	t.Helper()
	return sessionlog.Event{Type: kind, Data: data(t, payload)}
}

// steppedTurn 造一个「进过模型步骤、然后按 reason 收尾」的完整回合。
//
// [github.com/snight1983/ds-harness-go/harness/agent.FoldConsumedWork] 只把这样的回合认成「交代得了消耗」，
// 所以本包每一处要 HasEnd 为真的用例都从这里取事件。
func steppedTurn(t *testing.T, turn int, reason sessionlog.TurnEndReason) []sessionlog.Event {
	t.Helper()
	return []sessionlog.Event{
		event(t, sessionlog.EventTurnStart, sessionlog.TurnStartData{Turn: turn}),
		event(t, sessionlog.EventStepStart, sessionlog.StepStartData{Turn: turn, Step: 0}),
		event(t, sessionlog.EventTurnEnd, sessionlog.TurnEndData{Turn: turn, Reason: reason}),
	}
}

// assistantMessage 造一条带内容的 assistant/message 事件。
//
// 带上 AppendOp 是必须的：这条事件够格上表面，[sessionlog.SurfaceOpOf] 要求这种事件
// 一定带着自己的表面操作。
func assistantMessage(t *testing.T, turn int, content llm.Content) sessionlog.Event {
	t.Helper()
	built := event(t, sessionlog.EventAssistantMessage, sessionlog.AssistantMessageData{
		Turn:    turn,
...
func newFreeSession(t *testing.T, id sessionlog.SessionID) *coresession.Session {
	t.Helper()
...
// fakeAgent 是一个只为满足 [github.com/snight1983/ds-harness-go/harness/agent.Agent] 契约而存在的假 agent。
...
func newChildAgent(id sessionlog.SessionID, agentScope *scope.Scope, live *coresession.Session) *childAgent {
	return &childAgent{
...
func (c *childAgent) Cancel(cause sessionlog.TurnEndCancelCause, _ agent.CancelOptions) {
	c.mutex.Lock()
...
// cancelled 交出历次取消的理由。
func (c *childAgent) cancelled() []sessionlog.TurnEndCancelCause {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	return append([]sessionlog.TurnEndCancelCause(nil), c.cancels...)
}

// appendAll 把这些事件挨个追加到孩子自己的日志上。
func (c *childAgent) appendAll(t *testing.T, events []sessionlog.Event) {
	t.Helper()
...
type fakeFactory struct {
	...
	children map[sessionlog.SessionID]*childAgent
...
func completedTurn(t *testing.T, answer string) func(*childAgent) {
	...
	return func(child *childAgent) {
		child.appendAll(t, []sessionlog.Event{
			event(t, sessionlog.EventTurnStart, sessionlog.TurnStartData{Turn: 0}),
			event(t, sessionlog.EventStepStart, sessionlog.StepStartData{Turn: 0, Step: 0}),
			assistantMessage(t, 0, textContent(answer)),
			event(t, sessionlog.EventTurnEnd, sessionlog.TurnEndData{
				Turn: 0, Reason: sessionlog.CompletedTurnEnd{},
			}),
		})
		child.settle()
	}
