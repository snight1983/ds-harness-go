...
// [github.com/snight1983/ds-harness-go/scope.Layers]——全局层加各作用域的覆盖层，派发时按载体作用域的
// 父链取并集（成例见 [github.com/snight1983/ds-harness-go/harness/agent] 那十二个事件）。
...
func newActivationObserver(emitter *lifecycleEmitter, provider string, childID sessionlog.SessionID, parent agent.Agent) *activationObserver {
...
//
// [github.com/snight1983/ds-harness-go/harness/agent.FoldConsumedWork] 补上光看回合序列拿不到的那两半：
// 哪个回合交代得了这一轮消耗掉的活儿，以及被接受的活儿有没有在那之后被取消、
// 且中间没有任何一个回合开在它上面。一次**记下来的失败**仍旧压过一次取消——
// 停掉一个本来就已经失败的孩子，不会把它的失败变成一次取消。
func epochStopReason(events []sessionlog.Event) StopReason {
	...
	case sessionlog.ReasonAborted, sessionlog.ReasonInterrupted:
		return StopAborted
	case sessionlog.ReasonError:
		return StopError
