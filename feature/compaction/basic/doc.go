...
// # 不做什么
//
//   - **不定义压缩在日志上长什么样。**四条事件的词汇、那枚检查点标记、下刀处的配平
//     和那几条不变量都在 [github.com/snight1983/ds-harness-go/feature/compaction]，
//     本包只是照着那套形状写。
//   - **不裁剪单条工具结果。**一次 grep 打回来几万行属于
//     [github.com/snight1983/ds-harness-go/feature/compaction/toolresultpruner]：那条路确定、不花钱，
//     而本包这条一定要发一次模型请求。两个后端互补，不是二选一。
//   - **不连提供方，也不自己数 token。**[Streamer]、[Meter] 和 [Summarize] 都是收窄到
//     单方法的接缝，谁去说话、谁去计量由装配方填；写窄的后果是本包在类型上就发不出
//     别的模型请求。
//   - **不保证摘要事实正确。**摘要模型、提示词和那个可换的摘要钩子都是部署选的，
//     本包只保证这一刀落得下去、落完之后表面仍然自洽。
//   - **不回滚一次做砸的压缩。**中途失败只做一次合括号的尝试，已经写进日志的那几条
//     原样留着——一次失败的压缩本身就该在日志里看得见。
//   - **不替装配方接线。**[Engine] 自己不带可变状态，那几条按压力和超窗触发的观察者
//     由 [Install] 单独挂；只想手动调 [Engine.CompactNow] 的装配不必先备一张 agent 注册表。
package basic
