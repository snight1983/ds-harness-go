...
func TestOnRequestError已经落地的缩减之后总结才失败也重试(t *testing.T) {
	...
	deps.Prune = func(live *coresession.Session) error {
		...
		_, err := live.Append(sessionlog.Event{
			Type:            sessionlog.EventUserMessage,
			Data:            marshalPayload(t, sessionlog.UserMessageData{Message: llm.NewUserMessage(llm.Content{llm.TextBlock{Text: "砍过了"}}, llm.UserSource{})}),
			SurfaceOp:       sessionlog.ReplaceOp{Start: log.seqs[0], End: log.seqs[0]},
...
func TestOnRequestError那串计数在三处归零(t *testing.T) {
	...
	for name, reset := range map[string]func(setup installed, live *stubAgent){
		...
		"助手消息落地": func(setup installed, live *stubAgent) {
			setup.sessions.event(live.Session(), sessionlog.Event{
...
func TestOnStatus和会话事件只认那一种(t *testing.T) {
	...
	setup.sessions.event(live.Session(), sessionlog.Event{Type: sessionlog.EventUserMessage})
