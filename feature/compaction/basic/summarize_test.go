...
var summarizeWorkspaceID = sessionlog.WorkspaceID("ws-compaction-basic")
...
func liveSession(t *testing.T, id string, headers ...llm.CallConfig) *coresession.Session {
	...
	sid := sessionlog.SessionID(id)
	live, err := coresession.NewSession(sid, coresession.Options{
		Header: &sessionlog.SessionHeader{ID: sid, WorkspaceID: summarizeWorkspaceID},
	})
	if err != nil {
