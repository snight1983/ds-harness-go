...
func appendTo(t *testing.T, live *coresession.Session, event sessionlog.Event) int {
	t.Helper()
...
func surfaceMessage(t *testing.T, kind sessionlog.EventType, payload any) sessionlog.Event {
	t.Helper()
...
func openLog(t *testing.T, headers ...llm.CallConfig) logSession {
	...
	for index := range 4 {
		if index%2 == 0 {
			seqs = append(seqs, appendTo(t, live, surfaceMessage(t, sessionlog.EventUserMessage,
				sessionlog.UserMessageData{Message: llm.NewUserMessage(
					llm.Content{llm.TextBlock{Text: "第几句"}}, llm.UserSource{})})))
			continue
		}
		seqs = append(seqs, appendTo(t, live, surfaceMessage(t, sessionlog.EventAssistantMessage,
			sessionlog.AssistantMessageData{
				Turn: 1, Step: index,
...
func eventsByType(events []sessionlog.Event, kind sessionlog.EventType) (int, sessionlog.Event) {
	count := 0
	var last sessionlog.Event
...
func TestCompactSurfaceRegion把四样东西按顺序落下去(t *testing.T) {
	...
	wantOrder := []sessionlog.EventType{
	...
	if op, ok := tail[2].SurfaceOp.(sessionlog.ReplaceOp); !ok ||
...
func TestCompactSurfaceRegion整表面那一档不许别处也变(t *testing.T) {
	...
	meter.before = func() {
		meter.before = func() {
			appendTo(t, log.live, surfaceMessage(t, sessionlog.EventUserMessage,
				sessionlog.UserMessageData{Message: llm.NewUserMessage(
		...
		"那一段本身被换掉了": func(meter *meteredSurface, log logSession) func() {
			...
			return func() {
				appendTo(t, log.live, sessionlog.Event{
					...
					Data: marshalPayload(t, sessionlog.UserMessageData{
						Message: llm.NewUserMessage(
							llm.Content{llm.TextBlock{Text: "别人先压了"}}, llm.UserSource{})}),
					SurfaceOp:       sessionlog.ReplaceOp{Start: log.seqs[0], End: log.seqs[1]},
...
func TestCompactSurfaceRegion重放的输入就是那一段自己(t *testing.T) {
	...
	appendTo(t, log.live, sessionlog.Event{
		...
		Data: marshalPayload(t, sessionlog.RequestHeaderData{
...
func toolLog(t *testing.T) logSession {
	...
	seqs := []int{
		appendTo(t, live, surfaceMessage(t, sessionlog.EventUserMessage,
			sessionlog.UserMessageData{Message: llm.NewUserMessage(
				llm.Content{llm.TextBlock{Text: "查一下"}}, llm.UserSource{})})),
		appendTo(t, live, surfaceMessage(t, sessionlog.EventAssistantMessage,
			sessionlog.AssistantMessageData{
				Turn: 1, Step: 1,
		...
		appendTo(t, live, surfaceMessage(t, sessionlog.EventToolResult,
			sessionlog.ToolResultData{
				Turn: 1, Step: 1,
				Message: llm.Message{
					ID:      "t",
					Role:    llm.RoleUser,
...
func replaceNodes(t *testing.T, shadowed ...int) sessionlog.Event {
	t.Helper()

	return sessionlog.Event{
		Type: sessionlog.EventUserMessage,
		Data: marshalPayload(t, sessionlog.UserMessageData{Message: llm.NewUserMessage(
			llm.Content{llm.TextBlock{Text: "别人下的刀"}}, llm.UserSource{})}),
		SurfaceOp:       sessionlog.ReplaceOp{Start: shadowed[0], End: shadowed[len(shadowed)-1]},
...
func TestCompactSurfaceRegion表面本身坏了就原样报出来(t *testing.T) {
	...
	orphan := appendTo(t, live, surfaceMessage(t, sessionlog.EventToolResult,
		sessionlog.ToolResultData{
			Turn: 1, Step: 1,
			Message: llm.Message{
				ID:      "t",
...
func TestCompactSurfaceRegion被遮的那一段读不回来就算失败(t *testing.T) {
	...
	broken := appendTo(t, live, sessionlog.Event{
		Type:      sessionlog.EventUserMessage,
		Data:      json.RawMessage("[1,2]"),
		SurfaceOp: sessionlog.AppendOp{},
	})
	tail := appendTo(t, live, surfaceMessage(t, sessionlog.EventAssistantMessage,
		sessionlog.AssistantMessageData{
			Turn: 1, Step: 1,
...
func TestCompactSurfaceRegion在一次事件通告里开不了工(t *testing.T) {
	...
	head := appendTo(t, live, surfaceMessage(t, sessionlog.EventUserMessage,
		sessionlog.UserMessageData{Message: llm.NewUserMessage(
