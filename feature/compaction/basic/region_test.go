...
func TestSelectCompactableRange配平算不出来时把错误交出去(t *testing.T) {
	...
	// 不能当成「这一刀不配平」继续往前退。
	broken := eventAt(2, sessionlog.EventAssistantMessage, json.RawMessage(`{"turn":"一"}`))
...
func TestInspectEntryState读出三件事(t *testing.T) {
	...
	state, err := InspectEntryState([]sessionlog.Event{
		endSeed(1),
...
func TestInspectEntryState三样都定下来就提前收工(t *testing.T) {
	...
	broken := logEventAt(1, sessionlog.EventTurnStart, json.RawMessage(`{"turn":"一"}`))
	state, err := InspectEntryState([]sessionlog.Event{
		broken,
		endSeed(2),
		compactionEnd(t, 3, 1),
...
func TestInspectEntryState回合号读不回来就报(t *testing.T) {
	...
	broken := logEventAt(1, sessionlog.EventTurnStart, json.RawMessage(`{"turn":"一"}`))
	if _, err := InspectEntryState([]sessionlog.Event{broken}); !errors.Is(err, compaction.ErrMalformedEvent) {
		t.Fatalf("报的是 %v", err)
	}
}
...
func TestCheckNoActiveCompaction(t *testing.T) {
	...
	for name, item := range map[string]struct {
		...
		"没有开着的括号": {[]sessionlog.Event{turnStart(t, 1, 1)}, false},
		"括号还开着":   {[]sessionlog.Event{compactionStart(t, 1, 1)}, true},
		"括号已经配对":  {[]sessionlog.Event{compactionStart(t, 1, 1), compactionEnd(t, 2, 1)}, false},
		// 种子边界之前的日志是继承来的，那边的括号在这一侧永远等不到它的 end，
		// 不该把当前这个会话一直锁着。
		"开着的括号在种子边界之前": {[]sessionlog.Event{compactionStart(t, 1, 1), endSeed(2)}, false},
		"开着的括号在种子边界之后": {[]sessionlog.Event{endSeed(1), compactionStart(t, 2, 1)}, true},
	} {
		t.Run(name, func(t *testing.T) {
			t.Parallel()
...
func TestCheckNoActiveCompaction日志读不回来时把错误交出去(t *testing.T) {
	...
	broken := logEventAt(1, sessionlog.EventTurnStart, json.RawMessage(`{"turn":"一"}`))
	if err := CheckNoActiveCompaction([]sessionlog.Event{broken}, "开工前"); !errors.Is(
