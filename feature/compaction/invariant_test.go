...
func TestValidateLog一次完整的回合内压缩(t *testing.T) {
	...
	trace, err := ValidateLog([]sessionlog.Event{
		turnStart(t, 0, 1),
		userText(t, 1, "你好"),
...
func TestValidateLog一次独立压缩(t *testing.T) {
	...
	_, err := ValidateLog([]sessionlog.Event{
		turnStart(t, 0, 1),
		userText(t, 1, "你好"),
		turnEnd(2),
...
func TestValidateLog回合边界不许跨过开着的压缩(t *testing.T) {
	...
	for name, closer := range map[string]sessionlog.Event{
...
func TestValidateSummary的几种违规(t *testing.T) {
	...
	opened := []sessionlog.Event{
		turnStart(t, 0, 1),
		compactionStart(t, 1, StartData{CompactionID: "c-1", SourceCommandID: "cmd-1", Turn: 1}),
	}
	withOpen := func(events ...sessionlog.Event) []sessionlog.Event {
		return append(append([]sessionlog.Event{}, opened...), events...)
	}
	...
	for name, item := range map[string]struct {
		...
		"没有配对的 start": {
			[]sessionlog.Event{compactionSummary(t, 0, summaryOf("c-1", 4))},
...
func TestValidateEnd的几种违规(t *testing.T) {
	...
	for name, item := range map[string]struct {
		...
		"没有配对的 start": {
			[]sessionlog.Event{compactionEnd(t, 0, EndData{CompactionID: "c-1", Standalone: true})},
			"没有配对的 compaction/start",
		},
		"身份是空的": {
...
func TestValidateCheckpoint的几种违规(t *testing.T) {
	...
	withOpen := func(event sessionlog.Event) []sessionlog.Event {
		return append(append([]sessionlog.Event{}, opened...), event)
	}
	...
	for name, item := range map[string]struct {
		...
		"用户消息负载读不回来": {
			func() []sessionlog.Event {
				broken := eventAt(3, sessionlog.EventUserMessage, json.RawMessage(`{"message":42}`))
				broken.SurfaceOp = sessionlog.ReplaceOp{Start: 1, End: 1}
...
func TestValidateCheckpoint追加进来的用户消息不查(t *testing.T) {
	...
	appended := eventAt(3, sessionlog.EventUserMessage, marshalPayload(t, sessionlog.UserMessageData{
		Message: llm.Message{
			ID:      "x",
			Role:    llm.RoleUser,
	...
	if _, err := ValidateLog([]sessionlog.Event{
		turnStart(t, 0, 1),
		userText(t, 1, "你好"),
		compactionStart(t, 2, StartData{CompactionID: "c-1", Turn: 1}),
		appended,
...
func TestValidate不认识的事件原样放过(t *testing.T) {
	...
	var trace Trace
	if _, err := trace.Validate(logEventAt(0, sessionlog.EventRequestHeader, json.RawMessage(`{}`))); err != nil {
		t.Fatalf("报了：%v", err)
	}
}
...
func TestValidate回合边界的负载读不回来(t *testing.T) {
	...
	var trace Trace
	broken := logEventAt(0, sessionlog.EventTurnStart, json.RawMessage(`{"turn":"第一回合"}`))
