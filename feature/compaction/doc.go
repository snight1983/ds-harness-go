...
//     Go 这边是嵌 [AgentContext] 加一个单方法接口字段，签名和 harness/agent 里
//     Agent.RunMaintenance 逐字相同——于是一个真的 agent 结构上就满足它，
//     装配方直接填进去，不用现包一层适配。少掉的是 DSH 那个 `<T>` 泛型，
//     取舍和 harness/agent 那一处相同：要产出的调用方自己在闭包里接住。
//
//  13. **不变量那一侧的 `apply` 整条 SKIP。**它只是往注册表上挂一个安装器，
//     核心的 [ValidateLog] 在这里写完了——成例是 harness/session 的 session.Trace。
//
// # 覆盖率为什么到不了 99%
//
...
// # 不做什么
//
//   - **不压缩。**挑哪一段、发不发模型请求、什么时候动手，全归后端
//     （[github.com/snight1983/ds-harness-go/feature/compaction/basic]、
//     [github.com/snight1983/ds-harness-go/feature/compaction/toolresultpruner]）和装配层。
//     这里只写下「写进日志的东西必须长成什么样」和「什么形状才叫一个压缩后端」。
//   - **不删日志。**四条事件全是追加，被遮住的那段历史原样留在会话日志里；改掉的只是
//     模型看得见的那层表面。这不是归档，也不是数据清理。
//   - **不判断摘要写得对不对。**摘要正文是模型产出的，本包只验它带的那几个字段自洽——
//     范围、序号和估价对不对得上。事实准不准归选摘要模型和提示词的那一方。
//   - **不管日志怎么落盘。**事件追加进会话日志就完事，耐久介质归
//     [github.com/snight1983/ds-harness-go/feature/persistence]。
//   - **不替装配方登记词汇。**[EventTypes] 交出去，拼进 [session.Vocabulary] 是装配方的动作——
//     Go 没有声明合并，漏拼的后果是整段带压缩的日志被拒掉。
package compaction
