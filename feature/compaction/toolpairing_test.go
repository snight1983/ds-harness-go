...
func TestBalance代数变了就整个重建(t *testing.T) {
	...
	after := SurfaceView{
		Nodes:      []int{3, 2},
		Generation: 2,
		Events:     []sessionlog.Event{userText(t, 2, "继续"), assistantCalls(t, 3, 1)},
		BaseSeq:    2,
	}
...
func TestBalance表面上的seq在日志里找不到(t *testing.T) {
	...
	for name, view := range map[string]SurfaceView{
		"基准之前": {
			...
			Events: []sessionlog.Event{userText(t, 5, "只拿到一段后缀")}, BaseSeq: 5,
		},
		"越过尾巴": {
			Nodes: []int{0, 1}, Generation: 1,
			Events: []sessionlog.Event{userText(t, 0, "只有一条")}, BaseSeq: 0,
		},
		"下标对上了但 seq 不对": {
			Nodes: []int{0}, Generation: 1,
			Events: []sessionlog.Event{userText(t, 3, "错位")}, BaseSeq: 0,
		},
	} {
...
func TestBalance坏掉的追加不会留下折了一半的状态(t *testing.T) {
	...
	broken := viewOf(1,
		...
		logEventAt(2, sessionlog.EventAssistantMessage, json.RawMessage(`{`)),
	)
...
func TestEventDelta助手消息读不回来(t *testing.T) {
	...
	event := eventAt(0, sessionlog.EventAssistantMessage, json.RawMessage(`{"message":42}`))
...
func TestBalance非工具内容块不算数(t *testing.T) {
	...
	event := eventAt(0, sessionlog.EventAssistantMessage, marshalPayload(t, sessionlog.AssistantMessageData{
		Turn: 1, Step: 1,
		Message: llm.Message{
			ID:   "a",
