...
type SurfaceView struct {
	// Nodes 是当前表面上的节点 seq，按表面顺序。取自 [sessionlog.SurfaceFolder.Nodes]。
	Nodes []int
	// Generation 是表面的改写代数。取自 [sessionlog.SurfaceFolder.ReplaceGeneration]。
	//
...
func (v SurfaceView) eventForSeq(seq int) (sessionlog.Event, error) {
	index := seq - v.BaseSeq
	if index < 0 || index >= len(v.Events) || v.Events[index].Seq != seq {
		return sessionlog.Event{}, fmt.Errorf("%w：表面上的 seq %d 在日志里找不到对应的事件",
...
func eventDelta(event sessionlog.Event) (int, error) {
	switch event.Type {
	case sessionlog.EventAssistantMessage:
		...
		message, ok := data.(sessionlog.AssistantMessageData)
		if !ok {
			// 不可达：[sessionlog.DecodeData] 按 Type 分发，assistant/message 只会
