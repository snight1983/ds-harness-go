...
// # 不做什么
//
//   - **停不下任何东西。**Go 里没法从外面掐死一段正在跑的代码。一个不理会 ctx 的工具
//     装了这一层之后仍然会跑到底，变的只是模型收到的那份结果。
//   - **不替工具声明预算。**期限来自
//     [github.com/snight1983/ds-harness-go/tools.Definition.Timeout]；工具写下它，
//     就是在断言自己会把 ctx 转发给一个取消时收得住的实现。没写的工具这一层不管。
//   - **不接管上游取消。**只有 [github.com/snight1983/ds-harness-go/feature/timeout.OfContext]
//     认出这条期限是本包挂的那一条时才换结果；外层更早到期的期限原样往上传——
//     超时和用户按停止在模型那边该有完全不同的反应。
//   - **不做进程隔离，也不回收资源。**换掉结果之后，那段代码占着的连接、文件句柄和
//     内存还占着，谁开的谁收。
//   - **不重试。**要不要换个更小的输入再来一次，是读到那份超时结果的模型的事。
