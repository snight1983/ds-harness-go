...
// [github.com/snight1983/ds-harness-go/tools.Definition.Timeout] 上写下预算，就是在断言它会把 ctx
// 转发给一个取消时收得住的实现。
//
...
// 结束了」，而是 [github.com/snight1983/ds-harness-go/feature/timeout.OfContext] 认出这条期限**是本包挂的那一条**：
// 外层还有一个更早到期的期限时，那是上游取消，原样往上传。
//
// # 落在哪条缝上
//
// DSH 把它挂在 cordis 的 `tools/execute` 事件上。Go 这边对应的是
// [github.com/snight1983/ds-harness-go/tools.Runtime.AroundDispatch] 那条绕派发瀑布——同一个位置：
