...
// # 不做什么
//
//   - **不碰宿主机文件系统。**只走 [fs.FileSystem]，没有 DSH 那条退回 `node:fs` 的路，
//     也没有「当前工作目录」这个概念——路径说的是执行世界里的位置，而服务端没有硬盘。
//   - **不替宿主判断哪些指令文件可信。**读到什么就渲染什么，能不能读、准不准读，
//     由 [fs] 那条缝背后的后端和它的策略说了算。
//   - **不知道工作区根在哪。**会话头记的是一个不透明的工作区标识，
//     把它换成一条根路径归装配方（[Deps.WorkspaceRoot]）。
//   - **不自己调度。**什么时候对账、结果挂到哪条消息上、谁来触发，全在 [Install]
//     挂的那几个观察者里；不装它，上面那三件纯函数照样能单独调。
//   - **不做长期记忆。**每次都从介质上重读并现场对账，不维护向量索引，也不建全文检索。
