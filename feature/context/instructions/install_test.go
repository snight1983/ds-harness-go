...
// 是一个不透明的工作区标识（见 [sessionlog.SessionHeader.WorkspaceID]），从标识换到根路径
...
var (
	...
	testWorkspaceID = sessionlog.WorkspaceID("ws-instructions")
...
// 别的标识一律答「不属于任何工作区」。
func rootedAt(root string) func(context.Context, sessionlog.WorkspaceID) (string, bool, error) {
	return func(_ context.Context, id sessionlog.WorkspaceID) (string, bool, error) {
...
//
// 三条改动方法各自走一遍这把锁，和 [github.com/snight1983/ds-harness-go/harness/agentloop.ReactLoopAgent]
...
func newWorldRooted(
	t *testing.T,
	config Config,
	rootOf func(context.Context, sessionlog.WorkspaceID) (string, bool, error),
) *world {
	t.Helper()
...
// append 往这段会话里写一条事件。
func (w *world) append(kind sessionlog.EventType, payload any, surface sessionlog.SurfaceOp) sessionlog.Event {
	w.t.Helper()
	...
	written, err := w.live.sess.Append(sessionlog.Event{Type: kind, Data: data, SurfaceOp: surface})
	if err != nil {
...
func (w *world) openStep(turn, step int) {
	w.t.Helper()
	w.append(sessionlog.EventStepStart, sessionlog.StepStartData{Turn: turn, Step: step}, nil)
}

// endStep 关一个步骤。
func (w *world) endStep(turn, step int) {
	w.t.Helper()
	w.append(sessionlog.EventStepEnd, sessionlog.StepEndData{Turn: turn, Step: step}, nil)
}
...
func (w *world) commit(messages []llm.Message) {
	...
	for _, message := range messages {
		w.append(sessionlog.EventUserMessage, sessionlog.UserMessageData{Message: message}, sessionlog.AppendOp{})
	}
}
...
func (w *world) appendQuiet(kind sessionlog.EventType, payload any) {
	w.t.Helper()
	...
	if _, err := w.live.sess.Append(sessionlog.Event{Type: kind, Data: data}); err != nil {
		w.t.Fatalf("追加 %s 失败：%v", kind, err)
	}
}
...
func Test会话不属于任何工作区时一个字都不发(t *testing.T) {
	...
	w := newWorldRooted(t, Config{MaxBytes: 1 << 20},
		func(context.Context, sessionlog.WorkspaceID) (string, bool, error) { return "", false, nil })
	decision := w.run(2, userSaid("u", "喂"))
	if len(decision.Messages) != 1 {
...
func Test换不出工作区根路径时把错交出去(t *testing.T) {
	...
	w := newWorldRooted(t, Config{MaxBytes: 1 << 20},
		func(context.Context, sessionlog.WorkspaceID) (string, bool, error) { return "", false, planted })
...
func Test表面上认不出来的那几条被跳过(t *testing.T) {
	...
	w := newWorld(t, Config{MaxBytes: 1 << 20})
	w.append(sessionlog.EventAssistantMessage, sessionlog.AssistantMessageData{
		Turn: 1, Step: 1,
		Message: llm.Message{
	...
	w.append(sessionlog.EventUserMessage, map[string]any{"message": "这不是一条消息"}, sessionlog.AppendOp{})
...
func Test头一次看见一段跑到一半的会话时靠重放算出步骤开着(t *testing.T) {
	...
	w.appendQuiet(sessionlog.EventStepStart, sessionlog.StepStartData{Turn: 1, Step: 3})
	w.appendQuiet(sessionlog.EventStepEnd, sessionlog.StepEndData{Turn: 1, Step: 3})
	w.appendQuiet(sessionlog.EventStepStart, sessionlog.StepStartData{Turn: 1, Step: 4})
...
func Test会话事件只认已经在场的那几段(t *testing.T) {
	...
	w.sessions.observer(other.sess, sessionlog.Event{Type: sessionlog.EventStepStart})
