...
func (i *installer) lookupState(id sessionlog.SessionID) (*sessionState, bool) {
	i.mutex.Lock()
	defer i.mutex.Unlock()
...
func (i *installer) stepIsOpen(state *sessionState, live *coresession.Session) bool {
	...
	for _, event := range live.Events() {
		...
		case sessionlog.EventStepStart:
			open = true
		case sessionlog.EventStepEnd, sessionlog.EventTurnEnd:
			open = false
		}
	}
...
func (i *installer) onSessionEvent(live *coresession.Session, event sessionlog.Event) {
	switch event.Type {
	case sessionlog.EventStepStart, sessionlog.EventStepEnd, sessionlog.EventTurnEnd:
	default:
		return
	}
	...
	state.mutex.Lock()
	state.stepKnown = true
	state.stepOpen = event.Type == sessionlog.EventStepStart
	var released []touch
	if event.Type == sessionlog.EventStepEnd {
...
func (i *installer) onToolResult(exec tools.Execution, result tools.Result) {
	...
	// 本来就会变成 IsError 为真的结果（见 tools 的包文档：一切失败都是结果），
	// 所以那一条被上一条吃掉了。
