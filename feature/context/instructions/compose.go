...
// 先例是 sessionlog/surface.go 的 sameMessageSource。
//
...
func userMessageOf(event sessionlog.Event) (llm.Message, bool) {
	...
	if !ok {
		// [sessionlog.DecodeData] 按事件类型分发，一条 user/message 只会得到
		// [sessionlog.UserMessageData]，所以这一支构造不出来。留着而不是断言掉，
...
func surfaceMessages(live *coresession.Session) []llm.Message {
	events := live.Events()
	bySeq := make(map[int]sessionlog.Event, len(events))
...
// 不加锁、只当只读投影用（见 harness/agent 的包文档），而本层的观察者跑在各自的
