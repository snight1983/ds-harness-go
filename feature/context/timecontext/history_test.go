...
func Test上一条模型可见消息认三种事件(t *testing.T) {
	...
	for _, one := range cases {
		t.Run(one.name, func(t *testing.T) {
			events := []sessionlog.Event{turnStart(t, 1, 1), one.event, requestHeader(9)}
...
func Test模型看不见的事件不算基线(t *testing.T) {
	...
	events := []sessionlog.Event{turnStart(t, 1, 1), stepStart(t, 2, 1, 1), requestHeader(3), stepEnd(t, 4, 1, 1)}
...
func Test日志里没有读数时取不到步骤基线(t *testing.T) {
	...
	events := []sessionlog.Event{userText(t, 1, "你好"), otherPluginText(t, 2, "别人的注入")}
...
func Test没注入过时上一次注入取不到(t *testing.T) {
	...
	_, ok, err := LatestInjectionTime([]sessionlog.Event{userText(t, 1, "你好")})
	if err != nil {
		t.Fatalf("这段日志该读得回来，却报了 %v", err)
	}
	if ok {
...
func Test间隔不为正时永远注入(t *testing.T) {
	...
	events := []sessionlog.Event{readingAt(t, 1, Reading{Now: at(1), Turn: 1, Step: 1}, time.UTC)}
...
func Test第一次注入不受节流拦(t *testing.T) {
	...
	inject, err := ShouldInject([]sessionlog.Event{userText(t, 1, "你好")}, at(2), time.Hour)
	if err != nil {
		t.Fatalf("判定该成功，却报了 %v", err)
	}
	if !inject {
...
func Test间隔没走满就跳过(t *testing.T) {
	...
	events := []sessionlog.Event{readingAt(t, 10, Reading{Now: at(10), Turn: 1, Step: 1}, time.UTC)}
...
func Test时钟往回跳时照样注入(t *testing.T) {
	...
	events := []sessionlog.Event{readingAt(t, 100, Reading{Now: at(100), Turn: 1, Step: 1}, time.UTC)}
...
func Test坏掉的用户消息负载被报出来(t *testing.T) {
	...
	broken := eventAt(1, sessionlog.EventUserMessage, json.RawMessage(`{`))

	if _, err := IsReadingEvent(broken); !errors.Is(err, ErrMalformedEvent) {
		t.Fatalf("IsReadingEvent 该报 ErrMalformedEvent，得到 %v", err)
	}
	if _, _, err := LatestInjectionTime([]sessionlog.Event{broken}); !errors.Is(err, ErrMalformedEvent) {
		t.Fatalf("LatestInjectionTime 该报 ErrMalformedEvent，得到 %v", err)
	}
	if _, _, err := PrecedingStepContextTime([]sessionlog.Event{broken}, 1); !errors.Is(err, ErrMalformedEvent) {
		t.Fatalf("PrecedingStepContextTime 该报 ErrMalformedEvent，得到 %v", err)
	}
	if _, err := ShouldInject([]sessionlog.Event{broken}, at(1), time.Minute); !errors.Is(err, ErrMalformedEvent) {
		t.Fatalf("ShouldInject 该报 ErrMalformedEvent，得到 %v", err)
	}
	if _, _, err := PreviousBaseline([]sessionlog.Event{broken}, 1, 2); !errors.Is(err, ErrMalformedEvent) {
...
func Test坏掉的回合开头负载被报出来(t *testing.T) {
	...
	broken := eventAt(1, sessionlog.EventTurnStart, json.RawMessage(`{"turn":"一"}`))

	if _, _, err := PrecedingStepContextTime([]sessionlog.Event{broken}, 1); !errors.Is(err, ErrMalformedEvent) {
		t.Fatalf("PrecedingStepContextTime 该报 ErrMalformedEvent，得到 %v", err)
	}
	if _, _, err := PreparationPosition([]sessionlog.Event{broken}); !errors.Is(err, ErrMalformedEvent) {
