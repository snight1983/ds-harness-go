...
var workspaceID = sessionlog.WorkspaceID("ws-timecontext")
...
func newWorld(t *testing.T, config Config) *world {
	...
	live, err := coresession.NewSession("s", coresession.Options{
		Header: &sessionlog.SessionHeader{ID: "s", WorkspaceID: workspaceID},
...
func (w *world) openTurn(turn int) {
	w.t.Helper()
	w.append(sessionlog.EventTurnStart, sessionlog.TurnStartData{Turn: turn}, nil)
}
...
func (w *world) say(text string) {
	w.t.Helper()
	w.append(sessionlog.EventUserMessage, sessionlog.UserMessageData{Message: llm.Message{
...
func (w *world) corrupt() {
	w.t.Helper()
	if _, err := w.live.sess.Append(sessionlog.Event{
...
func (w *world) appendReading(decision agent.PreStepDecision) {
	...
	w.append(sessionlog.EventUserMessage,
		sessionlog.UserMessageData{Message: decision.Messages[len(decision.Messages)-1]},
		sessionlog.AppendOp{})
}
