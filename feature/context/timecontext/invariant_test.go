...
func openStepAt(t *testing.T) []sessionlog.Event {
	t.Helper()
...
func Test没有开着的回合就没有追加位置(t *testing.T) {
	...
	cases := []struct {
		...
		{"空日志", nil},
		{"只有一条用户消息", []sessionlog.Event{userText(t, 1, "你好")}},
		{"回合已经关掉", []sessionlog.Event{turnStart(t, 1, 1), stepStart(t, 2, 1, 1), turnEnd(3)}},
	}
...
func Test步骤没开时没有追加位置(t *testing.T) {
	...
	cases := []struct {
		...
		{"回合刚开", []sessionlog.Event{turnStart(t, 1, 1)}},
		{"步骤已经关掉", []sessionlog.Event{turnStart(t, 1, 1), stepStart(t, 2, 1, 1), stepEnd(t, 3, 1, 1)}},
	}
	for _, one := range cases {
		t.Run(one.name, func(t *testing.T) {
...
func Test坏掉的负载在复核时被报出来(t *testing.T) {
	...
	broken := eventAt(4, sessionlog.EventUserMessage, json.RawMessage(`{`))
...
func forgedReading(t *testing.T, old string, replacement string) sessionlog.Event {
	t.Helper()
...
func Test整段日志里坏掉的事件被报出来(t *testing.T) {
	...
	events := []sessionlog.Event{eventAt(1, sessionlog.EventUserMessage, json.RawMessage(`{`))}
...
func Test坏掉的步骤开头负载在算位置时被报出来(t *testing.T) {
	...
	events := []sessionlog.Event{
		turnStart(t, 1, 1),
		eventAt(2, sessionlog.EventStepStart, json.RawMessage(`{"step":"一"}`)),
	}
