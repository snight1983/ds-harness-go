...
func eventTime(event sessionlog.Event) time.Time {
...
func PrecedingMessageTime(events []sessionlog.Event) (time.Time, bool) {
...
func PrecedingStepContextTime(events []sessionlog.Event, turn int) (time.Time, bool, error) {
	for index := len(events) - 1; index >= 0; index-- {
		event := events[index]
...
func LatestInjectionTime(events []sessionlog.Event) (time.Time, bool, error) {
	for index := len(events) - 1; index >= 0; index-- {
...
// 放在一起，改一处就看得见另一处。
func PreviousBaseline(events []sessionlog.Event, turn int, step int) (time.Time, bool, error) {
	if step == 1 {
...
func ShouldInject(events []sessionlog.Event, now time.Time, refreshInterval time.Duration) (bool, error) {
...
func IsReadingEvent(event sessionlog.Event) (bool, error) {
...
func decodeUserMessage(event sessionlog.Event) (llm.Message, error) {
	data, err := sessionlog.DecodeData(event)
	if err != nil {
