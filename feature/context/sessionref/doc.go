...
// 以及**各层注入的上下文**都不进去。注入内容跨会话写一遍会叠罗汉，
// 而推理内容是模型的草稿，写过去会被另一个模型当成结论。
//
// # 快照是「当时」的事实
//
...
// # 不做什么
//
//   - **不认自然语言里的会话名。**只认 `dsh-session:` 这个规范 URI 和主机包出来的
//     那种提及记号，一段说「参考我上次那个会话」的话在这里什么都不是。
//   - **不替人挑会话，也不画界面。**[Resolver.ListCandidates] 只排出候选，
//     点哪几个是主机的事。
//   - **不审核引用进来的内容。**它加的是边界声明、标签转义和一个硬上限，
//     被引用的文本仍然是别人写的——「不可信」是承认，不是解决。
//   - **不搬工具结果和推理内容。**投影只留用户自己说的话和助手的回答，各层注入的
//     上下文也不进去；写过去会叠罗汉，而模型的草稿会被另一个模型当成结论。
//   - **不刷新已经落下的快照。**[Reference] 记的是那一次观察的事实，想要新的就再引用一次。
//   - **不自己接进 agent。**[Install] 是可选的一条边；主机的自动补全只要
//     [Resolver.MentionCandidates]，不该为此拖上一整套 agent 装配。
package sessionref
