...
type fakeSessions struct {
	...
	surfaces   map[sessionlog.SessionID]sessionquery.SurfaceSnapshot
	listErr    error
	surfaceErr map[sessionlog.SessionID]error

	// beforeSurface 在每次读表面之前被调用，用来在读的中途撤掉 ctx。
	beforeSurface func(id sessionlog.SessionID)
...
func (f *fakeSessions) put(id sessionlog.SessionID, workspace sessionlog.WorkspaceID, createdAt int64, events []sessionlog.Event) {
	header := sessionlog.SessionHeader{Version: 1, ID: id, CreatedAt: createdAt, WorkspaceID: workspace}
...
func (f *fakeSessions) ReadSurface(ctx context.Context, id sessionlog.SessionID) (sessionquery.SurfaceSnapshot, error) {
...
func (f *fakeSessions) putRaw(id sessionlog.SessionID, events []sessionlog.Event) {
	header := sessionlog.SessionHeader{Version: 1, ID: id}
	f.records = append(f.records, sessionquery.Record{Header: header, Live: true})
...
func (f *fakeTitles) ReadTitles(ctx context.Context, ids []sessionlog.SessionID) ([]string, error) {
	if f.err != nil {
		return nil, f.err
	}
...
func injectedEvent(t *testing.T, seq int, text string) sessionlog.Event {
	t.Helper()
...
func checkpointEvent(t *testing.T, seq int, text string) sessionlog.Event {
	t.Helper()
...
func messageEvent(t *testing.T, seq int, message llm.Message) sessionlog.Event {
	...
	return sessionlog.Event{
		...
		Seq:       seq,
		Time:      int64(seq) * 1000,
		Data:      marshalPayload(t, sessionlog.UserMessageData{Message: message}),
		SurfaceOp: sessionlog.AppendOp{},
	}
}

// assistantEvent 排一条助手消息。
func assistantEvent(t *testing.T, seq int, blocks ...llm.ContentBlock) sessionlog.Event {
	t.Helper()
...
func toolResultEvent(t *testing.T, seq int) sessionlog.Event {
	t.Helper()
	...
	return sessionlog.Event{
		...
		Seq:  seq,
		Time: int64(seq) * 1000,
		Data: marshalPayload(t, sessionlog.ToolResultData{
			Turn: 1, Step: 1,
			Message: llm.Message{
				ID:      "t",
				Role:    llm.RoleUser,
...
func snapshotOf(id sessionlog.SessionID, workspace sessionlog.WorkspaceID, events []sessionlog.Event) sessionquery.SurfaceSnapshot {
	captured, capturedAny := 0, false
	if len(events) > 0 {
		captured, capturedAny = events[len(events)-1].Seq, true
	}
	return sessionquery.SurfaceSnapshot{
		Session:            sessionlog.SessionHeader{Version: 1, ID: id, WorkspaceID: workspace},
		CapturedThroughSeq: captured,
		CapturedAny:        capturedAny,
		Events:             events,
	}
