...
func TestListCandidates按工作区的亲疏排序(t *testing.T) {
	...
	// （见 [sessionlog.SessionHeader.WorkspaceID]），三档的语义一字不变。
...
func TestListCandidates同一档里保持列出来的先后(t *testing.T) {
	sessions := newFakeSessions()
	for _, id := range []sessionlog.SessionID{"甲", "乙", "丙", "丁"} {
...
func TestListCandidates按关键词搜得到没进前几名的那个(t *testing.T) {
	sessions := newFakeSessions()
	for _, id := range []sessionlog.SessionID{"a", "b", "c"} {
		sessions.put(id, "ws-1", 1, nil)
	}
	sessions.put("要找的", "ws-2", 2, nil)
	titles := &fakeTitles{titles: map[sessionlog.SessionID]string{"要找的": "上个月的调研"}}
...
func TestListCandidates按会话id和工作区也搜得到(t *testing.T) {
	...
	for needle, want := range map[string]sessionlog.SessionID{
		"独一无二":  "独一无二的id",
		"独特工作区": "别的",
	} {
...
func TestListCandidates截到上限(t *testing.T) {
	sessions := newFakeSessions()
	for _, id := range []sessionlog.SessionID{"a", "b", "c", "d"} {
		sessions.put(id, "ws-1", 1, nil)
	}
	resolver := newTestResolver(t, sessions, nil, Config{})
...
func TestListCandidates标题为空时也退回会话id(t *testing.T) {
	...
	titles := &fakeTitles{titles: map[sessionlog.SessionID]string{"s2": "有标题的"}}
	resolver := newTestResolver(t, sessions, titles, Config{})
	...
	labels := map[sessionlog.SessionID]string{}
	for _, candidate := range candidates {
...
func TestMentionCandidates每条都带上规范提及(t *testing.T) {
	sessions := newFakeSessions()
	sessions.put("s1", "ws-1", 1, nil)
	titles := &fakeTitles{titles: map[sessionlog.SessionID]string{"s1": "上一次调研"}}
	resolver := newTestResolver(t, sessions, titles, Config{})
...
func TestPrepare把全部引用聚成一条不可信上下文(t *testing.T) {
	sessions := newFakeSessions()
	sessions.put("s1", "ws-1", 1, []sessionlog.Event{userEvent(t, 1, "甲说的")})
	sessions.put("s2", "ws-1", 2, []sessionlog.Event{assistantEvent(t, 1, llm.TextBlock{Text: "乙答的"})})
	resolver := newTestResolver(t, sessions, nil, Config{})
...
func TestPrepare被引用的内容拼不出一个闭标签(t *testing.T) {
	sessions := newFakeSessions()
	sessions.put("s1", "", 1, []sessionlog.Event{
...
func TestPrepare把这次引用了谁记进持久来源(t *testing.T) {
	sessions := newFakeSessions()
	sessions.put("s1", "ws-1", 1, []sessionlog.Event{userEvent(t, 5, "甲说的")})
	sessions.put("s2", "ws-1", 2, []sessionlog.Event{userEvent(t, 9, "乙说的")})
	resolver := newTestResolver(t, sessions, nil, Config{})
...
func TestPrepare把重复的引用去掉而不是报错(t *testing.T) {
	sessions := newFakeSessions()
	sessions.put("s1", "", 1, []sessionlog.Event{userEvent(t, 1, "甲说的")})
	resolver := newTestResolver(t, sessions, nil, Config{})
...
func TestPrepare取消先于读失败(t *testing.T) {
	...
	sessions.put("s1", "", 1, nil)
	sessions.surfaceErr["s1"] = errNotFound
	sessions.beforeSurface = func(sessionlog.SessionID) { cancel() }
	resolver := newTestResolver(t, sessions, nil, Config{})
...
func TestPrepare坏掉的来源日志一路报上来(t *testing.T) {
	// 读表面成功了，投影那一步才发现负载坏了。
	sessions := newFakeSessions()
	sessions.putRaw("s1", []sessionlog.Event{
		{Type: sessionlog.EventUserMessage, Seq: 1, Data: []byte(`{"role":123}`)},
	})
	resolver := newTestResolver(t, sessions, nil, Config{})
...
func TestPrepare预算装不下时报预算超了(t *testing.T) {
	...
	sessions.put("很长的会话名字很长的会话名字", "ws-一个很长的工作区标识", 1,
		[]sessionlog.Event{userEvent(t, 1, strings.Repeat("长", 100))})
...
func TestCandidateRank把三档分清楚(t *testing.T) {
	for name, item := range map[string]struct {
		candidate, target sessionlog.WorkspaceID
		want              int
	}{
		"同一个工作区":       {"ws-1", "ws-1", 0},
		"候选不属于任何工作区":   {"", "ws-1", 1},
