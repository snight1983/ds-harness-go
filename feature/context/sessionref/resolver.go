...
type SessionSource interface {
	...
	ReadSurface(ctx context.Context, id sessionlog.SessionID) (sessionquery.SurfaceSnapshot, error)
}
...
type TitleReader interface {
	ReadTitles(ctx context.Context, ids []sessionlog.SessionID) ([]string, error)
}

// Target 是这次操作针对的那个会话。
//
// 新增: DSH 那两个方法收的是整个 Agent，用到的只有 `agent.id` 和
// `agent.session.header.cwd`。收一个 Agent 会把本包钉死在 harness/agent 上，
// 而它真正需要的就是这两个值。
type Target struct {
...
func (r *Resolver) readLabels(ctx context.Context, records []sessionquery.Record) ([]string, error) {
	...
	ids := make([]sessionlog.SessionID, len(records))
...
// 写法，不是错误。而自引用是错误——它会让一个会话把自己的历史又写一遍塞回自己，
// 每一轮翻一倍。
//
// 新增: DSH 那边先逐个查「是不是对象」「sessionId 是不是字符串」，因为主机递过来的
// 是没验过的 JSON。Go 这边 []Input 的形状由类型系统保住，那两条检查没有产出方。
func normalizeReferences(target sessionlog.SessionID, references []Input, maxReferences int) ([]Input, error) {
	seen := make(map[sessionlog.SessionID]struct{}, len(references))
...
func candidateRank(candidate sessionlog.WorkspaceID, target sessionlog.WorkspaceID) int {
