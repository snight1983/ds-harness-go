...
// [Scope] 和各张表各自带锁。这不是照录来的，是 Go 侧的必需品。
//
// # 不做什么
//
//   - **不做级联关停。**关掉父作用域不会关掉子作用域，链只用来继承和路由。
//   - **不定义注册表里装什么。**层的内容由使用方给的类型决定，
//     本包对工具、观察者、提示片段一无所知。
//   - **不做鉴权。**[Admits] 管的是可见性，不是访问权限。
//   - **不做 I/O、不起 goroutine、不计时。**没有超时、没有 TTL、没有后台回收，
//     层的回收是登记撤销时顺带发生的。
