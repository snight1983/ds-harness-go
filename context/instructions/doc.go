// Package instructions 是工作区指令这一层：把会话工作目录到项目根之间每一级的
...
// 三件事，别的都不管：
//
//  1. **发现与基线。**从 cwd 往上走找到项目根（默认看 `.git`），把根到 cwd
//     这条链上每一级目录里的候选文件全部读出来，加上一份用户全局指令，
//     渲染成一段带 `<system-reminder>` 框的文字。这就是[LoadBaselineSet]。
...
//
// 不管的：什么时候去对账、对账结果挂到哪条消息上、谁来触发。那是插件装配那一层
// 的事（DSH 的 `apply`），在 DESIGN.md 第八节的第 6 块。本包全部是可以直接调的
// 纯函数加一个文件系统接缝，没有任何隐含的调度。
//
// # 路径是斜杠分隔的世界绝对路径
//
...
//  8. **`apply` 和 `invariant.ts` 不在本包。**前者要 Agent、收件箱和三个
//     `ctx.on` 钩子，归第 6 块；后者装的是一个空的不变量安装器，没有东西可注册。
//     两处的裁决行都留在 docs/portmap/portmap.tsv 里。
//
// # 覆盖率为什么到不了 99%
//
// docs/DESIGN.md 第九节给纯逻辑包定的线是 99%，有 I/O 的包低于这个数要在源码里
// 写明为什么。本包的用例覆盖到 98.5%，剩下的九个分支分成两类，都**不是**没写用例：
//
