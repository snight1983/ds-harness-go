...
//
// 不管的：什么时候调它、这条上下文挂到哪条消息后面。那是 Agent 装配那一层的事
// （DSH 的 `agent/pre-step` 钩子），在 docs/DESIGN.md 第八节的第 6 块。
//
// # 引用进来的内容是不可信的
//
...
//  9. **`agent/pre-step` 那段装配和 `invariant.ts` 不在本包。**前者要 Agent
//     和收件箱，归第 6 块，所以 `src/index.ts` 里 `SessionReferenceResolver`
//     那两行裁决仍然是 PENDING——[Resolver] 本身在这里写完了，缺的只是把它挂上
//     Agent 的那一段。后者装的是一个空的不变量安装器，没有东西可注册。
//
// # 覆盖率为什么到不了 99%
//
// docs/DESIGN.md 第九节给纯逻辑包定的线是 99%，低于这个数要在源码里写明为什么。
// 本包的用例覆盖到 96.5%，剩下的十三个分支**全部**是证明不可达、留着当护栏的
// 错误支，分成三类：
//
