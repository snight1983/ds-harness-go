...
func PreparationPosition(events []session.Event) (int, int, error) {
...
func ValidateReading(history []session.Event, event session.Event, location *time.Location) error {
...
func decodeStepStart(event session.Event) (session.StepStartData, error) {
	data, err := session.DecodeData(event)
	if err != nil {
		return session.StepStartData{}, fmt.Errorf("%w：seq %d 的 step/start：%w",
			ErrMalformedEvent, event.Seq, err)
	}
	start, ok := data.(session.StepStartData)
	if !ok {
		// 不可达，理由同 [decodeTurnStart]。
		return session.StepStartData{}, fmt.Errorf("%w：seq %d 声称是 step/start，负载却是 %T",
