...
// # 不做什么
//
//   - **不存字节。**本包只有词汇和次序规则，落盘归 [Store] 的实现方，
//     本仓库的那一个在 adapter/imagestore。
//   - **不做图片编解码。**解码由 adapter/imagestore 负责；重新编码是可选能力
//     [RequestImageProjector]，不实现就是一个明确的「不支持」码。
//   - **不判断某条模型路由收不收图片。**那由 llm 那条路由和它的模型目录回答，
//     本包只按调用方给的预算办事。
//   - **不做事务，也不清理垃圾。**一批写到一半失败时前面的对象还在，本包只保证
//     部分成功的批次不返回任何引用；那些没人引用的对象归 [Store] 背后那套介质的保留策略收。
//   - **不当文件系统用。**路径、目录、列举、原地编辑那一套在 fs，附件只有「提交、按引用取回」两个动作。
package attachment
