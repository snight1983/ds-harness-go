...
// statuses 是 [session.TodoStatus] 的三个合法取值，按给模型看的顺序排。
//
// 源: packages/todo/tool-todo/src/index.ts:26
//
// 顺序就是语义：它会原样进 schema 的 enum，而那份 schema 逐字进提示词缓存的键。
var statuses = []session.TodoStatus{session.TodoPending, session.TodoInProgress, session.TodoCompleted}
...
type Tool struct {
	allowParallel bool
	append        func(*scope.Key, []session.TodoItem) error
}
...
// writeArgs 是模型写出来的参数。
//
// 单独收一个 Status 字符串而不是直接收 [session.TodoStatus]，是因为这一层只做
...
// 而不是被默默拍平）。
func toTodoList(raw writeArgs, allowParallel bool) ([]session.TodoItem, error) {
	todos := make([]session.TodoItem, 0, len(raw.Todos))
	...
	for _, item := range raw.Todos {
		...
		seen[content] = struct{}{}
		if item.Status == string(session.TodoInProgress) {
			active++
		}
		todos = append(todos, session.TodoItem{Content: content, Status: session.TodoStatus(item.Status)})
	}
...
func countByStatus(todos []session.TodoItem) counts {
	var tally counts
