...
//
// 这里的登记／公布／摘除三步和 [github.com/snight1983/ds-harness-go/harness/session.Store] 逐字同构，
...
// [github.com/snight1983/ds-harness-go/harness/session.CreateOptions] 上那条逐字相同——那层嵌套不承载语义。
//
...
//
// 新增: 这个类型有自己的互斥锁，规矩和 [github.com/snight1983/ds-harness-go/harness/session.Store] 逐字
...
// [github.com/snight1983/ds-harness-go/harness/session.Store.Create] 上那条逐字相同。
...
//
// panic 也算否决不是随手定的，理由和 [github.com/snight1983/ds-harness-go/harness/session.Store.Announce]
...
func (r *Registry) callNotify(id sessionlog.SessionID, what string, run func()) {
	defer func() {
		if recovered := recover(); recovered != nil {
			r.logger.Warn("harness/agent: 通知观察者 panic 了",
