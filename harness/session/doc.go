...
//
// # 为什么和 github.com/snight1983/ds-harness-go/sessionlog 分成两个包
//
// DSH 那个包里既有词汇（事件信封、负载、表面折叠、不变量），也有活的对象
// （Session、SessionStore）。本仓库把它劈成两半：
//
//   - [github.com/snight1983/ds-harness-go/sessionlog] 是**词汇**——纯值和纯函数，持久化后端、查询、
//     回放、统计都只需要它，谁都不必为了读一条事件而把一个内存注册表拖进来。
//   - 本包是**活的那一半**——可变状态、发布钩子、作用域派发。它依赖词汇，
//     词汇不依赖它。
//
// 这条边界不是审美：本仓库里 feature/persistence、sessionlog/projection、
// feature/sessionstats 这些包全部只 import 词汇那一半。合成一个包的话，一个只想
// 排一条事件的调用方会连带拿到 [Store] 和它整套 [github.com/snight1983/ds-harness-go/scope]
// 依赖。
//
