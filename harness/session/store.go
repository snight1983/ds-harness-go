...
// [Store.OnFlush] 和摘除时落盘。这条分工是本仓库 feature/persistence 那一族包
// 存在的前提。
//
...
func (s *Store) OnCreated(
	...
	if observer == nil {
		return nil, errors.New("harness/session: 创建观察者不能是 nil")
	}
...
func (s *Store) OnDisposed(
	...
	if observer == nil {
		return nil, errors.New("harness/session: 摘除观察者不能是 nil")
	}
	return s.layers.Effect(ctx, owner, func(layer *storeLayer) (func(), error) {
...
func (s *Store) OnEvent(
	...
	if observer == nil {
		return nil, errors.New("harness/session: 追加观察者不能是 nil")
	}
	return s.layers.Effect(ctx, owner, func(layer *storeLayer) (func(), error) {
...
func (s *Store) OnFlush(
	...
	if observer == nil {
		return nil, errors.New("harness/session: 刷盘观察者不能是 nil")
	}
	return s.layers.Effect(ctx, owner, func(layer *storeLayer) (func(), error) {
...
type CreateOptions struct {
	...
	// [github.com/snight1983/ds-harness-go/sessionlog.OriginSubagent]。
...
func (s *Store) Enter(owner *scope.Scope, session *Session) (func(context.Context) error, error) {
	if owner == nil {
		return nil, errors.New("harness/session: 登记一个会话需要一个载体作用域")
	}
	if session == nil {
		return nil, errors.New("harness/session: 登记的会话不能是 nil")
	}
	id := session.ID()
...
func (s *Store) callDisposedObserver(id sessionlog.SessionID, observer DisposedObserver, session *Session) {
	defer func() {
		if recovered := recover(); recovered != nil {
			s.logger.Warn("harness/session: 摘除观察者 panic 了",
...
func callFlushObserver(ctx context.Context, observer FlushObserver, session *Session) (err error) {
	defer func() {
		if recovered := recover(); recovered != nil {
			err = fmt.Errorf("harness/session: 刷盘观察者 panic 了: %v", recovered)
		}
	}()
	return observer(ctx, session)
}
