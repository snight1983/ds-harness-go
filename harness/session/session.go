...
type Options struct {
	...
	// feature/telemetry 那一处逐字相同——不然测试里断言不了时间戳。
	Now func() int64
...
// [github.com/snight1983/ds-harness-go/sessionlog.Event] 日志。
//
...
// [github.com/snight1983/ds-harness-go/scope.Layers]，那一层自己并发安全，不碰存储的锁，
// 于是「会话锁 → 存储锁」这条边根本不存在。
type Session struct {
...
// [github.com/snight1983/ds-harness-go/sessionlog.Event.Clone] 复制，调用方留着的那些切片之后怎么改
// 都改不动这份日志。
...
// （盖上当下的 [github.com/snight1983/ds-harness-go/sessionlog.FormatVersion]），所以它**总是**在。
...
//
// 它和 [github.com/snight1983/ds-harness-go/sessionlog.SessionHeader.SeedLength] 不是一回事：后者是
...
// 第二个参数在不在」，而 [github.com/snight1983/ds-harness-go/sessionlog.Event] 上那两个字段本来就在，
// 所以这里收的就是一条填了一半的事件，见裁决表上 SurfaceIntent 那一行。
// 那条「表面事件必须带 SurfaceOp」的约束由
// [github.com/snight1983/ds-harness-go/sessionlog.SurfaceOpOf] 在下面验，DSH 在运行期也验同一条。
//
...
// [github.com/snight1983/ds-harness-go/scope.Layers] 的两层，不会失败，所以次序上没有可观察的
...
// [github.com/snight1983/ds-harness-go/sessionlog.EpochHeader]——也就是**下一次**请求要拿去比对的那份。
//
// 源: packages/core/session/src/index.ts:664-687
//
// 第二个返回值为假表示还没有过任何一条 request/header 快照。
//
// 它是 [github.com/snight1983/ds-harness-go/sessionlog.FoldRequestHeader] 活着的、增量维护的那个形态：
...
// 是 [github.com/snight1983/ds-harness-go/sessionlog.DeriveEventMessage]。
//
...
func (s *Session) DeriveMessages() ([]llm.Message, error) {
	...
	for _, seq := range nodes[s.derivedNodes:] {
		...
		// 才碰巧一样（`sessionlog/surface.go` 往节点里塞的是 plan.seq）。这份日志
...
// DeriveEventMessage 是 [github.com/snight1983/ds-harness-go/sessionlog.DeriveEventMessage] 挂在会话上的
// 那一面。
//
