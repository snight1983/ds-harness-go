...
func mustNamespace(value string) settings.Namespace {
	...
	if err != nil {
		panic(fmt.Sprintf("harness/agentdefaultmodel: 命名空间字面量不合法：%v", err))
	}
	return namespace
}
...
func validateSettings(value Settings) error {
	if value.Provider == "" {
		return fmt.Errorf("harness/agentdefaultmodel: 默认模型选择缺了提供方")
	}
	if value.Model == "" {
		return fmt.Errorf("harness/agentdefaultmodel: 默认模型选择缺了模型")
	}
	return nil
}
...
func New(config Config) (*Service, func(), error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("harness/agentdefaultmodel: 登记设置小节失败：%w", err)
	}
	service.scope = scope
