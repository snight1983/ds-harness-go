// 本文件的作用：从包外驱动一个真的回合，证明 [harness.New] 拼出来的那份闭环不只是
// 编译得过，而是跑得通——并且顺带钉住 [harness.Harness].Vocabulary 确实盖得住这个回合写下的
// 每一条事件。
//
...
//
// 这是这份装配测试存在的理由：[github.com/snight1983/ds-harness-go/harness/agentloop] 自己的测试全在包内、用的是那个
...
//
// 人设是 [harness.Options] 里唯一一项会一路走到提供方请求上的配置。它没接上的话
...
func TestAssembleRejectsIncompleteOptions(t *testing.T) {
	...
	cases := map[string]harness.Options{
	...
	for name, options := range cases {
		t.Run(name, func(t *testing.T) {
			...
			host, unwind, err := harness.New(t.Context(), options)
			if err == nil {
				_ = unwind(t.Context())
				t.Fatal("该被拒")
			}
