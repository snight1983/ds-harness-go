...
// 后端、不接协议入口——那几样各自需要一个真的介质，只能由宿主自己决定。
//
// 持久化是个例外，它**装在里面**：后端要活会话存储和并好的事件词汇才造得出来，
// 而循环工厂要后端才接得上续跑，这两头都在 [New] 内部，宿主在外面没有任何一刻
// 能同时拿到。所以 [Options.Persistence] 是一个时机钩子——[New] 在两样都在场、
// 循环工厂还没造的那一刻回调宿主一次，收回一个持久化实现。不给这个钩子的装配
// 仍然是一份不碰外部介质的闭环，代价是 Resume 接不上。
//
// # 运行期包在它下面
//
// 底下五个子包是运行期本身：
