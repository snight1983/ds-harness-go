...
//
// 这个字符串照录 DSH，一个字符都没改：它进了会话日志，是本投影**认领自己那些
...
// Go 这边追加事件的观察者由 [github.com/snight1983/ds-harness-go/harness/session.Session.Append] 在
...
func NewRuntimeContextProjection(
	...
	if sessions == nil || live == nil {
		return nil, fmt.Errorf("harness/agentloop: 运行期上下文投影要有会话存储和一个活会话")
	}
	...
	if _, err := sessions.OnEvent(ctx, owner, func(subject *session.Session, event sessionlog.Event) {
		...
		return nil, fmt.Errorf("harness/agentloop: 挂运行期上下文投影的事件观察者失败：%w", err)
	}
