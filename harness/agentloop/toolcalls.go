...
// [github.com/snight1983/ds-harness-go/tools.Runtime] 的 Prepare／Dispatch／Finalize／Finish
...
func ExecuteToolCalls(
	...
	if runtime == nil || acceptContext == nil {
		return false, errors.New("harness/agentloop: 调度工具调用要有工具运行时和一个上下文接收方")
	}
	initiator, err := agent.RequireInitiator(ctx)
	if err != nil {
		return false, fmt.Errorf("harness/agentloop: 调度工具调用时读不出发起者：%w", err)
	}
	batch := &toolBatch{
		tools:         runtime,
...
func (b *toolBatch) runGroup(
	...
	if committed != started {
		...
		return groupOutcome{}, errors.New("harness/agentloop: 工具调度器有 settle 了却没提交的调用")
	}
...
func (b *toolBatch) appendToolCall(block llm.ToolCallBlock) (int, error) {
	...
	if err != nil {
		// 走不到：这个负载里只有整数和字符串。
		return 0, fmt.Errorf("harness/agentloop: 排 tool/call 负载失败：%w", err)
	}
	event, err := b.session.Append(sessionlog.Event{Type: sessionlog.EventToolCall, Data: payload})
	if err != nil {
		return 0, fmt.Errorf("harness/agentloop: 追加 tool/call 失败：%w", err)
	}
...
func (b *toolBatch) appendToolResult(
	...
	if err != nil {
		return fmt.Errorf("harness/agentloop: 排 tool/result 负载失败：%w", err)
	}
	if _, err := b.session.Append(sessionlog.Event{
		...
		return fmt.Errorf("harness/agentloop: 追加 tool/result 失败：%w", err)
	}
	return nil
}
