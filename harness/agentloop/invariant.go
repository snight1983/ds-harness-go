...
// 它折得出一份请求头；请求带的消息和 [github.com/snight1983/ds-harness-go/harness/session.Session.DeriveMessages]
...
func RegisterInvariants(
	...
	if registry == nil {
		return nil, fmt.Errorf("harness/agentloop: 注册不变量需要一个不变量注册表")
	}
	if owner == nil {
		return nil, fmt.Errorf("harness/agentloop: 注册不变量需要一个持有这次登记的作用域")
	}
	if runtime == nil {
		return nil, fmt.Errorf("harness/agentloop: 注册不变量需要一个 llm 运行时")
	}
	if sessions == nil {
		return nil, fmt.Errorf("harness/agentloop: 注册不变量需要一个会话存储")
	}
	...
	install := func(installCtx context.Context, invariantScope *invariants.Scope, fail invariants.Fail) error {
		...
		if err != nil {
			return fmt.Errorf("harness/agentloop: 往 llm/stream 上装不变量失败：%w", err)
		}
