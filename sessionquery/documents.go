...
func BuildEventRecords(id session.SessionID, events []session.Event) ([]EventRecord, error) {
...
func BuildEventSearchDocuments(id session.SessionID, events []session.Event) ([]EventSearchDocument, error) {
	surfaces, err := classifySurface(events)
	if err != nil {
		return nil, err
	}
