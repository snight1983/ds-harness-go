...
// 所以它不返回错误。Go 这边 [session.Event.Data] 是原始字节，解不回来说明
// 这份日志坏了——那是一件必须说出来的事，不是「这条没有可检索的文字」。
// 把它压成空串会让一份坏日志安静地检索不出东西，那比报错难查得多。
func ExtractEventText(event session.Event) (string, error) {
