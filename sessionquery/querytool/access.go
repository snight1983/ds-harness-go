...
func targetID(sessionID string, from caller) session.SessionID {
	if sessionID == "" {
		return from.id
	}
...
func (c *Controller) authorizeSessionIDs(
	...
	authorized := map[session.SessionID]struct{}{}
	var other []session.SessionID
	...
	requested := make(map[session.SessionID]struct{}, len(other))
...
func (c *Controller) readTitles(
	...
	from caller,
	ids []session.SessionID,
) (map[session.SessionID]titleView, error) {
	...
	result := make(map[session.SessionID]titleView, len(observations))
...
func (c *Controller) readTitle(ctx context.Context, from caller, id session.SessionID) (titleView, error) {
	titles, err := c.readTitles(ctx, from, []session.SessionID{id})
	if err != nil {
...
func descendantIDs(nodes []*authorizedDescendant) []session.SessionID {
