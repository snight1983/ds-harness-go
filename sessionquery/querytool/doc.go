// Package querytool 把 [github.com/snight1983/ds-harness-go/sessionquery] 那台读引擎摆到模型面前，
...
// # 本包不是一次引擎移植
//
// 读、过滤、检索、追溯这些事一行都不在这里，它们全在 [github.com/snight1983/ds-harness-go/sessionquery]。
// 本包只做三件引擎不该做的事：
//
...
//     绝不透露那个会话到底存不存在。
//  2. **把失败翻译成模型读得懂、又不泄露内情的一句话。**引擎的 18 个错误码里
...
//     前缀去掉，那样这个包会和 [github.com/snight1983/ds-harness-go/sessionquery] 撞名。也没有并进那个包：
//     引擎那一层现在只依赖 llm/session/persistence，把 core/tools、core/scope、
//     core/systemprompt 拖进去会让一台纯粹的读引擎背上整套工具运行时。
//  2. **时间戳的上下界用整数取整，不是浮点数的相邻可表示值。**DSH 的
//     nextUpFinite / nextDownFinite 在 float64 的位表示上找相邻值，因为那边的
//     范围端点是浮点毫秒，而 ISO 时间戳可以带亚毫秒小数。Go 侧
//     [github.com/snight1983/ds-harness-go/sessionquery.Range] 的端点是 *int64 毫秒，压根表示不了亚毫秒，
...
//     和 [github.com/snight1983/ds-harness-go/plan/planmode] 同一个做法。
//  4. **超时不由本包计时。**DSH 在每次 invoke 外面自己包一个 timeoutMs。Go 侧
//     [github.com/snight1983/ds-harness-go/core/tools.Definition.Timeout] 就是这件事，由工具运行时统一
//     执行，也统一不发给模型。
