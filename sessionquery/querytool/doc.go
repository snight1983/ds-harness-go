// Package querytool 把 [ds-harness-go/sessionquery] 那台读引擎摆到模型面前，
...
// # 本包不是一次引擎移植
//
// 读、过滤、检索、追溯这些事一行都不在这里，它们全在 [ds-harness-go/sessionquery]。
// 本包只做三件引擎不该做的事：
//
...
//     前缀去掉，那样这个包会和 [ds-harness-go/sessionquery] 撞名。也没有并进那个包：
...
//     [ds-harness-go/sessionquery.Range] 的端点是 *int64 毫秒，压根表示不了亚毫秒，
...
//     和 [ds-harness-go/plan/planmode] 同一个做法。
//  4. **超时不由本包计时。**DSH 在每次 invoke 外面自己包一个 timeoutMs。Go 侧
//     [ds-harness-go/core/tools.Definition.Timeout] 就是这件事，由工具运行时统一
//     执行，也统一不发给模型。
