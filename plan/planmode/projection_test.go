...
func modeEvent(seq int, data string) session.Event {
	return session.Event{Seq: seq, Type: planmode.EventMode, Data: json.RawMessage(data)}
}

// mode 造一条负载合法的 plan/mode。
func mode(seq int, active bool) session.Event {
	if active {
...
// plain 造一条只有类型的会话事件。
func plain(seq int, kind session.EventType) session.Event {
	return session.Event{Seq: seq, Type: kind}
}

// runEvent 造一条 `/plan` 的 command/run。
func runEvent(t *testing.T, seq int, id commands.ID, args string) session.Event {
	t.Helper()
...
func doneEvent(t *testing.T, seq int, id commands.ID, kind commands.ResultKind) session.Event {
	t.Helper()
	return commandEvent(t, seq, commands.EventDone, commands.DoneData{ID: id, Kind: kind})
}

// commandEvent 把一份命令负载排成事件。
func commandEvent(t *testing.T, seq int, kind session.EventType, payload any) session.Event {
	t.Helper()
	encoded, err := json.Marshal(payload)
	if err != nil {
		t.Fatalf("排负载失败：%v", err)
	}
	return session.Event{Seq: seq, Type: kind, Data: encoded}
}
...
func projectionOf(t *testing.T, events ...session.Event) planmode.Projection {
	t.Helper()
...
func TestTheLastModeEventWins(t *testing.T) {
	t.Parallel()
	events := []session.Event{mode(0, true), mode(1, false), mode(2, true)}
...
func TestFoldingStopsAtTheGivenEnd(t *testing.T) {
	t.Parallel()
	events := []session.Event{mode(0, true), mode(1, false)}
...
func TestAnUnreadableModeEventKeepsThePreviousState(t *testing.T) {
	...
	for _, bad := range []string{`not json`, `{}`, `{"active":null}`, `{"active":42}`} {
		events := []session.Event{mode(0, true), modeEvent(1, bad)}
		if !planmode.FoldMode(events) {
