...
type Controller struct {
	...
	mutex   sync.Mutex
	pending map[session.SessionID]pendingIntent
}
...
func New(config Config) (*Controller, error) {
	...
	return &Controller{
		...
		logger:  logger,
		pending: make(map[session.SessionID]pendingIntent),
	}, nil
}
...
func (c *Controller) OnSessionDisposed(id session.SessionID) {
	c.clearPending(id)
}
...
// 已经是那种模式时也不通知。
func (c *Controller) narration(events []session.Event, target bool) (llm.Message, bool) {
...
func (c *Controller) pendingOf(id session.SessionID) (pendingIntent, bool) {
	c.mutex.Lock()
...
func (c *Controller) setPending(id session.SessionID, pending pendingIntent) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
...
func (c *Controller) clearPending(id session.SessionID) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	delete(c.pending, id)
}
...
func appendMode(sess *coresession.Session, active bool) error {
	...
	if _, err := sess.Append(session.Event{Type: EventMode, Data: payload}); err != nil {
