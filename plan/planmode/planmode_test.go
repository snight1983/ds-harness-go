...
func (w *world) Append(kind session.EventType, data any) error {
	payload, err := json.Marshal(data)
	if err != nil {
		return err
	}
	_, err = w.agent.sess.Append(session.Event{Type: kind, Data: payload})
	return err
}
...
func openTurn(t *testing.T, w *world) {
	t.Helper()
	w.agent.append(t, session.Event{Type: session.EventTurnStart})
}
...
func TestSetNarratesOnceTheModelHasBeenToldAMode(t *testing.T) {
	...
	w.agent.append(t, session.Event{Type: session.EventRequestHeader})
