...
func TestAttachSession挂上来的排在账目最前面(t *testing.T) {
	...
	if got := mustSessionIDs(t, ctx, created); !slices.Equal(got, []session.SessionID{"s4", "s3"}) {
...
func TestAttachSession认活会话表里的会话头(t *testing.T) {
	...
	if got := mustSessionIDs(t, ctx, created); !slices.Equal(got, []session.SessionID{"刚建的"}) {
		t.Fatalf("拿到 %v", got)
	}
}
...
func TestAttachSession工作区对得上的会话挂得上来(t *testing.T) {
	...
	if got := mustSessionIDs(t, ctx, created); !slices.Equal(got, []session.SessionID{"对得上的"}) {
		t.Fatalf("拿到 %v", got)
	}
}
...
func TestInsertSessionBefore按锚点挪位(t *testing.T) {
	...
	if got := mustSessionIDs(t, ctx, created); !slices.Equal(got, []session.SessionID{"s1", "s3", "s2"}) {
		t.Fatalf("拿到 %v", got)
	}
	...
	if got := mustSessionIDs(t, ctx, created); !slices.Equal(got, []session.SessionID{"s3", "s2", "s1"}) {
		t.Fatalf("拿到 %v", got)
	}
}
...
func TestInsertSessionBefore挪到原位是空操作(t *testing.T) {
	...
	if got := mustSessionIDs(t, ctx, created); !slices.Equal(got, []session.SessionID{"s2", "s1"}) {
		t.Fatalf("拿到 %v", got)
	}
	if !mustUpdatedAt(t, ctx, created).Equal(before) {
...
func TestInsertSessionBefore点名账目外的会话时不写(t *testing.T) {
	...
	if got := mustSessionIDs(t, ctx, created); !slices.Equal(got, []session.SessionID{"s1"}) {
...
func TestDetachSession摘掉并落盘且不在账目里的是空操作(t *testing.T) {
	...
	if got := mustSessionIDs(t, ctx, created); !slices.Equal(got, []session.SessionID{"s2"}) {
		t.Fatalf("拿到 %v", got)
	}
	...
	registry = h.reopen(ctx)
	if got := mustSessionIDs(t, ctx, mustGet(t, ctx, registry, ws)); !slices.Equal(got, []session.SessionID{"s2"}) {
...
func TestSessionIDs筛掉归属对不上的候选并在下一次真实写入时裁掉(t *testing.T) {
	...
	registry := h.open(ctx)
	if got := mustSessionIDs(t, ctx, mustGet(t, ctx, registry, here)); !slices.Equal(got, []session.SessionID{"s2", "s1"}) {
	...
	if got := mustRecord(t, ctx, registry, created.ID()).SessionIDs; !slices.Equal(got, []session.SessionID{"s2", "s1"}) {
	...
	if got := mustRecord(t, ctx, registry, created.ID()).SessionIDs; !slices.Equal(got, []session.SessionID{"s1"}) {
