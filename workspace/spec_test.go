...
func goodRecord() Record {
	...
	return Record{
		...
		DisplayPath: "/a",
		Title:       "甲",
		SessionIDs:  []session.SessionID{"s1"},
		CreatedAt:   at,
		UpdatedAt:   at,
	}
}
...
func TestRecordValidate按能不能用划界(t *testing.T) {
	...
	cases := []struct {
		...
		{"账目里混进空会话id", func(r *Record) {
			r.SessionIDs = []session.SessionID{"s1", ""}
		}, "第 1 项"},
	}
...
func TestDomainStateValidate只查这一份值自己(t *testing.T) {
	...
	full := DomainState{
		...
		ArchivedSessionIDs: []session.SessionID{"s1"},
	...
	cases := []struct {
		...
		{"归档集合里混进空会话id", DomainState{ArchivedSessionIDs: []session.SessionID{""}}, "归档集合第 0 项"},
