...
type entityHost interface {
	...
	// 本身，一次文件系统调用都不发，见 [session.SessionHeader.WorkspaceID]。
	sessionWorkspaceID(id session.SessionID) (session.WorkspaceID, bool)
...
func (e *entity) SessionIDs(ctx context.Context) ([]session.SessionID, error) {
	record, err := e.read(ctx)
	if err != nil {
		return nil, err
	}
...
// 这一项——那是它在表里的键。
func filterAccounted(host entityHost, workspaceID WorkspaceID, record Record) []session.SessionID {
...
// 而是归属（见 [session.SessionHeader.WorkspaceID]），三步塌成一次相等，
// 一次文件系统调用都不发。
//
// 跟着消失的还有 [CodeStorageFailed] 这一支：它当初只可能从那次 stat 里来。
// 读会话头本身出故障仍然报它，那是 readSessionHeader 自己的事。
func (e *entity) validateAttach(ctx context.Context, sessionID session.SessionID) error {
