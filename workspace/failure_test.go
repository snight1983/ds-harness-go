...
// # 剩下那几条盖不到的分支
//
// 本包的语句覆盖率停在 99.2%，差的那几条都在 registry.go 里，且都要求
...
func TestOpen收尾待恢复标记时删记录失败(t *testing.T) {
	...
	record := mustRecord(t, registry, created.ID())
	seedState(t, ctx, registry, DomainState{
		Initialized:     true,
...
func TestEnqueue每次操作之前先收尾待恢复标记(t *testing.T) {
	...
	registry.mutex.Lock()
	registry.state.PendingMutation = &PendingMutation{
...
// interruptedBootstrap 跑一次被打断的 bootstrap：记录都落了盘，
...
func TestOpenBootstrap重跑时已有账目里的会话留在历史候选后面(t *testing.T) {
	...
	h.persistence.set(header("s2", "/a", 200))
	registry := h.open(ctx)
	created := mustList(t, registry)[0]
	if got := mustRecord(t, registry, created.ID()).SessionIDs; len(got) != 2 || got[0] != "s2" || got[1] != "s1" {
		t.Fatalf("历史候选该在前、已有账目在后，拿到 %v", got)
	}
	// 读投影另说：s1 没有会话头，筛不出来。
	if got := created.SessionIDs(); len(got) != 1 || got[0] != "s2" {
		t.Fatalf("拿到 %v", got)
	}
}
...
func TestOpenBootstrap一条历史都没有时按记录自己的建时刻和id定序(t *testing.T) {
	...
	h.persistence.set()
	registry = h.open(ctx)
	if got := workspaceIDs(mustList(t, registry)); len(got) != 2 || got[0] != "ws-a" || got[1] != "ws-b" {
...
func TestReadPaths域在底下关掉之后一律报存储失败而不是读到一张空表(t *testing.T) {
	...
	t.Run("状态校验里读记录", func(t *testing.T) {
		if err := registry.validateStoredState(state); !errors.Is(err, CodeStorageFailed) {
			t.Fatalf("要的是 CodeStorageFailed，拿到 %v", err)
		}
	})
	t.Run("状态校验里读整张表", func(t *testing.T) {
		// 次序为空时前一段的逐条读跳过，第一次真正的读是整张表那一次。
		if err := registry.validateStoredState(DomainState{}); !errors.Is(err, CodeStorageFailed) {
			t.Fatalf("要的是 CodeStorageFailed，拿到 %v", err)
		}
	})
	t.Run("重建实体缓存", func(t *testing.T) {
		if err := registry.rebuildEntities(); !errors.Is(err, CodeStorageFailed) {
			t.Fatalf("要的是 CodeStorageFailed，拿到 %v", err)
		}
	})
	...
	t.Run("排 bootstrap 次序", func(t *testing.T) {
		if _, err := registry.bootstrapOrder(registry.workspace, nil, nil); !errors.Is(err, CodeStorageFailed) {
			t.Fatalf("要的是 CodeStorageFailed，拿到 %v", err)
		}
	})
	_ = created
...
func TestNotStarted关掉之后每一条内部路径都各自守着(t *testing.T) {
	...
	t.Run("删工作区", func(t *testing.T) {
		// 关掉时缓存被清空了，得先放一条回去才走得到取表那一步。
		registry.putEntity("ws-1", newEntity(registry, "ws-1", goodRecord()))
	...
	t.Run("收尾待恢复标记", func(t *testing.T) {
		...
		registry.mutex.Lock()
		registry.state.PendingMutation = nil
		registry.mutex.Unlock()
	})
	...
	t.Run("状态校验", func(t *testing.T) {
		if err := registry.validateStoredState(DomainState{}); !errors.Is(err, CodeNotStarted) {
			t.Fatalf("要的是 CodeNotStarted，拿到 %v", err)
		}
	})
	t.Run("重建实体缓存", func(t *testing.T) {
		if err := registry.rebuildEntities(); !errors.Is(err, CodeNotStarted) {
			t.Fatalf("要的是 CodeNotStarted，拿到 %v", err)
		}
	})
...
func TestRebuildEntities次序指向一条不存在的记录时报不一致(t *testing.T) {
	ctx := t.Context()
	h := newHarness(t)
	registry := h.open(ctx)
	...
	if err := registry.rebuildEntities(); !errors.Is(err, CodeInconsistentState) {
		t.Fatalf("要的是 CodeInconsistentState，拿到 %v", err)
	}
	_ = ctx
}

func TestReportFilteredCandidates次序里没有实体的那一条直接跳过(t *testing.T) {
	ctx := t.Context()
	sink := &logSink{}
	h := newHarness(t)
	h.logger = sink.logger()
	...
	// 这是一个纯诊断投影，不该因为次序里多出一个查不到的 id 就崩掉或者乱报。
	registry.reportFilteredCandidates()
	if len(sink.dump()) != 0 {
...
func TestEntityByTargetKey认得还没进次序的那一条(t *testing.T) {
	...
	// 建工作区的三步之间，实体已经在缓存里、但还没进次序。这一段窗口里
	// 按目标标识找必须也找得到它，否则同一个目录会被建出第二个工作区。
	record := goodRecord()
	registry.putEntity("ws-在途", newEntity(registry, "ws-在途", record))

	found, ok := registry.entityByTargetKey(record.TargetKey)
	if !ok {
	...
	if _, ok := registry.entityByTargetKey("key:谁也不是"); ok {
		t.Fatal("不认识的目标标识不该找出东西")
	}
	_ = ctx
}
...
func TestInsertBefore次序写不下去时报存储失败(t *testing.T) {
	...
	before := workspaceIDs(mustList(t, registry))
	h.backend.armGlobal(map[int]error{1: errBackend})
	if _, err := registry.InsertBefore(ctx, second.ID(), ""); !errors.Is(err, CodeStorageFailed) {
		t.Fatalf("要的是 CodeStorageFailed，拿到 %v", err)
	}
	h.backend.armGlobal(nil)
	if got := workspaceIDs(mustList(t, registry)); !equalIDs(got, before) {
