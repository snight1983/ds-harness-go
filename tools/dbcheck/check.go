...
const datastorePath = modulePath + "/datastore"
...
const (
	...
	// zoneDatastore 是 datastore/ 底下：数据库就是它的活儿，三条全放。
	zoneDatastore
...
func zoneOf(path string) zone {
	switch {
	case strings.HasPrefix(path, "tools/dbcheck/"):
		return zoneExempt
	case path == "datastore" || strings.HasPrefix(path, "datastore/"):
