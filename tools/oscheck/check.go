...
const (
	...
	// zoneLocal 是 tools/ 和 cmd/：它们跑在本机上，两条全放。
	zoneLocal
...
func zoneOf(path string) zone {
	...
	switch {
	case strings.HasPrefix(path, "tools/"), strings.HasPrefix(path, "cmd/"):
		return zoneLocal
	default:
		return zoneBusiness
	}
