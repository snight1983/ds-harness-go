...
//
//   - 本仓自造的工具目录（tools/…）没有上游对应物，一条溯源都不该有。
//   - 包文档里写了 `新增:` 且全包一条 `源:` 都没有的包，等于已经在最显眼的地方
//     交代过「这个包整份是新写的」。fs/objectstore 就是这样一个包：DSH 那边挂在
...
func auditThinProvenance(all []packageProvenance) []packageProvenance {
	...
	for _, stats := range all {
		if stats.pkg == "tools" || strings.HasPrefix(stats.pkg, "tools/") {
			continue
		}
...
func writeAuditHeader(out *strings.Builder, totalRows, portedRows int) {
	...
	out.WriteString("由 `go run ./tools/portcheck -mode audit` 生成，**不要手工编辑**——它每次会被整份覆盖。\n\n")
...
func writeThinProvenance(out *strings.Builder, thin, all []packageProvenance) {
	...
	out.WriteString("**这一份指的是该去哪儿细读，不是哪一行有 bug**。两类包已排除：本仓自造的 `tools/`，")
