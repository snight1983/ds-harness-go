...
func TestCheckRejectsPortedPointingAtNothing(t *testing.T) {
	...
	writeFile(t, exportsPath, strings.Join([]string{
		...
		"core/session\tpackages/core/session/src/index.ts\t1\tclass\tStore\tsrc/index.ts",
		"core/session\tpackages/core/session/src/index.ts\t2\tclass\tGhost\tsrc/index.ts",
	}, "\n")+"\n")

	rulingPath := filepath.Join(t.TempDir(), "portmap.tsv")
	writeFile(t, rulingPath, strings.Join([]string{
		rulingtable.Header,
		"core/session\tpackages/core/session/src/index.ts\t1\tclass\tStore\tsrc/index.ts\tPORTED\tsession.Store\t",
