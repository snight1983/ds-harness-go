...
//
// tools/ 底下的东西没有上游对应物，一条溯源都不该有。不豁免的话这份报告的前几名
// 永远是它们，真正该看的包会被挤到看不见的地方。
func TestAuditThinProvenanceExcludesOwnTooling(t *testing.T) {
	t.Parallel()

	thin := auditThinProvenance([]packageProvenance{
		{pkg: "tools/portcheck", lines: 500, sources: 0},
		{pkg: "fs/objectstore", lines: 1000, sources: 0},
		{pkg: "llm", lines: 1000, sources: 100},
	})

	if len(thin) != 1 || thin[0].pkg != "fs/objectstore" {
		t.Fatalf("只该留下 fs/objectstore，实际 %#v", thin)
	}
}
...
func TestAuditThinProvenanceHonoursAPackageThatDeclaredItselfNew(t *testing.T) {
	...
	thin := auditThinProvenance([]packageProvenance{
		{pkg: "fs/objectstore", lines: 1000, sources: 0, declaredNew: true},
		{pkg: "half/ported", lines: 1000, sources: 5, declaredNew: true},
		{pkg: "silent/gap", lines: 1000, sources: 0},
	})

	if len(thin) != 2 {
		t.Fatalf("只该豁免 fs/objectstore 那一个，实际留下 %#v", thin)
	}
	for _, stats := range thin {
		if stats.pkg == "fs/objectstore" {
