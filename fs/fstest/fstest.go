...
// # 不做什么
//
//   - **不是生产后端。**全部内容在进程内存里，进程一停全没；真要落地的实现
//     在 adapter/objectstore。
//   - **不模仿任何后端的局限。**空目录在这里是存在的，对象存储那边不是——
//     照着装一遍等于把后端特有的毛病伪装成契约。
//   - **不替代窄假件。**只需要两三个方法、别的一律 panic 的场合（见 feature/workspace、
//     feature/context/instructions）该继续用窄假件，那种假件的价值就在于会当场炸。
//   - **不做性能保证。**分块尺寸取得很小是为了逼出多块路径，不是给人测吞吐用的。
package fstest
