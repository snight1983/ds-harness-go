...
// # 不做什么
//
//   - **不是 os.File 的替代。**没有文件描述符、没有符号链接创建、没有 mmap、
//     没有随机位置写；这条接缝上只有解析、读、列、原子写和字面编辑。
//   - **不做读取窗口和「观察过的状态」那套策略。**它们在消费方和 [Policy] 的订阅方那边，
//     留在这条缝上的只有后端才做得了的事。
//   - **不做沙箱。**DSH 的 sandboxMode 与 sandboxPolicy 都没有搬，
//     一个恒返回「不限制」的能力位比没有这个位更误导人。
//   - **不解释 [TargetKey] 和 [Version]。**它们是后端自己定的不透明字符串，
//     本包不拼接、不比较结构、不推断它背后是什么形状的介质。
//   - **不管预设目录。**预设树属于宿主部署配置，故意不走这套执行世界文件系统。
package fs
