...
// # 不做什么
//
//   - **不持有权威状态。**内存里一份都不留，每一次读都穿到介质上；
//     多副本部署里照着本地内存回答就是答错。
//   - **不做分布式锁。**那条写链只把本副本的写串起来，跨副本靠 [Table.Update]
//     的条件写和记录级修订号挡，撞满 5 次就把 [storage.CodeStaleRevision] 原样交回去。
//   - **不广播跨副本的变更。**变更事件只反映本副本写下去的那些。
//   - **不碰介质。**读写最终都落到 [storage.Backend] 的键值形态上，
//     本包只加类型、校验、事件和生命周期。
//   - **不做迁移。**打开时校验不过就整个域打不开，不会自动改写老记录去迁就新声明。
package domain
