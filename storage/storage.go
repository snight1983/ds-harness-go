...
// # 不做什么
//
//   - **不做任何 IO。**中枢只登记名字和形态，介质归后端、语义归数据形态；
//     它一动手读写，「谁拥有这份介质」就有了第二个答案。
//   - **不管数据库的运维。**建库、备份、迁移编排、跨区域复制都在这条接缝之外。
//   - **不提供分布式锁。**跨副本的并发只靠记录级修订号，那是乐观的，不是互斥；
//     变更事件也只反映本副本的写。
//   - **不替宿主定策略。**租户键、保留期限、加密、访问控制都由装配方决定。
//   - **不存会话事件。**会话日志走 feature/persistence 那条接缝，
//     这里只放非会话数据。
package storage
