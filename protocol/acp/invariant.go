...
//
// 源: packages/acp/acp/src/invariant.ts
...
//
// 源: packages/acp/acp/src/invariant.ts:12-13（name）
const InvariantPluginName = "acp-invariant"

// RegisterInvariants 把这个包登记进不变量注册表，返回注销函数。
//
// 源: packages/acp/acp/src/invariant.ts:28-29
//
// 装进去的检查是**空的**（DSH invariant.ts:21 那个 `() => {}`），这是刻意的：这一层是
// 一条传输，它不拥有任何耐久的、属于本包的事件流——它搬的每一条边都归发出那条边的
// 那个包所有，也在那边被检查。这一层自己的协议映射和收摊次序由边界测试覆盖。
//
...
func RegisterInvariants(ctx context.Context, registry *invariants.Registry) (func(), error) {
	if registry == nil {
		return nil, fmt.Errorf("acp: 注册不变量需要一个不变量注册表")
	}
