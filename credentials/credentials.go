...
//
// 界面上一屏凭据是一批行，每行一个引用。[DescribeRefs] 一次把这一批问完，
// 因为它们要一起落定：拆成一个个问，先回来的行先渲染，后回来的行在旁边空着，
// 而它们描述的本该是同一个时刻的同一份配置。
//
// 它带着两道这条接缝本身不管的关：名字要过 [IsRefName]（有一个不过就整次拒绝），
// 数量不超过 [MaxDescribeRefs]。第二道挡的是「一次通过了鉴权的请求让提供方
// 开始一段没有上界的活」。
//
// 值在这条路上是**单向**的：[Provider.Set] 送进去，没有任何一条读路径送回来。
//
// # 抽象类怎么变成了接口加一个通知器
//
...
// # 不做什么
//
//   - **不持有值，也不缓存解析结果。**值和它的存储归 [Provider] 的实现方；
//     消费方每次操作重新解析一遍，换掉密钥之后下一次操作就用上新值。
//   - **不加密、不轮换、不跑 OAuth 流程。**这些都在提供方和宿主那一侧，
//     本包只定义「引用怎么解析」「记录怎么原子替换」这两件事。
//   - **不做访问控制。**[Provider.ModifyRecord] 管的是一次写的原子性，
//     谁有资格改哪条记录由宿主判。
//   - **不把值放进通知。**[Observer] 收到的只是「该重读了」，不携带密钥正文；
//     记录的序列化形式只给受保护的后端用，不进普通应用日志。
//   - **不给值的打码预览。**配置界面读得到的是 [Info]（在不在、来自哪一层、能不能写）
//     和 [RecordInfo]，一条读路径都不交出值本身，[DescribeRefs] 也不例外。
//     不做打码是因为一个足够认人的掩码同样足够认贼：留几位尾巴就是留几位给猜，
//     而「这一位配没配」这个问题本来就不需要看见值。
//   - **不枚举引用。**记录那一半有 [Provider.ListRecords]，引用这一半没有对应物：
//     有哪些引用是由设置的 schema 说的，所以 [DescribeRefs] 要调用方点名来问。
package credentials
