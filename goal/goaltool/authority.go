...
func openTurn(caller agent.Agent) (session.Event, []session.Event, error) {
	events := caller.Session().Events()
	for index := len(events) - 1; index >= 0; index-- {
		switch events[index].Type {
		case session.EventTurnEnd:
			return session.Event{}, nil, fail(CodeDriverRequired, missingTurn)
...
func (c *Controller) hasDirectHumanInput(execution Execution) bool {
	...
	return slices.ContainsFunc(execution.Events, func(event session.Event) bool {
