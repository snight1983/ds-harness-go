...
func turnStart(turn int) session.Event {
...
// turnEnd 造一条回合结束事件。
func turnEnd(turn int) session.Event {
	return session.Event{
...
func userEvent(t *testing.T, source llm.MessageSource) session.Event {
	t.Helper()
...
func goalRoundEvent(t *testing.T, source goal.Source) session.Event {
	t.Helper()
...
func newWorld(t *testing.T) *world {
	...
	return &world{
		...
		agents: &stubAgents{
			live:  map[session.SessionID]agent.Agent{caller.ID(): caller},
...
func (w *world) goalTurn(source goal.Source) {
	w.t.Helper()
	w.caller.append(w.t, turnStart(1))
	w.caller.append(w.t, session.Event{Type: goal.EventChange, Data: json.RawMessage(`{}`)})
...
func TestDirectHumanNeedsATopLevelAgentAndAHumanMessage(t *testing.T) {
	...
	cases := map[string]func(*world){
		...
		"这一轮里那条用户消息读不回来": func(w *world) {
			...
			// 「一条坏掉的事件被当成一次人类授权」。
			w.caller.append(w.t, session.Event{
				Type:      session.EventUserMessage,
