...
// # 不做什么
//
//   - **不读。**这条接缝是单向的，没有 readText，也不是键值存储；
//     取回靠后端自己的通道，模型照着 [Ref.RetrievalHint] 去调别的工具。
//   - **不决定什么时候外置。**判断一份结果是不是太大、原地留什么话，
//     是 feature/spillpolicy 的事。
//   - **不管留多久。**保留期在 feature/outputretention 和后端那套介质里，
//     本包不保证存进去的东西永远还在。
//   - **不解析 [Locator]。**它是后端自己定的字符串，消费方只渲染，
//     不拼接、不推断、不当路径用。
//   - **不做访问控制和加密。**这些归 [Store] 背后那套介质。
package spill
...
type SaveText struct {
	...
	// 这条约束照录 DSH，理由是这个字段一路上来自工具名，而工具名可以由插件自定，
	// 直接当路径用就是一个目录穿越。
