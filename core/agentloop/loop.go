...
func mustNamespace(value string) settings.Namespace {
	...
	if err != nil {
		panic(fmt.Sprintf("core/agentloop: 命名空间字面量不合法：%v", err))
	}
	return namespace
}
...
func New(ctx context.Context, deps Deps, owner *scope.Scope, config Config) (*AgentLoop, func(context.Context) error, error) {
	if deps.Agents == nil || deps.Sessions == nil || deps.LLM == nil ||
		...
		return nil, nil, errors.New("core/agentloop: 装一个循环工厂要有注册表、会话存储、模型、工具和系统提示词五样")
	}
	if owner == nil {
		return nil, nil, errors.New("core/agentloop: 装一个循环工厂要有一个持有它的作用域")
	}
	...
	if config.Settings != nil {
		...
		if err != nil {
			return fail(fmt.Errorf("core/agentloop: 登记设置小节失败：%w", err))
		}
...
func (l *AgentLoop) OnConfigStartFailed(observer ConfigStartFailedObserver) (func(), error) {
	if observer == nil {
		return nil, errors.New("core/agentloop: OnConfigStartFailed 需要一个观察者")
	}
...
func (l *AgentLoop) reportConfiguredStartupFailure(
	...
	l.logger.Warn("core/agentloop: 配置驱动的启动失败",
...
func (l *AgentLoop) notifyStartFailed(
	...
	defer func() {
		if recovered := recover(); recovered != nil {
			l.logger.Warn("core/agentloop: 启动失败观察者 panic 了",
				slog.String("agent", configID),
				slog.String("session", string(sessionID)),
...
func (l *AgentLoop) waitForDrainingConfiguredIdentity(ctx context.Context, sessionID sessionlog.SessionID) error {
	...
	if err != nil {
		return fmt.Errorf("core/agentloop: 等身份 %q 排干时挂不上 agent 观察者：%w", string(sessionID), err)
	}
	unwatchSession, err := l.deps.Sessions.OnDisposed(ctx, l.owner, func(*session.Session) { checkReleased() })
	if err != nil {
		return errors.Join(
			fmt.Errorf("core/agentloop: 等身份 %q 排干时挂不上会话观察者：%w", string(sessionID), err),
			unwatchAgent(ctx))
	}
...
func (l *AgentLoop) prepare(
	...
	if err != nil {
		...
		return nil, fmt.Errorf("core/agentloop: 把 agent %q 的生命周期挂到 owner 上失败：%w", string(id), err)
	}
...
func (l *AgentLoop) CreateAgent(
	...
	if owner == nil {
		return agent.Handle{}, errors.New("core/agentloop: 造一个 agent 要有一个持有它的作用域")
	}
...
func (l *AgentLoop) resumeWith(
	...
	if owner == nil {
		return agent.Handle{}, errors.New("core/agentloop: 续跑一个 agent 要有一个持有它的作用域")
	}
