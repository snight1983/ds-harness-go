...
func (e *CancelError) Error() string {
	if e.Cause == nil {
		...
		// 而接口值为 nil 的取消没有意义。
		return "core/agentloop: 回合被取消"
	}
	return fmt.Sprintf("core/agentloop: 回合被取消（%s）", e.Cause.CancelCauseKind())
}
...
func NewReactLoopAgent(
	...
	if deps.Agents == nil || deps.Sessions == nil || deps.LLM == nil ||
		...
		return nil, errors.New("core/agentloop: 装一个循环要有注册表、会话存储、模型、工具和系统提示词五样")
	}
	if live == nil {
		return nil, errors.New("core/agentloop: 装一个循环要有一个活会话")
	}
	if sessionID := live.ID(); id != sessionID {
		return nil, fmt.Errorf("core/agentloop: agent 身份是 %q，会话身份是 %q", string(id), string(sessionID))
	}

	own, err := scope.New(scope.NewKey("agent:"+string(id)), scope.Options{Parent: parent})
	if err != nil {
		return nil, fmt.Errorf("core/agentloop: 建 agent 作用域失败：%w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("core/agentloop: 建收件箱失败：%w", err)
	}
	loop.inbox = inbox
...
func lastTurnOf(live *session.Session) (int, error) {
	...
	for index := len(events) - 1; index >= 0; index-- {
		...
		if err != nil {
			return 0, fmt.Errorf("core/agentloop: turn/start 的负载读不回来：%w", err)
		}
		payload, ok := data.(sessionlog.TurnStartData)
		if !ok {
			// 走不到：这条事件的类型就是 turn/start，解码按类型分派。
			return 0, errors.New("core/agentloop: turn/start 解出来不是回合负载")
		}
...
func (a *ReactLoopAgent) Send(message llm.Message, target agent.InboxTarget, wakeup bool) {
	...
	if err != nil {
		...
		// 就没有它要唤醒的活儿。
		a.reportError(fmt.Errorf("core/agentloop: 往收件箱送消息失败：%w", err))
		return
	}
	if wakeup {
...
func (a *ReactLoopAgent) Cancel(cause sessionlog.TurnEndCancelCause, options agent.CancelOptions) {
	...
	if clearErr != nil {
		a.reportError(fmt.Errorf("core/agentloop: 清空收件箱失败：%w", clearErr))
	}
	if cancel != nil {
...
func (a *ReactLoopAgent) RunMaintenance(ctx context.Context, task func(context.Context) error) error {
	if task == nil {
		return errors.New("core/agentloop: 维护活儿不能是 nil")
	}

	a.mutex.Lock()
	if a.phase.kind != phaseIdle {
		a.mutex.Unlock()
		return fmt.Errorf("core/agentloop: agent %q 已经有活儿在跑了", string(a.id))
	}
...
func (a *ReactLoopAgent) turn() (bool, error) {
	...
	if a.phase.kind != phaseRunning {
		a.mutex.Unlock()
		err := fmt.Errorf("core/agentloop: agent %q：没有驱动预约就开回合", string(a.id))
		a.reportError(err)
		return false, err
	}
...
func (a *ReactLoopAgent) preStep(
	...
	if a.phase.kind != phaseRunning {
		...
		return agent.PreStepDecision{}, empty,
			fmt.Errorf("core/agentloop: agent %q：在 running 之外提议步骤", string(a.id))
	}
	...
	runNotifications(pending)
	if err != nil {
		return agent.PreStepDecision{}, empty, fmt.Errorf("core/agentloop: 认领收件箱失败：%w", err)
	}

	assembly, err := a.deps.SystemPrompt.Assemble(ctx, systemprompt.AssembleContext{Scope: a.scope.Key()})
	if err != nil {
		return agent.PreStepDecision{}, empty, fmt.Errorf("core/agentloop: 装配系统提示词失败：%w", err)
	}
	if err := abortedErr(ctx); err != nil {
	...
	if err != nil {
		return agent.PreStepDecision{}, empty, fmt.Errorf("core/agentloop: 渲染运行期上下文失败：%w", err)
	}
...
func (a *ReactLoopAgent) step(
	...
	if err != nil {
		return nil, fmt.Errorf("core/agentloop: 渲染系统提示词失败：%w", err)
	}
	...
	for {
		...
		if err != nil {
			return nil, fmt.Errorf("core/agentloop: 推导边界消息失败：%w", err)
		}
...
func assembledMessage(request llm.GenerateOptions, assembler *llm.BlockAssembler) (llm.Message, error) {
	...
	if err != nil {
		return llm.Message{}, fmt.Errorf("core/agentloop: 装配助手消息失败：%w", err)
	}
	provenance := llm.Provenance{Provider: request.Provider, Model: request.Model}
	envelope, hasReplay, err := assembler.ReplayState()
	if err != nil {
		return llm.Message{}, fmt.Errorf("core/agentloop: 取重放状态失败：%w", err)
	}
	if hasReplay {
		...
		if err != nil {
			return llm.Message{}, fmt.Errorf("core/agentloop: 排重放状态失败：%w", err)
		}
...
func (a *ReactLoopAgent) acceptToolContext(message llm.Message) {
	...
	runNotifications(pending)
	if err != nil {
		a.reportError(fmt.Errorf("core/agentloop: 收下工具捎回来的上下文失败：%w", err))
	}
}
...
func (a *ReactLoopAgent) buildRequest(
	...
	if err != nil {
		return empty, nil, fmt.Errorf("core/agentloop: 读请求头失败：%w", err)
	}
	a.mutex.Lock()
	...
	if proposed.Provider == "" || proposed.Model == "" {
		return empty, nil, fmt.Errorf(
			"core/agentloop: agent %q 没有 provider/model：给 agent.Options 填上这两项，或者在 agent/request 那条瀑布上一起给出",
			string(a.id))
	}
...
func (a *ReactLoopAgent) foldRequestHeader(header sessionlog.EpochHeader, headerLogged bool) error {
	...
	if err != nil {
		return fmt.Errorf("core/agentloop: 读请求头失败：%w", err)
	}
	if headerLogged {
...
func (a *ReactLoopAgent) foldRequestContext(config llm.CallConfig, prepared *llm.PreparedCall) error {
	...
	if err != nil {
		return fmt.Errorf("core/agentloop: 读请求元数据失败：%w", err)
	}
...
func (a *ReactLoopAgent) appendEvent(
	...
	if err != nil {
		return sessionlog.Event{}, fmt.Errorf("core/agentloop: 排 %s 负载失败：%w", eventType, err)
	}
	...
	if err != nil {
		return sessionlog.Event{}, fmt.Errorf("core/agentloop: 追加 %s 失败：%w", eventType, err)
	}
	return event, nil
}
