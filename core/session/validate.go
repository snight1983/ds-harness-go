...
var ErrInvalidSeed = errors.New("core/session: 构造 seed 不合法")
...
var ErrNotLive = errors.New("core/session: 会话不在这个存储里活着")
...
// 存储登记，名字那边是空的。
var ErrAlreadyAttached = errors.New("core/session: 这个会话已经登记在某个存储里了")

// ErrAlreadyAnnounced 是这个会话的创建公布已经开始过了。
//
// 一个观察者在创建回调里又去公布同一个会话，撞的也是这一条。
var ErrAlreadyAnnounced = errors.New("core/session: 这个会话已经公布过了")
