...
装完了不等于跑得动。一棵装好的树要能过的最低那道线是三件事连起来：
...
那道「看见它进收件箱了吗」的闸是有讲究的：等静下来等的是「这一层没活动了」，不是「我这条消息落地了」。没有这道闸，一次冒烟会把上一轮还没收敛的尾巴算进自己的账。

它对树的要求只有一条——**恰好一个顶层 agent**。多一个就报错而不是随便挑：挑错了，后面所有断言验的都是另一棵树。

上游那台冒烟机还起一个子进程、按环境变量在「源码态」和「构建态」两条启动路径之间二选一、等一个干净的退出码。这三样在 Go 里都不存在（没有 Loader，没有那份配置文件，编出来就一个可执行文件），所以没有移过来。
