...
flowchart TB
    ...
    C["skill 工具当前不可见"] --> C2["目录跟着消失<br/>不提示模型去调一个不存在的入口"]
    D["旧的 skill 目录损坏"] --> D2["下一步重新建<br/>不让会话失败"]
```

分界在于**这一次失败会不会让模型据以行动**：目录少一条，模型顶多不知道有这个 Skill；预设装了一半，模型会拿到一组互相矛盾的能力，那是必须整体回滚的。

其余几条：

- **名称、描述和 Provider 身份在进目录之前就校验**，不指望模型容错。
- **同层重名直接报错**，不静默挑一个；跨层重名才按「近的盖远的」解释。
- **展示信息坏了不阻止预设被发现，能力清单坏了禁止装载**——前者只影响给人看的那一面。
- **Provider 的 I/O 认 `context`**，调用方取消之后不接着等一个不合作的 Provider。
