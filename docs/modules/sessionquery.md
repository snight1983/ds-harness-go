...
本文只讲 `sessionquery` 这一个包，和它下面唯一的子包 `feature/sessionquery/querytool`。会话日志本身怎么记、怎么落盘、怎么折成当前状态（`sessionlog/` 那条线）不在本文范围内，只在需要说明接缝时点到名字。
...
| 按关键词搜整个历史 | 检索挂点（后端另挂，本包不自带；生产实现见[持久化抽象层](datastore.md)） |
...
- **三个方向的箭头都是单向的。**工具层调引擎，引擎调语料，语料读来源。没有一条反向的线——引擎不知道工具层的存在，`sessionlog/` 那条线上也没有任何一个文件 import 这个包。
- **`Searcher` 是虚线。**它是这张图上唯一可选的东西。没挂的时候，两个检索方法报「没挂检索后端」，**其余方法全部照常工作**——精确读、过滤、追溯是纯计算，不依赖任何后端。生产实现是 `adapter/datastore/searchstore`，装配方挂上去就生效。
...
会话头上那条「工作目录」（`WorkspaceRoot`）是一条**执行世界**里的绝对路径。别的包（`workspace`、`feature/context/instructions`）拿它当解析基准去问 `fs` 接缝；**本包不碰盘**——这里它只参与身份比对和工作区边界那两次字符串相等，仅此而已。
...
生产实现是 [`adapter/datastore/searchstore`](datastore.md)：它把落地的会话折成文档存进一份介质，活着的那些在本进程里当场折，两侧算出同一个分再并起来排。**挂不挂由装配方决定。**不挂的时候，两件检索工具报「没挂检索后端」，另外三件读工具照常可用——这不是缺陷，是上面那条分界的直接后果：检索是加进来的，不是这一层的基础。
...
读窗口上限这里和上游有一处取舍：上游写 0 表示「一条上下文都不许带」，这边零值就是「没填」、走默认 50。代价是「禁掉上下文窗口」这个配置表达不出来——那是上游也只是顺带允许、没人会用的配置，换来的是装配方不必为了拿默认值而把 50 写一遍。
