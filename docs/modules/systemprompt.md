...
五种登记、一条作用域链、一次装配。下面十节按「模型这一步收到什么 → 能往里塞什么 → 四条规则怎么裁 → 装配跑一遍是什么样」的顺序展开，最后一节是这些登记沿作用域链怎么合。

```mermaid
flowchart TB
    R1["提示词段落"] --> REG["harness/systemprompt<br/>注册表"]
    R2["环境快照"] --> REG
    R3["变量提供方"] --> REG
    R4["工具提供方"] --> REG
    R5["最后拦截"] --> REG
    REG --> RULE["四条规则<br/>排序 · 覆盖 · 开关 · 整份替换"]
    RULE --> ASM["一次装配<br/>沿作用域链合并 · 现问 · 渲染"]
    ASM --> LOOP["harness/agentloop<br/>拼请求发给模型"]
```

前三种是**值**，后两种是**待询问的函数**——工具清单和变量的答案要等这个 agent、这次请求的参数定下来才有。
...
`feature/preset/persona` 这个包存在的全部理由就是这一条。没有它，一份预设能换掉一个 agent 的工具，却永远换不掉这个 agent 的身份。
...
- 注册表允许多个 agent 并发读取与登记；登记返回撤销函数，作用域释放时自动撤销，预设换代之后不会留下上一代的段落。
- **一次装配用快照。** 并发进来的新登记只影响下一次装配，不会让当前这次请求看到半套配置。
- **每次装配把每个提供方重新问一遍，不缓存。** 变量和工具清单本来就依赖这次的 agent 和这次的请求参数，缓存等于让下一次请求读到上一次的答案。
- 装配是纯计算：不读写存储、不起后台任务、不计时。
- 挂最后拦截不触发「配置变了」的回调，其余四种登记都会——拦截没有改变登记了什么，只改变这一次装配的结果。
...
|---|---|---|---|---|
| 系统提示词组装注册表，按作用域收集段落、工具schema和变量 | `core/system-prompt` | 需要 | `harness/systemprompt` | — |
