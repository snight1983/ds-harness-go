...
flowchart TD
    ...
    subgraph U["消费方"]
        U1["harness/agentloop<br/>建会话、追加事件"]
        U2["feature/persistence<br/>四组观察者全登记"]
...
仓库里 `harness/agentloop` 走的正是三步那条路，不是 `Create`。
...
- **不做持久化。**它只在追加和刷盘时把钩子拉响，落盘是挂上去的插件自己的事。这条分工是 `feature/persistence` 那一族包存在的前提。
- **不管会话内容。**它不读日志、不折表面。
...
```mermaid
flowchart TD
    A["会话头"] --> A1["版本对不对<br/>id 对不对得上<br/>时间戳非负<br/>工作目录是绝对世界路径<br/>血统边界非负<br/>来源取值合法<br/>委派层数非负"]
    Z["日志起点"] --> Z1["不能是负数"]
