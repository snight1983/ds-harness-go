...
本文只讲 `core/scope` 一个包。用它搭出来的东西——工具运行时、Skill 目录、Agent 名册、活会话注册表——各有各的文档，不在这里。
...
```mermaid
graph LR
  AG["agent<br/>（core/agentloop）"]
  SC["Scope<br/>core/scope"]
...
graph LR
  KEY["一把 Key"]
  KEY --> M1["core/agent<br/>12 张表"]
  KEY --> M2["core/tools<br/>7 张表"]
  KEY --> M3["core/systemprompt<br/>6 张表"]
  KEY --> M4["core/session<br/>4 张表"]
  KEY --> M5["subagent/subagent<br/>4 张表"]
  ...
  KEY --> M10["goal/goal<br/>1 张表"]
```

10 个包各建一个 `map[*Key]*layer`。同一把 `Key` 会同时出现在这 10 个 map 里，各自查出一个只属于这个包的层。层里装什么由这个包自己定，`core/scope` 一无所知。
...
`core/agent` 的层，12 张（`observer.go:198`）：

```mermaid
graph LR
  AG["core/agent 的 layer"]
...
`core/tools` 的层，7 张（`runtime.go:95`）：

```mermaid
graph LR
  TL["core/tools 的 layer"]
  TL --> t1["tools *<br/>工具定义"]
...
`core/systemprompt` 的层，6 张（`registry.go:56`）：

```mermaid
graph LR
  SP["core/systemprompt 的 layer"]
...
| 路径 | 内容 |
|---|---|
| `core/scope/scope.go` | 身份、父链、准入、事件载体、所有权边界 |
| `core/scope/layers.go` | 全局层与覆盖层的叠放、登记原语与三条回滚、空层回收 |
| `core/scope/entries.go` | 两张登记表、精确撤销、快照遍历 |
| `core/scope/bench_test.go` | 关停路径的压力基线 |
