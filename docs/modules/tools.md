...
注册表的生命周期挂在 `scope` 的层上：一个作用域释放，它那一层的七张表一起消失，不需要谁去逐条注销。登记返回撤销函数，预设换代之后不会留下上一代的工具。

**一次调用用的是快照，不是当前注册表。** `FinalizeContent` 在调用开始时就取下来，可见性视图 `viewOf` 也是一次层遍历算出来的。调用跑到一半有人注销了这个工具，这次调用照样按开始时那份定义走完——否则一次调用的结尾会归另一个工具管。

**并发判定失败即独占。** 看不见的工具、没声明 `IsConcurrencySafe` 的、判定本身 panic 的，全都独占。并行是工具要明确认领的能力，不是默认待遇：错判成独占只是慢一点，错判成并行会让两次调用同时改同一份状态。

**每个结果观察者拿自己那一份副本。** `Result` 里的内容树、值、呈现载荷和上下文都是切片，结构体赋值复制不到它们，一个观察者改了会串到下一个。

**一批能同时跑几个不归本包管**，由 `harness/agentloop` 定；本包只回答一次调用能不能和兄弟重叠。
