...
- **Tool 和模型发出的调用不是一回事。** Tool 是程序预先注册的定义和执行体；模型只会交回工具名、调用标识和 JSON 参数。真正找到函数并执行的是本包的运行时。
- **模型给的参数一律是不可信输入。** 参数必须先过大小限制和 JSON Schema 校验；权限、审批和业务约束也必须由程序执行，不能把工具说明当成安全边界。
- **给模型看的清单和真正能执行的清单必须来自同一份作用域视图。** 两边分开计算，会出现模型看得见却调用不了，或者模型没看见却能越权调用的情况。本包的 `Schemas`、`Get` 和派发共用同一套解析结果。
- **业务值和模型看到的结果是两份东西。** 执行体交出权威 JSON 值，`Output.Render` 决定其中哪些内容变成模型可见的 Tool Result。会话日志保存的是后者；敏感字段、大块原始数据和模型下一步不需要的信息不应直接暴露。
- **成功和失败都必须形成 Tool Result。** 模型不认识 Go `error`，会话重放也不能缺掉失败的那一步。每份结果都要靠调用标识与模型发起的那次调用配对。
- **带副作用的工具必须自己处理幂等和结果未知。** 写库、扣款或发送消息可能已经成功，随后才发生取消、断线或落日志失败。调用方不能因为没收到成功结果就盲目重试。
- **取消只能协作完成。** Context 被取消不等于执行体已经停止；工具不观察 Context 时，本包无法强杀同进程代码。超时、并行安全和资源释放要由工具实现与外层策略共同保证。
- **Tool Result 仍会被模型解释。** 外部网页、搜索结果和第三方返回值可能包含错误信息或提示词注入。业务工具应通过 `Output.Render` 缩小输出、标清来源和可信度；绝对不能让模型看到的数据不要放进结果。
- **说明文字与执行契约要一起版本化。** 名称、描述、参数 Schema、输出 Schema 和执行行为共同组成一件工具。在线编辑说明或内部 Prompt 时，仍要保留版本、校验、发布和回滚机制，避免文字已经变化而代码行为没有变化。
...
[作用域](scope.md) · [Agent](agent.md) · [用户交互](interaction.md) · [运行时 Guard](guards.md) · [计划与待办](planning.md) · [MCP 客户端](mcp.md) · [运行预算与权限](../runtime-budgets-and-permissions.md)
