...
记录集的每一条都随身带一个修订号，写入可以附一个前置条件。上层拿它挡多副本之间的丢更新，细节见[存储、文件与附件](storage.md)；这一层只负责把三种条件落成语句：

```text
不带条件      -> 写下去，修订号 +1

必须不存在    -> 落点空着才写；已经有东西了 -> 不写，报冲突

修订号得是这个 -> 落点上的号还是这个才写，同时 +1
                 号已经变了 -> 不写，报冲突
```
...
| 写完回修订号 | `RETURNING` | `RETURNING`，需 3.35 以上 |
...
本模块自己认得出来的那几种有哨兵：流不在、头冲突、版面版本不符、名字不合法、介质里的值坏了、写的前置条件不成立、已经关掉、同名单元已打开。连不上库、事务被中止、死锁重试没有分类码，原样往上冒，只裹一层说明是哪一步。
