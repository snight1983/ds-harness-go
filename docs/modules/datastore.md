...
```mermaid
flowchart TD
    A["从最老那头弹掉一段"] --> A1["那条流还在"]
    A1 --> A2["还答得出「下一条写在哪儿」"]
    A2 --> A3["读的一方靠起点<br/>认出前面那截没了"]
    B["删掉整条流"] --> B1["这条流不存在了"]
    B1 --> B2["再问它得到「流不在」"]
    B2 --> B3["同名流重新建得起来<br/>序号从头数起"]
    C{"拿「弹到末尾」<br/>去顶删除"} -.->|"会留下一条空流<br/>而空流是合法的、还能接着写的"| C
```

删掉整条走一个事务，两句删按「先条目、后流」的次序。外键上挂着级联，但那句级联在各方言上的开关不一样，所以条目自己删一遍——不把正确性押在一个由连接串决定的设置上。

流不在时删除**报错**，不当成幂等成功：调用方要靠它把「删掉了一份」和「本来就没有」分开。
...
| `adapter/datastore/logs.go` | 日志集：流、条目、追加、弹出、整条删掉 |
