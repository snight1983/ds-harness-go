...
```mermaid
flowchart TD
    A["部署级默认<br/>core/agentdefaultmodel"] --> A2["建 agent 的时候<br/>没指定就用它"]
    B["单 agent 运行期切换<br/>core/agent"] --> B2["这个 agent 跑着跑着<br/>下一步换一个"]
    A2 -.互不覆盖.- B2
```
...
有一处确实空着：`example/minimalhost` 是本仓库唯一的示例宿主，它建 agent 时把提供方和模型直接硬传进去，没有演示这条线该怎么装。
...
| 路径 | 内容 | 行数 |
|---|---|---|
| `core/agentdefaultmodel/config.go` | 服务、装配入口、两层来源、读与存 | 208 |
| `core/agentdefaultmodel/doc.go` | 包说明与移植决定 | 64 |
| `core/agentdefaultmodel/invariant.go` | 在不变量注册表里占的那个名字 | 18 |
