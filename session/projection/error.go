...
var (
	...
	ErrInvalidDefinition = errors.New("session/projection: 单元定义不成立")

	// ErrStateVersionConflict 表示这个键已经被登记过，但版本号和这次的不一样。
	//
	// 该做的事：两个登记方对同一个键的折叠语义有分歧，共用它就等于让其中一方
	// 读到另一方算出来的状态。改代码：要么统一版本号，要么换一个键。
	ErrStateVersionConflict = errors.New("session/projection: 同一个投影键被登记成了两个不同的状态版本")
	...
	ErrCheckpointUnusable = errors.New("session/projection: 检查点行用不了，得从 seq 0 重读整份日志")
)
