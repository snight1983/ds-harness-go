...
type Figures struct {
	...
	// Steps 是关掉的步骤数（[session.EventStepEnd] 的条数）——跑完的、失败的、
	// 被取消的一律各算一个。
	Steps int `json:"steps"`

	// LLMMs 是模型墙上时间的总和，毫秒：每个装配出了消息的步骤，
	// 从 [session.EventStepStart] 到 [session.EventAssistantMessage] 那一段。
	LLMMs int64 `json:"llmMs"`

	// ToolMs 是工具墙上时间的总和，毫秒：按 callId 配上对的
	// [session.EventToolCall] → [session.EventToolResult] 那一段。
...
type OpenStep struct {
	...
	// Step 是这个步骤号。
	Step int `json:"step"`
	// StartTime 是 [session.EventStepStart] 那条事件的墙上时刻。
...
type State struct {
	...
	// LastTurn 是最后一个被数进去的 [session.EventStepEnd] 所属的回合号；
	// nil 表示还没数过任何步骤。
	//
...
func (s State) Validate() error {
	...
	for _, item := range nonNegative {
		if item.value < 0 {
			return fmt.Errorf("session/stats: %s 不能是负数，给的是 %d", item.name, item.value)
		}
	}

	if s.LastTurn != nil && *s.LastTurn < 0 {
		return fmt.Errorf("session/stats: lastTurn 不能是负数，给的是 %d", *s.LastTurn)
	}

	if open := s.OpenStep; open != nil {
		switch {
		case open.Turn < 0:
			return fmt.Errorf("session/stats: openStep.turn 不能是负数，给的是 %d", open.Turn)
		case open.Step < 0:
			return fmt.Errorf("session/stats: openStep.step 不能是负数，给的是 %d", open.Step)
		case open.StartTime < 0:
			return fmt.Errorf("session/stats: openStep.startTime 不能是负数，给的是 %d", open.StartTime)
		case open.FirstTokenTime != nil && *open.FirstTokenTime < 0:
			return fmt.Errorf("session/stats: openStep.firstTokenTime 不能是负数，给的是 %d",
				*open.FirstTokenTime)
		}
	}
	...
	for callID, dispatched := range s.PendingCalls {
		if dispatched < 0 {
			return fmt.Errorf("session/stats: pendingCalls[%q] 不能是负数，给的是 %d", callID, dispatched)
		}
	}
	return nil
}
...
func apply(state State, event session.Event) (State, bool) {
	switch event.Type {
	case session.EventStepStart:
		data, ok := decode[session.StepStartData](event)
		if !ok {
			return state, false
		}
	...
	case session.EventAssistantChunk:
		data, ok := decode[session.AssistantChunkData](event)
		if !ok {
			return state, false
		}
		open := state.OpenStep
	...
	case session.EventAssistantMessage:
		data, ok := decode[session.AssistantMessageData](event)
		if !ok {
			return state, false
		}
		open := state.OpenStep
	...
	case session.EventToolCall:
		data, ok := decode[session.ToolCallData](event)
		if !ok {
			return state, false
		}
	...
	case session.EventToolResult:
		data, ok := decode[session.ToolResultData](event)
		if !ok {
			return state, false
		}
	...
	case session.EventStepEnd:
		data, ok := decode[session.StepEndData](event)
		if !ok {
			return state, false
		}
...
func decode[T any](event session.Event) (T, bool) {
	payload := event.Data
	if len(payload) == 0 {
...
// 验非负。Go 这边前两样由 [session.AssistantMessageData.Usage] 的类型管住了，
