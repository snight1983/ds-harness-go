...
type Options struct {
	...
	// 断言不了。ledger 记录用的是源事件自己的 [session.Event.Time]，不走这里。
	Now func() int64
...
type Coordinator struct {
	...
	adopted map[session.SessionID]struct{}
	...
	cursor map[session.SessionID]int
	// chunkSeen 是每个会话里第一条分块已经交出去了的那些 "turn:step" 键。
	chunkSeen map[session.SessionID]map[string]struct{}
	closed    bool
}
...
func New(options Options) (*Coordinator, error) {
	if options.Sink == nil {
		return nil, fmt.Errorf("session/telemetry: 建协调器需要一个接收器")
	}
...
// 提交事件的那条路跟着失败。
func (c *Coordinator) Observe(view SessionView, event session.Event) {
...
func (c *Coordinator) Close(ctx context.Context) error {
	...
	for _, id := range ids {
		if err := c.deliver(shutdownRecord(id, at)); err != nil {
			c.logger.Warn("session/telemetry: 退休记录交不出去",
	...
	if err != nil {
		c.logger.Warn("session/telemetry: 接收器排空失败", slog.Any("error", err))
	}
	return err
}
...
func (c *Coordinator) track(id session.SessionID, event session.Event) {
...
func (c *Coordinator) captureEvent(view SessionView, event session.Event) error {
	id := view.ID()
	if event.Type == session.EventAssistantChunk {
		key, ok := chunkKey(event)
		if !ok {
...
func (c *Coordinator) markChunk(id session.SessionID, key string) bool {
...
// 见本包文档第 6 条。
func (c *Coordinator) contain(id session.SessionID, what string, step func() error) {
	defer func() {
		if recovered := recover(); recovered != nil {
			c.logger.Warn("session/telemetry: 采集这一步炸了",
				slog.String("session", string(id)),
	...
	if err := step(); err != nil {
		c.logger.Warn("session/telemetry: 采集这一步没走完",
			slog.String("session", string(id)),
			slog.String("step", what),
			slog.Any("error", err))
	}
...
func shutdownRecord(id session.SessionID, at int64) Record {
	return Record{
		Channel:  ChannelOps,
		Time:     at,
...
func severityOf(event session.Event) Severity {
	...
	case session.EventTurnEnd:
		...
		if data.Reason != nil && data.Reason.TurnEndReasonKind() == session.ReasonError {
			return SeverityError
		}
		return SeverityInfo
...
// 属性表逐字段一致。
func identityOf(id session.SessionID, header session.SessionHeader, event session.Event) map[string]any {
	attributes := map[string]any{
		"session.id": string(id),
...
//
// 和 [session.Event.MarshalJSON] 同一条规则：每条事件都有负载，空负载是 `{}`。
// 这里补齐一次，body 就永远是一段合法 JSON，[Record.Validate] 也就不会因为
// 一条本来没有负载的事件而把它扣下。
func payloadOf(event session.Event) json.RawMessage {
	if len(event.Data) == 0 {
...
func chunkKey(event session.Event) (string, bool) {
