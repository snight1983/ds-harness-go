...
var (
	...
	// 提示升级运行时，绝不要提示「文件损坏」。
	ErrFormatUnsupported = errors.New("session/persistence: 本构建没法忠实解释这份日志")
	...
	// 也有一条统一的说法可用。
	ErrMalformedSeq = errors.New("session/persistence: seq 水位不许是负的")

	// ErrRawArtifactsUnsupported 表示这个后端不提供逐会话的原始存档。
	//
	// 一个把所有会话装进同一个数据库的后端就没有「这个会话那份文件」可给。
	// 调用方先问 [Store.SupportsRawArtifacts]，问过了还调就得到这条。
	ErrRawArtifactsUnsupported = errors.New("session/persistence: 这个后端不提供逐会话的原始存档")
)
...
// errors.Is(err, session.ErrTraceViolation) 两问都能命中——调用方既要分辨
...
// 绝不是「日志损坏」。
func FormatVersionRefusal(id session.SessionID, version int) string {
	if version > session.FormatVersion {
		return fmt.Sprintf(
			"会话 %q 的日志格式是 v%d，而本运行时只读 v%d：这份日志是更新的运行时写的，升级之后才能打开它",
			string(id), version, session.FormatVersion,
		)
	}
	return fmt.Sprintf(
