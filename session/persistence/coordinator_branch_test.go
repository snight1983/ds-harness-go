...
// 后缀才真的是空的。
func endSeed(t *testing.T, seq int) session.Event {
	t.Helper()

	return session.Event{Type: session.EventSessionEndSeed, Seq: seq, Time: int64(seq)}
}
...
func TestCoordinator入册时读存档读不动(t *testing.T) {
	...
	live := h.createLive(t, session.SessionID("入册读不动"), coresession.CreateOptions{})
	if err := h.flush(live); !errors.Is(err, boom) {
		t.Fatalf("该原样交回那条读错，拿到 %v", err)
	}
}
...
func TestCoordinator把准备好的会话发布出去(t *testing.T) {
	...
	h := newHarness(t)
	id := session.SessionID("发布准备好的")
	// 存档已经以封种标记结尾，那个待发布的会话不会再补一条——于是它的日志和
	// 存档一样长，要补的后缀是空的。
	h.backend.seed(testHeader(t, id), []session.Event{userEvent(t, 0, "甲"), endSeed(t, 1)}, nil)
...
func TestCoordinator准备成果和持久化状态对不上(t *testing.T) {
	...
	// 就等于把两段不同的历史缝在一起。
	other, err := h.sessions.PrepareRestored(session.SessionID("别的"), coresession.RestoreOptions{
		Seed:   []session.Event{userEvent(t, 0, "甲")},
		Header: testHeader(t, session.SessionID("别的")),
	})
	if err != nil {
...
func TestCoordinator准备时修复写不下去(t *testing.T) {
	...
	h := newHarness(t)
	id := session.SessionID("修不了")
	h.backend.seed(testHeader(t, id), []session.Event{userEvent(t, 0, "甲")}, "坏尾巴")
...
func TestCoordinator查看时冷读读不动(t *testing.T) {
	...
	if _, err := h.Inspect(t.Context(), session.SessionID("查看读不动")); !errors.Is(err, boom) {
		t.Fatalf("该原样交回那条读错，拿到 %v", err)
	}
}
...
func TestCoordinator后台写撞上入册的错时只记日志(t *testing.T) {
	...
	live, err := store.Create(t.Context(), owner, session.SessionID("后台写失败"), coresession.CreateOptions{})
	if err != nil {
