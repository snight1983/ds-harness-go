...
func (b *trimmingBackend) TrimBefore(_ context.Context, id session.SessionID, beforeSeq int) error {
	b.mutex.Lock()
	...
	kept := make([]session.Event, 0, len(log.events))
...
// 一次写完的话，上限只会被跨过一次。
func fillTo(t *testing.T, h *harness, id session.SessionID, count int) {
	t.Helper()
