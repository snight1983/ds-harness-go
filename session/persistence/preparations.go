...
// [github.com/snight1983/ds-harness-go/core/session.Store] 那条一模一样：这个顺序**是语义**，
// 淘汰谁全看它。
...
// 那份扔掉。
func (p *preparations) discardReady(id session.SessionID, expected *preparedSource) discardOutcome {
	p.mutex.Lock()
	defer p.mutex.Unlock()
	entry, ok := p.entries[id]
...
// 抢输了的那一次会自己再来一遍。
func (p *preparations) entryFor(id session.SessionID, load preparationLoader) *preparationEntry {
	p.mutex.Lock()
...
func (p *preparations) removeLocked(entry *preparationEntry) {
	...
	p.order = slices.DeleteFunc(p.order, func(id session.SessionID) bool { return id == entry.id })
	settleLocked(entry)
}
