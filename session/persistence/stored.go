...
func CheckStoredIdentity(id session.SessionID, meta session.SessionHeader) error {
	if meta.ID == id {
		return nil
	}
	return fmt.Errorf(
...
func CheckStoredVersion(meta session.SessionHeader) error {
	if meta.Version == session.FormatVersion {
		return nil
	}
...
//
// 判据本身在 [session.CheckVocabulary]，这里做的是把它那条哨兵错误翻成
...
func CheckStoredVocabulary(
	...
	err := session.CheckVocabulary(events, vocabulary)
	if err == nil {
		return nil
	}
	return &FormatUnsupportedError{
...
func locateRefusal(backend Backend, meta session.SessionHeader, err error) error {
...
//
// 补出来的那几条是**确定的**（见 [session.InterruptedTurnClosers]），
// 所以这个函数跑两遍得到同样的字节——崩溃修复因此可以重跑。
func BalanceStored(events []session.Event) (balanced, closers []session.Event, err error) {
	...
	balanced = make([]session.Event, 0, len(events)+len(closers))
...
func sameEventBytes(a, b session.Event) (bool, error) {
