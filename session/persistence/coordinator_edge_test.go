...
func unknownEvent(t *testing.T, seq int) session.Event {
	t.Helper()
	...
	return session.Event{Type: session.EventType("未来/没见过"), Seq: seq, Time: int64(seq), Data: payload}
}
...
func TestCoordinator跳读回来的东西不成立就被拒(t *testing.T) {
	...
	cases := []struct {
		...
		{"身份对不上", func(t *testing.T, b *seekableMemory) {
			meta := testHeader(t, session.SessionID("别人的"))
		...
		{"词汇不认得", func(t *testing.T, b *seekableMemory) {
			b.suffix = &StoredSuffix{
				Meta:   testHeader(t, id),
				Events: []session.Event{unknownEvent(t, 0)},
			}
		}},
	}
...
func TestCoordinator读后缀的那几种拦法(t *testing.T) {
	...
	// 存档过不了那三道判据时，读也该拒掉。
	broken := session.SessionID("词汇不认得")
	h.backend.seed(testHeader(t, broken), []session.Event{unknownEvent(t, 0)}, nil)
	...
	good := session.SessionID("越界")
	h.backend.seed(testHeader(t, good), []session.Event{userEvent(t, 0, "甲")}, nil)
...
func TestCoordinator几条读路在ctx断掉时都停下(t *testing.T) {
	...
	if err := h.Create(ctx, testHeader(t, session.SessionID("断掉时登记"))); !errors.Is(err, context.Canceled) {
		t.Fatalf("登记该停下，拿到 %v", err)
	}
	if err := h.Append(ctx, id, []session.Event{userEvent(t, 1, "乙")}); !errors.Is(err, context.Canceled) {
...
func TestCoordinator准备一份自己不成立的存档会被裹成损坏(t *testing.T) {
	...
	h := newHarness(t)
	id := session.SessionID("坏掉的")
	h.backend.seed(testHeader(t, id), []session.Event{userEvent(t, 0, "甲"), userEvent(t, 5, "乙")}, nil)
...
func TestCoordinator词汇不认得也是版本级的拒绝(t *testing.T) {
	...
	h := newHarness(t)
	id := session.SessionID("词汇新")
	h.backend.seed(testHeader(t, id), []session.Event{unknownEvent(t, 0)}, nil)
...
func TestCoordinator登记时后端读不动(t *testing.T) {
	...
	if err := h.Create(t.Context(), testHeader(t, session.SessionID("读不动"))); !errors.Is(err, boom) {
...
func TestCoordinator追加到一份坏掉的存档上(t *testing.T) {
	...
	h := newHarness(t)
	id := session.SessionID("认领坏的")
	h.backend.seed(testHeader(t, id), []session.Event{userEvent(t, 0, "甲"), userEvent(t, 5, "乙")}, nil)

	// 认领要先读一遍，读出来的东西不成立就到此为止。
	if err := h.Append(t.Context(), id, []session.Event{userEvent(t, 2, "丙")}); err == nil {
...
func TestCoordinator排干失败时关闭的错要让位(t *testing.T) {
	...
	live, err := store.Create(t.Context(), owner, session.SessionID("排不干"), coresession.CreateOptions{
...
func TestCoordinator退场刷不下去就不划掉状态(t *testing.T) {
	...
	owner := scope.NewRoot()
	live, err := store.Create(t.Context(), owner, session.SessionID("退不掉"), coresession.CreateOptions{})
	if err != nil {
		t.Fatalf("造不出活会话：%v", err)
	}
...
func TestCoordinator没登记过的会话退场是空操作(t *testing.T) {
	...
	h := newHarness(t)
	live, err := h.sessions.PrepareRestored(session.SessionID("没登记"), coresession.RestoreOptions{
		Seed:   []session.Event{userEvent(t, 0, "甲")},
		Header: testHeader(t, session.SessionID("没登记")),
	})
	if err != nil {
...
func TestCoordinator收编时那几道判据(t *testing.T) {
	...
	cases := []struct {
		name  string
		setup func(t *testing.T, h *harness, id session.SessionID)
	}{
		{"存档里的身份对不上", func(t *testing.T, h *harness, id session.SessionID) {
			meta := testHeader(t, session.SessionID("别人的"))
			meta.ID = session.SessionID("别人的")
			h.backend.mutex.Lock()
			h.backend.logs[id] = &memoryLog{meta: meta, events: cloneEvents(seed), revision: 1}
			h.backend.mutex.Unlock()
		}},
		{"版本不认得", func(t *testing.T, h *harness, id session.SessionID) {
			meta := testHeader(t, id)
			meta.Version = session.FormatVersion + 1
			h.backend.seed(meta, seed, nil)
		}},
		{"词汇不认得", func(t *testing.T, h *harness, id session.SessionID) {
			h.backend.seed(testHeader(t, id), []session.Event{unknownEvent(t, 0)}, nil)
		}},
		{"修复写不下去", func(t *testing.T, h *harness, id session.SessionID) {
...
func TestCoordinator这个身份已经绑在另一个活会话上(t *testing.T) {
	...
	h := newHarness(t)
	h.drainMayFail = true
	id := session.SessionID("两个活的")

	// 会话存储本身不让同一个身份出现两个活会话，所以这里直接把册子摆成
	// 「已经有一个主人、而且它已经落过盘」的样子——那正是编排器要拦的局面。
	other, err := h.sessions.PrepareRestored(session.SessionID("另一个身份"), coresession.RestoreOptions{
		Seed:   []session.Event{userEvent(t, 0, "甲")},
		Header: testHeader(t, session.SessionID("另一个身份")),
	})
	if err != nil {
