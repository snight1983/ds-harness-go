...
// 用例里都不必真去恢复一个会话。
func sourceFor(id session.SessionID, revision Revision) *preparedSource {
	return &preparedSource{
		inspection: Inspection{Meta: session.SessionHeader{
			Version: session.FormatVersion, ID: id, CreatedAt: 1,
		}},
		revision: revision,
	}
}
...
func TestPreparations放回和扔掉那两条岔路(t *testing.T) {
	...
	// 还能再用：放回就绪，下一个人摘得走。
	reusable := session.SessionID("还能用")
...
func TestPreparations没就绪条目时摘不走(t *testing.T) {
	...
	pool := newPreparations(5)
	if got := pool.takeReady(session.SessionID("没有")); got != nil {
...
func TestPreparations发布别名会被拒掉(t *testing.T) {
	...
	pool := newPreparations(5)
	id := session.SessionID("别名")

	live, err := sessions.PrepareRestored(id, coresession.RestoreOptions{
		Seed:   []session.Event{userEvent(t, 0, "甲")},
		Header: session.SessionHeader{Version: session.FormatVersion, ID: id, CreatedAt: 1},
	})
	if err != nil {
...
func TestPreparations就绪条目超编时淘汰最久没碰的(t *testing.T) {
	...
	first := session.SessionID("甲")
	second := session.SessionID("乙")
