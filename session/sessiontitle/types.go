...
const EventSessionTitle session.EventType = "session/title"
...
//
//	vocabulary := session.CoreVocabulary().With(sessiontitle.EventTypes()...)
//
// 不拼的话，一段带标题的日志会被 [session.CheckVocabulary] 判成
// 「有不认识的事件类型」而整个拒掉。
func EventTypes() []session.EventType {
	return []session.EventType{EventSessionTitle}
}
...
func decodeEventData(event session.Event) (EventData, error) {
	var data EventData
