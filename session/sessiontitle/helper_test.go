...
func (s *fakeSession) Header() session.SessionHeader { return s.header }
...
func (s *fakeSession) watchAppends() chan session.Event {
	...
	s.appended = make(chan session.Event, 32)
	return s.appended
}
...
// 而测试想看的是它后面那条。
func waitForTitle(t *testing.T, appended <-chan session.Event, kind SourceKind) EventData {
	t.Helper()
...
func messageEvent(t *testing.T, source llm.MessageSource, content llm.Content) session.Event {
	t.Helper()

	return session.Event{
		Type: session.EventUserMessage,
		Data: mustJSON(t, session.UserMessageData{Message: llm.Message{
			ID:      "m",
			Role:    llm.RoleUser,
			Content: content,
			Source:  source,
...
// titleEvent 造一条标题事件。
func titleEvent(t *testing.T, data EventData) session.Event {
	t.Helper()

	return session.Event{Type: EventSessionTitle, Data: mustJSON(t, data)}
}

// headerEvent 造一条钉住路由的 request/header。
func headerEvent(t *testing.T, provider, model string) session.Event {
	t.Helper()

	return session.Event{
		Type: session.EventRequestHeader,
		Data: mustJSON(t, session.RequestHeaderData{
