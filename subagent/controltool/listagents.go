...
type ListingService interface {
	...
	ListChildren(ctx context.Context, parentSessionID session.SessionID) ([]subagent.ListEntry, error)
	// ListDescendants 按稳定的先序列举一个根底下完整的那棵树。
	ListDescendants(ctx context.Context, rootSessionID session.SessionID) ([]subagent.DescendantListEntry, error)
}
...
// 的终局结果。
func statusOf(agents Agents, id session.SessionID) string {
