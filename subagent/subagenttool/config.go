...
//
// 源: packages/subagent/tool-subagent/src/index.ts:22
...
// 没有空位，所以这里取 117，并把
// [ds-harness-go/subagent/reporttool.SectionOrder] 顺推到 118。次序这件事只有
...
const (
	...
	//
	// 它要提供方实现 [ds-harness-go/subagent/subagent.ContinuablePreparer]，
	// 装不上就大声失败。
...
// [ds-harness-go/subagent/controltool.Service]）。
...
type Config struct {
	...
	//
	// 这里**查不回来就是错**（和 [ds-harness-go/jobs/jobstool.Config.AgentOf]
	...
	// [ds-harness-go/core/tools.Options.Logger]。
	Logger *slog.Logger
	...
	// 默认行为（露出后台），成例见
	// [ds-harness-go/invariants.Config.Enabled] 那条说明里同一个问题的另一种解法。
