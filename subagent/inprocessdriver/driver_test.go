...
func TestStartInProcessRunCancelsThePublishedChild(t *testing.T) {
	...
	if _, ok := cancels[0].(session.ParentCancel); !ok {
...
func TestToStopReasonMapsTurnEndings(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		{name: "completed", reason: session.CompletedTurnEnd{}, want: subagent.StopCompleted},
		{name: "max tokens", reason: session.MaxTokensTurnEnd{}, want: subagent.StopMaxTokens},
		{
			name:   "aborted",
			reason: session.AbortedTurnEnd{Reason: session.ParentCancel{}},
			want:   subagent.StopAborted,
		},
		// 一次前置步骤的拒绝把认领下来的提示词丢掉了：这件活儿是被回绝了。
		{name: "blocked", reason: session.BlockedTurnEnd{}, want: subagent.StopRefusal},
		{
			name:   "error",
			reason: session.ErrorTurnEnd{Error: llm.Failure{Message: "炸了", Code: "provider_error"}},
			want:   subagent.StopError,
		},
		{name: "interrupted", reason: session.InterruptedTurnEnd{}, want: subagent.StopError},
	} {
		t.Run(testCase.name, func(t *testing.T) {
