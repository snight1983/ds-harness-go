...
func childOwnEvents(loaded persistence.Inspection) []session.Event {
...
func (m *ContinuationManager) acquireOwnership(parent agent.Agent, childID session.SessionID) error {
	m.mutex.Lock()
...
func (m *ContinuationManager) releaseOwnership(childID session.SessionID) {
	m.mutex.Lock()
...
func (m *ContinuationManager) startTeardown(target *activation, transaction *disposalTx) {
	...
	// 祖先继续跑模型或者工具。
	target.handle.Agent.Cancel(session.ParentCancel{}, agent.CancelOptions{})
