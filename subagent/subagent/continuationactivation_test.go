...
// 因此它的结清是父应得的一份交代。
func announcedActivation(childID, parentSession session.SessionID) *activation {
	return &activation{
		childID:       childID,
...
func TestChildOwnEventsSlicesOffTheInheritedPrefix(t *testing.T) {
	own := childOwnEvents(persistence.Inspection{
		Meta:   session.SessionHeader{SeedLength: 2},
		Events: []session.Event{{Seq: 0}, {Seq: 1}, {Seq: 2}},
	})
...
func TestChildOwnEventsClampsABoundaryFromTheMedium(t *testing.T) {
	events := []session.Event{{Seq: 0}, {Seq: 1}}

	if own := childOwnEvents(persistence.Inspection{
		Meta:   session.SessionHeader{SeedLength: -5},
		Events: events,
	}); len(own) != 2 {
		t.Fatalf("负的边界该当成 0，实际 %#v", own)
	}
	if own := childOwnEvents(persistence.Inspection{
		Meta:   session.SessionHeader{SeedLength: 99},
		Events: events,
	}); len(own) != 0 {
...
func TestColdResumeReportsCancellationFromTheArchiveProbe(t *testing.T) {
	...
	fixture.store.onInspect = func(session.SessionID) { cancel() }
...
func TestColdResumeRechecksAdmissionAfterReadingTheArchive(t *testing.T) {
	...
	parent := fixture.spawnParent(t, "parent", "")
	fixture.persistChild(t, "child", "parent", continuableInput())
	fixture.store.onInspect = func(session.SessionID) {
...
func TestColdResumeSurfacesAnUnreadableDescriptorRecord(t *testing.T) {
	...
	fixture.store.put(session.SessionHeader{
		...
		DelegationDepth: 1,
	}, session.Event{Seq: 0, Type: EventDescriptor, Data: json.RawMessage(`这不是 JSON`)})
