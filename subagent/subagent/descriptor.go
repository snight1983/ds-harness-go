...
const EventDescriptor session.EventType = "subagent/descriptor"
...
//
// 不拼的话，一段子 agent 的日志会被 [session.CheckVocabulary] 判成
// 「有不认识的事件类型」而整个拒掉。
func EventTypes() []session.EventType {
	return []session.EventType{EventDescriptor}
}
...
func FoldDescriptor(events []session.Event) (data DescriptorData, found bool, err error) {
