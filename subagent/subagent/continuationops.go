...
func (m *ContinuationManager) StartContinuable(
	...
	if childID == "" {
		childID = session.SessionID(uuid.NewString())
	}
...
func (m *ContinuationManager) assertChildIDAvailable(childID session.SessionID) error {
...
func (m *ContinuationManager) Interrupt(
	...
	var cause session.TurnEndCancelCause = session.ParentCancel{}
	if authority.Kind == AuthorityUser {
		cause = session.UserCancel{}
	}
