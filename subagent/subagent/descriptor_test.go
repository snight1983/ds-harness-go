...
func TestFoldDescriptorFindsNothingWithoutARecord(t *testing.T) {
	_, found, err := FoldDescriptor(steppedTurn(t, 1, session.CompletedTurnEnd{}))
	if err != nil || found {
...
func TestFoldDescriptorTreatsOtherVersionsAsUnclassifiable(t *testing.T) {
	...
	_, found, err := FoldDescriptor([]session.Event{descriptorEvent(t, payload)})
	if err != nil || found {
...
func TestFoldDescriptorRejectsMalformedRecords(t *testing.T) {
	unreadable := session.Event{Type: EventDescriptor, Data: json.RawMessage(`{"version":`)}
	if _, _, err := FoldDescriptor([]session.Event{unreadable}); !errors.Is(err, ErrInvalidRequest) {
		t.Fatalf("读不出来的记录该报错，实际 %v", err)
	}

	noVersion := descriptorEvent(t, map[string]any{"mode": ModeOneShot, "provider": "spawn"})
	if _, _, err := FoldDescriptor([]session.Event{noVersion}); !errors.Is(err, ErrInvalidRequest) {
...
func TestFoldDescriptorRejectsUnknownFields(t *testing.T) {
	...
	if _, _, err := FoldDescriptor([]session.Event{topLevel}); !errors.Is(err, ErrInvalidRequest) {
	...
	if _, _, err := FoldDescriptor([]session.Event{nested}); !errors.Is(err, ErrInvalidRequest) {
...
func TestFoldDescriptorRejectsIncompleteCurrentVersionRecords(t *testing.T) {
	...
	if _, _, err := FoldDescriptor([]session.Event{incomplete}); !errors.Is(err, ErrInvalidRequest) {
