...
func (f *continuationFixture) persistChild(
	t *testing.T,
	childID, parent session.SessionID,
...
// 留着的话整个测试进程会挂死在清理里。
func (f *continuationFixture) feignDisposal(t *testing.T, childID session.SessionID) {
	t.Helper()
...
func TestStartContinuableCarriesTheLineageBoundary(t *testing.T) {
	fixture := newContinuation(t)
	inherited := []session.Event{assistantMessage(t, 1, textContent("祖上说过的话"))}
	fixture.host.prepare = func(context.Context, string, ContinuableCreateRequest) (ContinuableCreateSpec, error) {
	...
	if created[0].Origin != session.OriginSubagent || created[0].DelegationDepth != 1 {
...
func TestStartContinuableStopsWhenTheProviderSeedCannotBeStaged(t *testing.T) {
	...
	broken.Seq = 7
	fixture.host.prepare = func(context.Context, string, ContinuableCreateRequest) (ContinuableCreateSpec, error) {
		return ContinuableCreateSpec{Seed: []session.Event{broken}}, nil
	}
...
func TestInterruptStopsTheCurrentActivityForAUser(t *testing.T) {
	...
	if _, byUser := causes[0].(session.UserCancel); !byUser {
...
func TestInterruptChecksTheRecordedLineageForAnAncestor(t *testing.T) {
	...
	if _, byParent := causes[0].(session.ParentCancel); !byParent {
