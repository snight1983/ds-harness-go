...
func TestSeedDescriptorTurnAppendsTheRecordToAnInheritedPrefix(t *testing.T) {
	inherited := []session.Event{
		{Seq: 0, Type: session.EventTurnStart, Data: data(t, session.TurnStartData{Turn: 1})},
		{Seq: 1, Type: session.EventTurnEnd, Data: data(t, session.TurnEndData{
	...
	if boundary := seed[len(inherited)]; boundary.Type != session.EventSessionEndSeed {
...
func TestSeedDescriptorTurnRejectsAMalformedPrefix(t *testing.T) {
	broken := []session.Event{
		{Seq: 7, Type: session.EventTurnStart, Data: data(t, session.TurnStartData{Turn: 1})},
	}
