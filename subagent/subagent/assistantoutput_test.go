...
func TestFinalAssistantOutputFallsBackToStreamedText(t *testing.T) {
	events := []session.Event{
		event(t, session.EventAssistantChunk, session.AssistantChunkData{
...
func TestFinalAssistantOutputIsNilWhenNothingWasProduced(t *testing.T) {
	...
	if output := FinalAssistantOutput(steppedTurn(t, 1, session.CompletedTurnEnd{})); output != nil {
...
func TestFinalAssistantOutputSkipsUndecodablePayloads(t *testing.T) {
	events := []session.Event{
		...
		{Type: session.EventAssistantMessage, Data: json.RawMessage(`{"turn":`)},
		{Type: session.EventAssistantChunk, Data: json.RawMessage(`not json`)},
	}
...
func TestAssistantOutputFoldIgnoresNonTextChunks(t *testing.T) {
	...
	fold.Push(event(t, session.EventAssistantChunk, session.AssistantChunkData{
...
func TestAssistantOutputFoldIgnoresUnrelatedEvents(t *testing.T) {
	var fold AssistantOutputFold
	fold.Push(session.Event{Type: "test/unrelated", Data: json.RawMessage(`{}`)})
	if output := fold.Collect(); output != nil {
