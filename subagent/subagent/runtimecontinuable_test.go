...
func (w *wiredRuntime) parent(t *testing.T, id session.SessionID) *childAgent {
	t.Helper()
...
func (w *wiredRuntime) start(t *testing.T, parent agent.Agent, childID session.SessionID) ContinuableStart {
	t.Helper()
...
func TestRuntimeHandsContinuableWorkToTheManager(t *testing.T) {
	...
	if err := wired.runtime.DrainContinuableChildren(
		t.Context(), parent, []session.SessionID{"child"},
	); err != nil {
