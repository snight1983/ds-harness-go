...
type fakePersistence struct {
	...
	listed []session.SessionHeader
	// stored 是每个会话在存档里那份逻辑视图。
	stored map[session.SessionID]persistence.Inspection
	...
	onInspect func(session.SessionID)
	// inspected 是历次探视的目标，按调用顺序。
	inspected []session.SessionID
}
...
func (p *fakePersistence) put(header session.SessionHeader, events ...session.Event) {
...
func (p *fakePersistence) putStored(listed, stored session.SessionHeader, events ...session.Event) {
	p.listed = append(p.listed, listed)
	p.stored[listed.ID] = persistence.Inspection{Meta: stored, Events: events}
}

// probes 给出历次探视的目标。
func (p *fakePersistence) probes() []session.SessionID {
	p.mu.Lock()
	defer p.mu.Unlock()

	return append([]session.SessionID(nil), p.inspected...)
}
...
func (p *fakePersistence) List(context.Context) ([]session.SessionHeader, error) {
	...
	hook := p.onList
	err := p.listErr
	listed := append([]session.SessionHeader(nil), p.listed...)
	p.mu.Unlock()
...
func (p *fakePersistence) Inspect(_ context.Context, id session.SessionID) (persistence.Inspection, error) {
	p.mu.Lock()
	hook := p.onInspect
...
func (p *fakePersistence) Locate(session.SessionHeader) (persistence.Location, bool) {
	return persistence.Location{}, false
}

func (p *fakePersistence) SupportsRawArtifacts() bool { return false }

func (p *fakePersistence) ReadRaw(context.Context, session.SessionID) (persistence.RawArtifact, error) {
	return persistence.RawArtifact{}, errNoFakeMethod
}

func (p *fakePersistence) Create(context.Context, session.SessionHeader) error {
	return errNoFakeMethod
}

func (p *fakePersistence) Append(context.Context, session.SessionID, []session.Event) error {
	return errNoFakeMethod
}

func (p *fakePersistence) Load(context.Context, session.SessionID) (persistence.Inspection, error) {
	return persistence.Inspection{}, errNoFakeMethod
}

func (p *fakePersistence) ReadFrom(
	context.Context, session.SessionID, int,
...
func descriptorLog(t *testing.T, seq int, label string) []session.Event {
	t.Helper()
	input := continuableInput()
	input.Label = label
	built := event(t, EventDescriptor, input)
	built.Seq = seq
	return []session.Event{built}
}
...
func newCache(
	...
	rows map[session.SessionID]projectioncache.Record,
...
func idsOf(entries []ListEntry) []session.SessionID {
	ids := make([]session.SessionID, 0, len(entries))
...
func TestListChildrenOnlyReadsSubagentChildrenOfThisParent(t *testing.T) {
	store := newPersistence()
	store.put(coldHeader("mine", "parent", 10, session.OriginSubagent), descriptorLog(t, 0, "我的")...)
	store.put(coldHeader("someone-else", "别的父", 20, session.OriginSubagent),
...
func TestListChildrenOrdersByCreationThenID(t *testing.T) {
	...
	store.put(coldHeader("b", "parent", 20, session.OriginSubagent), descriptorLog(t, 0, "b")...)
	store.put(coldHeader("a", "parent", 20, session.OriginSubagent), descriptorLog(t, 0, "a")...)
	store.put(coldHeader("early", "parent", 5, session.OriginSubagent), descriptorLog(t, 0, "早")...)
	services := newListing(t)
	services.Persistence = store
...
func TestListChildrenPrefersTheLiveRecordOverThePersistedOne(t *testing.T) {
	services := newListing(t)
	store := newPersistence()
	store.put(coldHeader("child", "parent", 10, session.OriginSubagent),
...
func TestListChildrenMarksAParentThatItselfHasSubagentChildren(t *testing.T) {
	store := newPersistence()
	store.put(coldHeader("child", "parent", 10, session.OriginSubagent), descriptorLog(t, 0, "孩子")...)
	store.put(coldHeader("grandchild", "child", 20, session.OriginSubagent),
...
func TestListChildrenReportsACorruptChildWhenNoIdentityFolds(t *testing.T) {
	store := newPersistence()
	for name, log := range map[string][]session.Event{
		"一条描述符都没有": nil,
		"描述符坏了": {{
			Seq:  0,
		...
		store.put(coldHeader(session.SessionID(name), "parent", 10, session.OriginSubagent), log...)
	}
	services := newListing(t)
	services.Persistence = store
...
func TestListChildrenTakesTheCachedIdentityWhenItPassesTheSeqGate(t *testing.T) {
	header := coldHeader("child", "parent", 10, session.OriginSubagent)
	header.SeedLength = 3
	store := newPersistence()
	store.put(header, descriptorLog(t, 3, "重折出来的")...)
	services := newListing(t)
	services.Persistence = store
	services.Cache = newCache(t, services.Projections, map[session.SessionID]projectioncache.Record{
...
func TestListChildrenReportsCancellationFromAColdProbe(t *testing.T) {
	for name, probeFails := range map[string]bool{"探视成功": false, "探视失败": true} {
		t.Run(name, func(t *testing.T) {
			...
			store.onInspect = func(session.SessionID) { cancel() }
			services := newListing(t)
			services.Persistence = store
...
func TestListDescendantsWalksThePreOrderWithDepths(t *testing.T) {
	store := newPersistence()
	store.put(coldHeader("a", "root", 10, session.OriginSubagent), descriptorLog(t, 0, "a")...)
	store.put(coldHeader("a1", "a", 20, session.OriginSubagent), descriptorLog(t, 0, "a1")...)
	store.put(coldHeader("plain", "root", 30, ""), descriptorLog(t, 0, "普通")...)
	store.put(coldHeader("under-plain", "plain", 40, session.OriginSubagent),
		descriptorLog(t, 0, "藏在普通会话底下")...)
	store.put(coldHeader("elsewhere", "别的根", 50, session.OriginSubagent),
...
func TestListDescendantsGuardsAgainstACycle(t *testing.T) {
	store := newPersistence()
	store.put(coldHeader("root", "root", 10, session.OriginSubagent), descriptorLog(t, 0, "自己指自己")...)
	store.put(coldHeader("child", "root", 20, session.OriginSubagent), descriptorLog(t, 0, "孩子")...)
	services := newListing(t)
	services.Persistence = store
...
func TestListDescendantsReportsCancellationRaisedBeyondTheColdReadLimit(t *testing.T) {
	...
	for index := range coldReadConcurrency + 2 {
		id := session.SessionID(fmt.Sprintf("child-%d", index))
		store.put(coldHeader(id, "root", int64(10+index), session.OriginSubagent),
			descriptorLog(t, 0, "冷的")...)
	}
	store.onInspect = func(session.SessionID) { cancel() }
	services := newListing(t)
