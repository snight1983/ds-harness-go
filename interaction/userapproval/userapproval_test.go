...
func (l *fakeLog) Events() []session.Event { return l.events }
...
func (l *fakeLog) Append(kind session.EventType, data any) error {
	...
	l.events = append(l.events, session.Event{Type: kind, Data: raw})
	return nil
}
...
func openTurn() []session.Event {
	return []session.Event{{Type: session.EventTurnStart}, {Type: session.EventUserMessage}}
}
...
func newHarness(t *testing.T, config userapproval.Config, seed []session.Event) *harness {
	t.Helper()
...
// eventOf 造一条带负载的事件。
func eventOf(t *testing.T, kind session.EventType, data any) session.Event {
	t.Helper()
	raw, err := json.Marshal(data)
	if err != nil {
		t.Fatalf("造事件失败：%v", err)
	}
	return session.Event{Type: kind, Data: raw}
}
...
func TestRefusesAnAskBetweenTurns(t *testing.T) {
	...
	// 和一段崩溃残尾长得一模一样。
	h := newHarness(t, userapproval.Config{}, []session.Event{
		{Type: session.EventTurnStart}, {Type: session.EventTurnEnd},
	})
...
func TestPropagatesAnAppendFailureThatPreventedAuditLogGrowth(t *testing.T) {
	t.Parallel()
	for label, failOn := range map[string]session.EventType{
...
func TestPublishesItsOwnEventVocabulary(t *testing.T) {
	...
	for _, kind := range userapproval.EventTypes() {
		...
		// 拼之前不认得——这正是这张单子存在的理由。
		if session.CoreVocabulary().Knows(kind) {
...
func askedEvent(t *testing.T, id userapproval.RequestID, toolName string) session.Event {
	t.Helper()
	return eventOf(t, userapproval.EventAsked, userapproval.AskedData{ID: id, ToolName: toolName})
}

// decidedEvent 造一条 approval/decided。
func decidedEvent(t *testing.T, id userapproval.RequestID, outcome tools.ApprovalOutcome) session.Event {
	t.Helper()
	return eventOf(t, userapproval.EventDecided, userapproval.DecidedData{ID: id, Outcome: outcome})
}

// brokenEvent 造一条负载读不回来的事件。
func brokenEvent(kind session.EventType) session.Event {
	return session.Event{Type: kind, Data: json.RawMessage(`"not an object"`)}
}
...
func TestTheTraceAcceptsAPairedAuditAndAClosedPolicyValue(t *testing.T) {
	...
	trace, err := userapproval.ValidateLog([]session.Event{
...
func TestTheTraceIgnoresEventsThatAreNotItsOwn(t *testing.T) {
	...
	trace, err := userapproval.ValidateLog([]session.Event{
		...
		{Type: session.EventType("todo/snapshot"), Data: json.RawMessage(`{"todos":[]}`)},
	})
	if err != nil {
...
func TestTheTraceCarriesAnUnmatchedQuestionForwardAcrossInstallation(t *testing.T) {
	...
	h := newInvariantHarness(t,
		session.Event{Type: session.EventTurnStart},
		askedEvent(t, "ask-resume", "bash"),
	)
	h.register(t)

	h.emit(decidedEvent(t, "ask-resume", tools.ApprovalCancelled))
	h.emit(session.Event{Type: session.EventTurnEnd})
}
...
func TestTheTraceCatchesAnUnenclosedAuditAlreadyInTheLog(t *testing.T) {
	...
	h := newInvariantHarness(t,
		session.Event{Type: session.EventTurnStart},
		session.Event{Type: session.EventTurnEnd},
...
func TestTheTraceCatchesMalformedAndUnpairedAudits(t *testing.T) {
	...
	h := newInvariantHarness(t, session.Event{Type: session.EventTurnStart})
	h.register(t)
...
func TestTheTraceCatchesAnUnreadablePayload(t *testing.T) {
	...
	cases := map[session.EventType]string{
	...
	for kind, want := range cases {
		t.Run(string(kind), func(t *testing.T) {
			t.Parallel()
			_, err := userapproval.ValidateLog([]session.Event{
				{Type: session.EventTurnStart},
				brokenEvent(kind),
			})
