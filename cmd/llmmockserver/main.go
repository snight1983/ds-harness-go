...
// # 不做什么
//
//   - **不实现模拟行为本身。**正常流、断流、延迟、错误和请求快照都在 llm/mockserver
//     里，这个包只是把它包成一个进程：解析参数、绑端口、播报、转发事件、接住信号。
//   - **不做进程内回放。**同一个进程里按脚本喂响应是 feature/replay 的活，
//     那条路根本不走 HTTP。
//   - **不追求实现完整的 OpenAI API。**只覆盖本项目用得到的那一小片兼容表面。
//   - **不是生产模型代理。**它是测试和复现用的替身，真实的模型调用走 llm 和它的适配器。
//   - **不解释播报出去的那些行，也不给脚本脱敏。**断言由编排脚本和测试做，
//     存档前的脱敏由调用方做。
package main
