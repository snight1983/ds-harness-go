...
// 从 GitHub 路径引进来时会撞上 `package ds-harness-go/scope is not in std`。
// 那正是这个门禁存在的由头。
//
...
var namedImports = map[string]string{
	...
	"github.com/snight1983/ds-harness-go/sessionlog":          "sessionlog",
	"github.com/snight1983/ds-harness-go/feature/persistence": "persistence",
}
...
// 以及被 Git 忽略的临时目录（不是可发布的东西）。口径和 internal/devtools/doccheck 一致。
