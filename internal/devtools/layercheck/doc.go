// Command layercheck 是分层门禁：它保证「A 在 B 上面」这句话有一处会变红的地方。
//
...
// # 不做什么
//
//   - **不决定一个包该在哪一档。**档位是人写进 docs/layers.tsv 的判断，这里只负责
//     让那个判断变成一条会红的规则。
//   - **不按目录名判档。**把一个包挪进 feature/ 不会改变它的档位；表和目录不一致时
//     以表为准。
//   - **不看 import 进来之后用了什么。**它只认 import 边的方向，包内怎么组织、
//     引的是哪个符号，都不在它眼里。
//   - **不管仓库外的依赖。**只有 module path 前缀是本仓库的 import 才进判断。
//   - **不管文档、数据库和磁盘那三条界线。**分别归 internal/devtools/doccheck、
//     internal/devtools/dbcheck、internal/devtools/oscheck。
package main
