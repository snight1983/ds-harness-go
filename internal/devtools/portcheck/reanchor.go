...
const (
	// tierNone 没有锚点。
	//
	//lint:ignore U1000 零值要有名字：删掉它后面四级的 iota 全部错位一格
...
//
// 和 [reCitation] 分开写而不是复用，是因为这里要拿子匹配的**位置**去做字节替换，
// 所以路径必须用 `[^\s:]+` 精确框住而不能靠 `\S+` 的回溯——一个错一格的偏移
// 会把行号写到路径中间去，而那种损坏是编译不出错的。
//
// 括号锚点用全角括号，是本仓库既有的写法（`// 源: ….ts:315-316（allowedFor 那两行）`）。
//
// 三处刻意的窄：只认锚在行首的第一处引用，只认带行号的，一行引好几处时只重定位
// 第一处。仓库里一行引两处且两处都带行号的共 5 条，第二处不会被这里改写。
// 不补是因为改写是**就地改源码**，多一条替换路径就多一种把文件写坏的方式，
// 而门禁那一项（[checkProvenance]）验的是全部引用，漏的这几处出处错了照样报。
// 换句话说：这里少做的是「帮你把行号改对」，不是「替你把错的放过去」。
