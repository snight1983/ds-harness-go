...
// # 不做什么
//
//   - **不判断裁决对不对，也不验 GoRef 指的符号存不存在。**它只保证列没错位、
//     取值在 [ValidDecisions] 里；真伪由 internal/devtools/portcheck 验。
//   - **不产生行。**表里前六列来自 internal/devtools/portmap 抽出的清单，
//     它只负责把这些行读进来、写回去。
//   - **不是门禁。**它是个库，不退出进程也不打印判决；什么算失败由调用它的那两个
//     程序决定。
//   - **不替调用方处理定位歧义。**[Row.MatchKey] 和 [MatchIndex] 只给出稳定的配对
//     口径，「匹配到多行就报错退出」是 internal/devtools/rule 的规矩。
package rulingtable
