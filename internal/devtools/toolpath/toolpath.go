...
// # 不做什么
//
//   - **不校验快照的内容。**[DSHRoot] 只回答「路径在哪、存不存在」；那份快照是不是
//     基准版本、里面有没有东西，由用它的工具自己看。
//   - **不替调用方决定降级。**存不存在原样交回去，硬失败还是跳过溯源校验由调用方定
//     ——跳过是允许的，静默跳过不是。
//   - **不读写账本文件。**[PortmapFile] 只拼出一条绝对路径，读写口径在
//     internal/devtools/rulingtable。
//   - **不是门禁。**它不判红、不退出进程，只把「路径错了」和「移植漏了」分开，
//     免得前者被报成后者。
//   - **不给业务代码用。**它碰宿主机磁盘，靠的是 internal/devtools/oscheck 对
//     internal/devtools/ 的整体放行；业务包的内容读写走 fs.FileSystem。
package toolpath
