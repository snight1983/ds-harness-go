...
//
// 为什么不是导出符号清单：符号清单管的是「这个函数落了没有」，那是字符级搬运。
// 我们要取的是**能力**——「进程崩在工具调用中间，重开后能说出结果未知」这种事。
// 用什么函数、几个函数、叫什么名字，Go 里自有 Go 的写法。
//
// 为什么能力清单也必须机器抽：如果由我来写「DSH 有哪些能力」，那等于我自己
// 定义了要取什么——我判断某块「用不上」于是不写进清单，这个判断就永远没人看见。
...
//
// 为什么需要这个：harness/agent 的 Agent 接口引用了 Session、Inbox、LlmCallConfig 等
...
//   - **不抽导出符号清单。**「这个函数落了没有」那一份由 internal/devtools/portmap 出，
//     两份清单回答的不是同一个问题。
//   - **不裁决。**某条能力取不取形、取到了哪儿，写在裁决表里，由 internal/devtools/rule
...
// slug 是这个包在清单里的短名，形如 harness/agent。
...
func writeMarkdown(path string, capabilities []Capability, cycles []string) error {
	...
	builder.WriteString("由 `internal/devtools/capmap` 生成，重跑覆盖。\n\n")
