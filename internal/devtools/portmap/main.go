...
// 生成一份可复现的清单。这份清单是「落全了没有」的唯一基准。
//
// 为什么需要它：这个移植工作最大的风险不是写错，是**悄悄漏掉**——我判断某个东西
// 「用不上」于是不取，而这个判断从来没有被人看见过。所以基准不能由人来写，只能由
// 脚本从原始源码里抽。人只被允许往清单上填「Go 里的对应物」或者「不取 + 理由」，
// 一行都不许删。
//
// 三条刻意的设计：
//
...
//     判红的是 internal/devtools/portcheck。
//   - **不裁决。**清单只记「DSH 导出了什么」，取不取形、取到了哪儿写在裁决表里，
//     由 internal/devtools/rule 一行一行填。
//   - **不删行，也不静默跳过。**解析不了的行落成 UNPARSED 进清单——一个没人看见的
//     解析失败，和一次故意的省略，在结果上完全一样。
//   - **不读 Go 侧源码。**Go 里有没有对应物、填进去那个名字存不存在，
//     由 internal/devtools/portcheck 验。
//   - **不抽能力清单。**「这个包干什么」那一份由 internal/devtools/capmap 出。
package main
...
type Export struct {
	Package string // 包路径，形如 harness/session
