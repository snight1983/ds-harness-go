...
func Test分区按路径分而不是按包名(t *testing.T) {
	cases := map[string]zone{
		"adapter/datastore/medium.go":               zoneDatastore,
		"adapter/datastore/sessionstore/backend.go": zoneDatastore,
		"cmd/llmmockserver/main.go":                 zoneCmd,
		"internal/devtools/doccheck/check.go":       zoneTools,
		"internal/devtools/dbcheck/check.go":        zoneExempt,
		"feature/persistence/backend.go":            zoneBusiness,
...
`
	findings := findingsIn(t, "feature/persistence/backend.go", source)
...
`
	if findings := findingsIn(t, "adapter/datastore/medium.go", source); len(findings) != 0 {
		t.Fatalf("adapter/datastore 底下不该判红，实际 %v", findings)
	}
}
...
var _ = datastore.Config{}
`
	findings := findingsIn(t, "internal/devtools/portcheck/main.go", source)
...
func Test该跳过的目录跳过了(t *testing.T) {
	...
	for _, name := range []string{"adapter", "sessionlog", "tools"} {
		if skipDir(name) {
