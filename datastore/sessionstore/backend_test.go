...
func seededBackend(t *testing.T, id session.SessionID, base int) (*Backend, session.SessionHeader) {
	t.Helper()
...
func Test带NUL的事件存得下也读得回(t *testing.T) {
	...
	if err := backend.AppendBatch(t.Context(), meta, []session.Event{event}, false); err != nil {
...
func TestCommitRepair把缺的收尾补上(t *testing.T) {
	...
	if err := backend.AppendBatch(t.Context(), meta, []session.Event{
	...
	if err := backend.CommitRepair(t.Context(), meta, nil, []session.Event{
		turnEndEvent(t, 2, 1),
	}); err != nil {
...
func TestList和ListSnapshots看得见每一个已落地的会话(t *testing.T) {
	...
	for _, id := range []session.SessionID{"alpha", "beta"} {
