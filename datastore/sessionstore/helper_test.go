...
import (
	...
	"github.com/snight1983/ds-harness-go/datastore/dbtest"
...
func testMeta(id session.SessionID) session.SessionHeader {
...
func oneTurnLog(t testing.TB, base int) []session.Event {
	t.Helper()
...
func turnStartEvent(t testing.TB, seq, turn int) session.Event {
	t.Helper()

	return session.Event{
		Type: session.EventTurnStart, Seq: seq, Time: int64(seq + 1),
		Data: marshalData(t, "回合开始", session.TurnStartData{Turn: turn}),
	}
}

func turnEndEvent(t testing.TB, seq, turn int) session.Event {
	t.Helper()

	return session.Event{
		Type: session.EventTurnEnd, Seq: seq, Time: int64(seq + 1),
...
func stepStartEvent(t testing.TB, seq, turn, step int) session.Event {
	t.Helper()

	return session.Event{
		Type: session.EventStepStart, Seq: seq, Time: int64(seq + 1),
		Data: marshalData(t, "步骤开始", session.StepStartData{Turn: turn, Step: step}),
	}
}

func stepEndEvent(t testing.TB, seq, turn, step int) session.Event {
	t.Helper()

	return session.Event{
		Type: session.EventStepEnd, Seq: seq, Time: int64(seq + 1),
		Data: marshalData(t, "步骤结束", session.StepEndData{Turn: turn, Step: step}),
	}
}

func userMessageEvent(t testing.TB, seq int, text string) session.Event {
	t.Helper()

	return session.Event{
		Type: session.EventUserMessage, Seq: seq, Time: int64(seq + 1),
...
func assistantMessageEvent(t testing.TB, seq, turn, step int, text string) session.Event {
	t.Helper()

	return session.Event{
		Type: session.EventAssistantMessage, Seq: seq, Time: int64(seq + 1),
...
func mustCreate(t *testing.T, store *Store, meta session.SessionHeader) {
	t.Helper()
...
func mustAppend(t *testing.T, store *Store, id session.SessionID, events []session.Event) {
	t.Helper()
...
func evictHead(t *testing.T, backend *Backend, id session.SessionID, belowSeq int) {
	t.Helper()
...
func writeRaw(
	t *testing.T, backend *Backend, id session.SessionID, head json.RawMessage, entries []datastore.Entry,
) {
	t.Helper()
