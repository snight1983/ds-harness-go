...
//
// 新增: 它说的是「一条条目的负载是 [session.Event] 的第几版编码」，和
// [session.SessionHeader.Version] 说的「这份日志是第几版」不是一回事——
...
func encodeHeader(meta session.SessionHeader) (json.RawMessage, error) {
	payload, err := json.Marshal(meta)
	if err != nil {
		return nil, fmt.Errorf("datastore/sessionstore: 会话 %q 的头排不出去：%w", string(meta.ID), err)
	}
	return payload, nil
}
...
func decodeHeader(id session.SessionID, payload json.RawMessage) (session.SessionHeader, error) {
	var meta session.SessionHeader
	if err := json.Unmarshal(payload, &meta); err != nil {
		return session.SessionHeader{}, &persistence.CorruptionError{
			ID:    id,
...
func decodeEvents(id session.SessionID, entries []datastore.Entry) ([]session.Event, error) {
	if len(entries) == 0 {
		return nil, nil
	}
...
func (b *Backend) List(ctx context.Context) ([]session.SessionHeader, error) {
	...
	headers := make([]session.SessionHeader, 0, len(snapshots))
...
func (b *Backend) ListSnapshots(ctx context.Context) ([]persistence.Snapshot, error) {
	...
	for _, stream := range streams {
		meta, err := decodeHeader(session.SessionID(stream.Name), stream.Head)
		if err != nil {
			return nil, err
		}
...
func (b *Backend) CommitRepair(
	...
	if torn != nil {
		return fmt.Errorf(
			"datastore/sessionstore: 收到一张断尾凭据（%T），而这个后端从不发断尾凭据", torn)
	}
	if len(closers) == 0 {
		return nil
	}
...
func (b *Backend) write(
	...
	for _, event := range events {
		...
		if err != nil {
			return fmt.Errorf("datastore/sessionstore: 会话 %q 的 seq %d 那条 event 排不出去：%w",
	...
	case errors.Is(err, datastore.ErrHeadConflict):
		...
		return fmt.Errorf(
			"datastore/sessionstore: 会话 %q 已经落地了，而且那份头和要落地的这份不一样，撞号了",
			string(meta.ID))
...
func (b *Backend) TrimBefore(ctx context.Context, id session.SessionID, beforeSeq int) error {
	if beforeSeq < 0 {
		return fmt.Errorf(
