...
// 时限的既有写法（feature/sessionstats、[llm.Failure].ProviderRetryAfterMs），也正好
...
// 一个已经写完、有测试、还被 feature/llmretry 用着的包，只为了让这一个插件的设置
...
func resolveProfile(provider string, source ProviderProfile) (ResolvedProviderProfile, error) {
	...
	// 所以那条上界不存在。理由和 [github.com/snight1983/ds-harness-go/feature/timeout.NewWatchdog] 上
	// 那条逐字相同。
