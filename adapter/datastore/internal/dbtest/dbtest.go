// Package dbtest 把「这一轮测试跑在哪种库上」这件事定下来，交给 datastore 底下
// 那几个测试包共用。
//
// 新增: 整个包都是本仓库自有的。
//
...
// # 不做什么
//
//   - **不对外发布。**它收在 internal 底下，只有 [datastore] 这棵子树引得到；
//     一个「挑测试跑在哪种库上」的公开包没有意义。
//   - **不建表、不管迁移。**它只交出一份连上的介质，建表那一步仍归 [datastore]。
//   - **不给 [datastore] 自己的测试用。**那边的用例写在包内、又被本包引着，
//     引回去就成环。
//   - **不做假库。**不拿 sqlmock 之类的东西凑覆盖率——那验的是「我拼出了我以为我会
//     拼的那句 SQL」，而真会出事的地方恰恰是假库看不见的。
//   - **不替生产进程挂驱动。**驱动只在这里空导入，是为了让这批测试跑起来；
//     生产那边挂哪个驱动由装配方决定。
package dbtest
