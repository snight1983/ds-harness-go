...
// 装配方交进来的 [github.com/snight1983/ds-harness-go/harness/agent.Registry] 自然满足它。
...
type Config struct {
	...
	// （见 github.com/snight1983/ds-harness-go/feature/workspace 那台注册表），测试因此不必靠真的时钟。
	Now func() time.Time
...
type jobLayer struct {
	...
	// [github.com/snight1983/ds-harness-go/scope.AnonymousEntries] 本来就是每次追加各发一个撤销
...
func (r *Registry) List(_ context.Context, caller agent.Agent) ([]jobs.Snapshot, error) {
	var callerSession sessionlog.SessionID
	if caller != nil {
...
//
// 新增: 形参叫 limit 而不是 timeout，让路给同名的 [github.com/snight1983/ds-harness-go/feature/timeout] 包。
