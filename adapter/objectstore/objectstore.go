...
// # 不做什么
//
//   - **不实现 [fs.OSPathFileSystem]。**这里没有子进程、没有文件系统命名空间，
//     一个对象既没有进程能打开的路径也没有 file: URI——这是静态事实，交给类型系统说。
//   - **不做符号链接。**一个对象只有一个键，[Store.Lstat] 和 [Store.Stat] 看到的是
//     同一件事，永远不会报 [fs.TypeSymlink]。
//   - **不碰本机磁盘。**读本机磁盘和读沙箱的那两个后端在本仓库是范围外；本包只跟
//     S3 兼容端点说话。
//   - **不做沙箱、租户根和授权规则。**前缀圈出的只是一个执行世界的根，策略由装配方
//     和上层的 Policy 决定。
//   - **不在构造时写对象。**[Store.VerifyCreateIfAbsent] 要由部署方或集成测试显式
//     调一次——构造函数不该偷偷往桶里写东西。
//   - **不保留 CRLF。**文本那几条路进出两侧都折成 LF；要原样字节的调用方走
//     [Store.ReadBytes]。
package objectstore
