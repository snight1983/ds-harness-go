...
//
// 新增: 整个文件都是本仓库自有的。它和
// [github.com/snight1983/ds-harness-go/adapter/localjobs] 那台注册表实现的是同一条缝
...
//
// 和 [github.com/snight1983/ds-harness-go/adapter/localjobs.TaskWaitTimeout] 同值：
// 它是 [github.com/snight1983/ds-harness-go/feature/timeout] 那一侧用来认领期限的代号，
// 两台注册表在这件事上没有分歧。
...
type Registry struct {
	...
	// [github.com/snight1983/ds-harness-go/adapter/localjobs]，见 subscribe.go。
...
func (r *Registry) admit(spec jobs.Start) error {
	if !r.servesOwner(spec.Owner) {
		return errors.New("background jobs unavailable: no job controller serves this agent " +
			"(load github.com/snight1983/ds-harness-go/feature/jobs/jobstool in its composition)")
	}
	if spec.Kind == "" {
...
// [github.com/snight1983/ds-harness-go/adapter/localjobs] 那一份。
...
// [github.com/snight1983/ds-harness-go/feature/timeout] 包。
...
func sessionOf(who agent.Agent) sessionlog.SessionID {
	if who == nil {
		return ""
	}
	return who.ID()
...
func (r *Registry) ensureOwnerCleanup(owner agent.Agent) error {
	if r.agents == nil {
		return errors.New("background job ownership requires the agent registry " +
			"(load github.com/snight1983/ds-harness-go/harness/agent)")
	}
